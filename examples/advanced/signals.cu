/*Raul P. Pelaez 2021. Example showing how to connect to ParticleData signals.
  ...
  Say the class called "User" wants to know when some particle property has been
  modified. For example because it needs to keep something up to date when the
  positions are modified. The class "User" might be an Integrator, Interactor, a
  Potential, a Transverser... or anything outside the UAMMD framework.
...
// the positions and/or the velocities of the particles with the intention of
// writting to them.
class User {
...
public:
  User(std::shared_ptr<ParticleData> pd) : pd(pd) {
    ...
    positionConnection = pd->getPosWriteRequestedSignal()->connect(
        [this]() { this->handle_pos_access(); });
    velocityConnection = pd->getVelWriteRequestedSignal()->connect(
        [this]() { this->handle_vel_access(); });
  }
  ~User() {
  ...
  // Say this function needs to be sure that it has processed the current
  // positions and velocities. Instead of assuming both have changed everytime
  // the function is called we can rely on the signals
  //  to do work only when any of them changed
  // Say this variable is the result of some operation applied to positions in
  // .x and velocities in .y
  real2 currentResults = {-1, -1};
  real2 some_work_using_current_pos_and_vel() {
    // No work needs to be done if neither pos nor vel were modified since last
    // call
    if (not pos_needs_processing and not vel_needs_processing) {
      System::log<System::MESSAGE>("[User] No work needs to be done");
      return currentResults;
    }
    // Only do work with positions if we know it has been potentially modified
    // since last call
    if (pos_needs_processing) {
      System::log<System::MESSAGE>(
          "[User] Doing position-dependent computation");
      // Be careful with writing to pos here, as it will result in the signal
      // emitting again and thus beating the purpose
      auto pos = pd->getPos(access::location::cpu, access::mode::read);
      // The center of mass in X, just because
      currentResults.x = std::accumulate(pos.begin(), pos.end(), real4()).x /
                         pd->getNumParticles();
      // No work needs to be done until the next time positions change
      pos_needs_processing = false;
    }
    // Only do work with velocities if we know it has been potentially modified
    // since last call
    if (vel_needs_processing) {
      System::log<System::MESSAGE>(
          "[User] Doing velocity-dependent computation");
    ...
    // Notice that if this function is called two times, without modifying pos
    // or vel in between, the second time
    //  will perform no work and simply return the stored value
    return currentResults;
  }

private:
  // This function will be called when pd->getPos() is called with
  // access::mode::write or readwrite.
  void handle_pos_access() {
    // Notice that this function is called when the property has been requested,
    // so when this function is called The positions have not been modified yet.
  ...
  // This function will be called when pd->getVel() is called with
  // access::mode::write or readwrite.
...
// Try to run this and see the output messages, you will see something like
// this:
int main(int argc, char *argv[]) {
  ...
  // User construction will connect to pd such that it will be informed when
  // positions or velocities are requested for writting.
  User user(pd);
  real2 res;
  ...
  modify_positions(pd);
  // When pd->getPos(...write) was called the "user" instance was informed via
  // signals and User::handle_pos_access was called Now some work must be done
  // to take into account the access to position
  res = user.some_work_using_current_pos_and_vel();
