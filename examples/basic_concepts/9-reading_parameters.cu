...
//Lets define some parameters like before, but this time lets not hardcode their values here
//We will read them from a parameter file
struct Parameters{
  int numberParticles;
  real3 boxSize;
  real viscosity;
...
//This function will read the file called datamain and fill and return a Parameters struct with it
Parameters readParameters(std::string datamain);

//Probably it is a good idea to write default parameters if the expected file does not exists
...
int main(){
  ...
  //Try to remove some parameter from data.main and running again, if it is marked as Required in readParameters and error will be thrown.
  return 0;
};
...
void writeDefaultParameters(std::string datamain){
  ...
  out<<"boxSize 32 32 32 #As will anything beyond the expected number of arguments\n";
  out<<"numberParticles 128\n";
  out<<"viscosity 1\n";
  out<<"dt 1\n";
  out<<"someFileName example.dat\n";
  out<<"someOption\n";
  out<<"#The special shell option will run the rest of the line and wait for completion when it is read by InputFile\n";
...
Parameters readParameters(std::string datamain){
  ...
  //When the second argument is Required InputFile will terminate the program with an error if the option in the first
  // argument is not found,
  //You can pass Optional instead, which will silently return an empty stringstream when the option is not present
  ...
  //You can have parameters without arguments and check if they exists or not like this
