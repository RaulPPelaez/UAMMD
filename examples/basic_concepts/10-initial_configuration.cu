/* Raul P. Pelaez 2021
   ...
   You can generate the initial state of the particles by reading it from a file
   or making it random, but in this tutorial we will learn about the
   InitialConditions.cuh header, which provices a convenient function that
   places positions in a lattice.
...
UAMMD initializeUAMMD(int argc, char *argv[]) {
  ...
  constexpr int numberParticles =
      16384; // Lets just hardcode a number of particles
...
void initializePositionsInALattice(UAMMD sim) {
  // initLattice will return a vector with numberParticles positions inside a
  // box of size L following a certain lattice. In this case we chose an FCC
  // lattice, but there are others, as sc (simple cubic), bcc, hcp,...
...
int main(int argc, char *argv[]) {
  ...
  // Doing this might be useful to start simulations with hard potentials that
  // cannot handle overlapping particles Destroy the UAMMD environment and exit
