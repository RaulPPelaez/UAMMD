...
public:
  MyFirstInteractor(std::shared_ptr<ParticleData> pd)
      : // The base class (Interactor) constructor must then be invoked with
  ...
  // It can be called at any time and it is expected to fill the forces,
  // energies and/or virials of the particles based on their current state.
  //"sum" must always sum to the existing properties, never overwritting them.
  // Our simple Interactor will just add some force and energy to the first
  // particle in the X direction. Also, we will make this interaction turn off
  // after a certain arbitrary time.
  //  As an exercise you can let your imagination fly here and implement any
  //  arbitrarily complex interaction.
  // See for instance the file Interactor/SpectralEwaldPoisson.cuh, which
  // contains a complex electrostatics interactor.
  void sum(Computables comp, cudaStream_t st = 0) override {
    // Like in Integrator, being an Interactor makes some members available to
    // us: pd: An instance of ParticleData pg: An instance of ParticleGroup
    // (something we will discuss in future tutorials). sys: An instance of
    // System
    sys->log<System::DEBUG2>("[MyFirstInteractor] Computing interaction");
    // The argument "comp" contains a series of booleans, if one of them is true
    // this call is supposed to sum to
    //  the relevant ParticleData array. Ideally, if this function is called
    //  with some requirement that the Interactor is not able to satisfy, the
    //  call should throw an error. For instance, suppose our Interactor is not
    //  capable of computing the virial:
    const real turnOffTime = 10;
    if (this->currentSimulationTime < turnOffTime) {
      if (comp.virial) {
        System::log<System::EXCEPTION>(
            "[MyFirstInteractor] I cannot compute the virial");
  ...
  // We already discussed the ParameterUpdatable interface in the previous
  // tutorial. We saw there that every Interactor is ParameterUpdatable by
  // default. In principle, the Interactor is unaware of time, but let us
  // imagine that our interaction changes with time. We can use the
  // ParameterUpdatable mechanism to make our Interactor aware of time by
  // overriding one function from it. In this case updateSimulationTime:
  void updateSimulationTime(real newTime) override {
    // Everytime the Integrator::forwardTime is called the Integrator will
    // update the simulation time of all its interactors. This will include this
    // interactor.
...
// When pulling one particle, as our Interactor is doing, we will see how the
// surrounding ones are dragged with it.
...
int main() {
  ...
  // Our new Interactor is just like every other, so we can add it to an
  // Integrator
  ...
  // As an exercise, you can evolve the simulation for some time and visualize
  // the results.
  fcm->forwardTime();
  return 0;
}
