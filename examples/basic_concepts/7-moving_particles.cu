...
// Now that we are closing in a full blown particle simulation, lets take the
// chance to encapsulate in functions what we have learned in previous
// tutorials. Consider this a suggestion based on how I usually organize these
// UAMMD simulation codes.
...
struct Parameters {
  ...
  // Since the diffusion coefficient of a spherical particle in BD is D =
  // T/(6*pi*viscosity*hydrodynamicRadius)
...
void randomlyPlaceParticles(UAMMD sim) {
  ...
  std::generate(positions.begin(), positions.end(), [&]() {
    return make_real4(rng(), rng(), rng(), 0) * sim.par.boxSize;
  });
}
...
std::shared_ptr<Integrator> createBrownianDynamicsIntegrator(UAMMD sim) {
  ...
  // First we choose the Integrator, in this case we are going to use the most
  // basic Brownian Dynamics Integrator, Euler Maruyama
  using BD = BD::EulerMaruyama;
  // You can see that there are several other integration algorithms in the BD
  // namespace, if you want to know more about them
  ...
  // You should check the relevant doc page for information on how to construct
  // each integrator, but 99% of the time you will see The arguments are the
  ...
  // base class Integrator. Meaning that they can pass as one This means that
  // you can store any integrator in a variable with type
...
int main(int argc, char *argv[]) {
  ...
  // Now lets create an Integrator, it has the ability to take the current state
  // of the particles and take it to the next time step.
  auto bd = createBrownianDynamicsIntegrator(sim);
  // The main function in the Integrator interface (and the only one we are
  // going to use in this tutorial) is Integrator::forwardTime. This function
  // takes the simulation state at the current time, t0, (usually encoded in the
  // relevant particle properties) and takes it to the next, t0+dt In the case
  // of a Brownian dynamics simulation of ideal particles, this means moving the
  // particles randomly according to a certain diffusion coeficient (which we
  // set to 1). Every other particle property (such as velocities) is irrelevant
  // and thus left untouched. First, lets print the first 10 particles
  printFirst10Particles(sim);
  // Now lets forward time:
  bd->forwardTime();
  // And print the positions again:
  printFirst10Particles(sim);
  // You can see particles moved a bit. Thats it, a Brownian Dynamics simulation
  // of ideal particles. Now you can call forwardTime as many times as needed.
  // Now that we are here, lets also check that the integration is correct.
  // Given that no forces are affecting particles their movement should come
  // only from thermal noise, AKA diffusion. We set the diffusion coefficient to
  // 1 and dt is 0.1, so the displacement of each particle after a step
  // (independently on each direction) should have zero mean and standard
  // deviation D*dt=0.1, lets check it Lets store particle positions:
  ...
  std::transform(positions_after.begin(), positions_after.end(),
                 positions_previous.begin(), std::back_inserter(displacements),
                 std::minus<real4>());
  // The values in this vector should have zero mean:
  // In fact, since we are basically summing gaussian random numbers of mean 0
  // and std sqrt(2*dt) the mean should be on the order of
  //  1/sqrt(N=1e5) = 0.003
  real3 mean = make_real3(std::accumulate(displacements.begin(),
                                          displacements.end(), real4())) /
               displacements.size();
  std::cout << "Mean displacement in each direction: " << mean
            << " expected something of the order of: "
            << 1 / sqrt(sim.par.numberParticles) << std::endl;
  // Lets now check the standard deviation:
  real3 stddev = sqrt(thrust::transform_reduce(
                          displacements.begin(), displacements.end(),
                          [=](auto x) {
                            auto d = make_real3(x) - mean;
                            return d * d;
                          },
                          real3(), std::plus<real3>()) /
                      displacements.size());
  // The standard deviation should be sqrt(2*D*dt) = sqrt(2*0.1)
  // Lets print the diffusion coefficient, which should be 1 -> D =
  // stddev/sqrt(2*0.1)
  std::cout
      << "Measured diffusion coefficient in each direction (should be 1): "
      << stddev / sqrt(2 * 0.1) << std::endl;
  // You might think the algorithmic way in which we wrote this is unnecesary
  // here, but with minimal changes
  //  we could have performed all post processing operations in the GPU, so this
  //  is good training.
  // Had we used C-style loops for this, porting it to the GPU would result in
  // quite different (much more complex) code.

  // Integrators provide a couple more functions, you can check them in the
  // relevant wiki page if you are impatient, but we will soon cover them.
