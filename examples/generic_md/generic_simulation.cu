...
Once compiled a lot of stuff can be chosen via a file called "data.main", which
will be autogenerated with instructions if not present. Start by compiling and
running this file (which will also serve to check your CUDA environment), then
read the data.main file that was generated.

If the wide variety of simulations that this code offers as-is does not meet
your needs, not all hope is lost yet. Please check customizations.cuh, were you
can find several ways to extend and adapt this code.

As already mentioned, the data.main covers the kind of simulation this code can
perform, but for completeness it is repeated here: Using this code, you can
fabricate via data.main a simulation using the following components:

Integration schemes (one must be chosen):
  -VerletNVT: Molecular dynamics with a thermostat.
  -VerletNVE: Molecular dynamics with constant energy (starting with a velocity
distribution to match the given temperature). -BD: Brownian dynamics -BDHI:
Brownian dynamics with Hydrodynamic interactions (via positively split Ewald
Rotne-Prager-Yamakawa or Force Coupling Method) -SPH: Smooth Particle
Hydrodynamics -DPD: Dissipative Particle Dynamics -FIB: Fluctuating Immersed
Boundary -ICM: Inertial Coupling Method
...
Interaction modules:
  ...
  -External potential: An external force acting on each particle. Gravity + a
wall in this example* -Electrostatics: Periodic electrostatics using an Ewald
splitted spectral Poisson solver.

*The different potentials are taken from the file customizations.cuh
accompanying this code, you can modify it to your needs.

Parameters:
Compile and run this code to generate a data.main with default parameters and
explanations for each of them. Once the data.main has been customized, run the
program again. The default data.main will run a LJ liquid MD simulation.

The code will output only the positions of the particles each print frame. With
each frame separated by a line with "#".

Additional notes:
All simulations that can be performed via this file assume a periodic box. But
you can put an arbitrarily large box size. Keep in mind though that some modules
encode algorithms that are inherently periodic, like electrostatics and
hydrodynamics. These inifinite ranged interactions make a big box not equivalent
to an open system.

This code is also intended to be a skeleton code if you are trying to write some
kind of UAMMD simulation code. For example, the function initialize (which
starts the basic UAMMD structures and the particles) is probably going to appear
everytime you write an UAMMD simulation. If you need to create a BD integrator,
you can copy paste the function createIntegratorBD. Need to read parameters from
a file? just copy paste the function readParameters. And so on.

If you would like a more bottom up approach to UAMMD, you can surf the
examples/basic folder, which will give you through UAMMD with an increasingly
complex set of example codes. Additionally, you can drop by the wiki:
https://github.com/RaulPPelaez/UAMMD/wiki
Which has a lot of information. From basic functionality to descriptions and
references for the algorithms implemented by each module.
*/

// Each UAMMD module has its own header, with uammd.cuh being the header with
...
// Read a data.main file from "fileName" and return the contents as a Parameters
// struct.
Parameters readParameters(std::string fileName);

// This function reads numberParticles lines from a file and returns the
// contents in a std::vector This function will assume the format of the file is
// a list of objects of type "T". For example, in order to read 10 positions
// (formatted as X Y Z per particle) from file "pos.init": auto pos_in_file =
// readPropertyFromFile<real3>("pos.init", 10);
template <class T>
std::vector<T> readPropertyFromFile(std::string file, int numberParticles) {
  ...
  if (data_in_file.size() < numberParticles) {
    System::log<System::CRITICAL>(
        "Not enough lines in file %s, expected %d, found %d", file.c_str(),
        numberParticles, data_in_file.size());
  }
  data_in_file.resize(numberParticles);
  return data_in_file;
}

// Given a file name, reads positions from it, the format of file is a list of
// lines containing 3 numbers each, X Y Z for each particle. Only the number of
// particles given in the data.main will be read into positions.
void initializePositionsFromFile(std::string file,
                                 std::shared_ptr<ParticleData> pd) {
  int numberParticles = pd->getNumParticles();
  auto pos_file = readPropertyFromFile<real3>(file, numberParticles);
  auto pos = pd->getPos(access::location::cpu, access::mode::write);
  std::transform(pos_file.begin(), pos_file.end(), pos.begin(),
                 [](real3 p) { return make_real4(p); });
}

// Initialize positions in an face centered cubic lattice, notice that several
// other lattices are available in initLattice, such as sc (simple cubic).
void initializePositionsInFCCLattice(std::shared_ptr<ParticleData> pd,
                                     real3 L) {
  int numberParticles = pd->getNumParticles();
  auto pos = pd->getPos(access::location::cpu, access::mode::write);
...
// Given a file name, reads the  charges from it, the format of file is a list
// of charges. The number of particles given in the data.main will be read into
// uammd charges.
void initializeChargesFromFile(std::string file,
                               std::shared_ptr<ParticleData> pd) {
  int numberParticles = pd->getNumParticles();
...
// this function will initialize particle velocities following a Boltzmann
// Distribution according to the given temperature
void initVelocitiesBoltzmannDistribution(std::shared_ptr<ParticleData> pd,
                                         real temperature) {
...
// Initialize particle properties, some properties are only initialized as a
// function of the parameters in the data.main If no initial position file is
// given in data.main's "readFile" positions are initialized in an FCC lattice
// Charges are only initialized if useElectrostatics is true.
// Masses are always initialzied to 1
// If a verletNVE integrator has beeen chosen, velocities are initialized
// following a Boltzmann distribution with the temperature in the data.main
// Execution is then handed over to furtherParticleInitialization in
// customizations.cuh for additional particle startup operations (if any).
std::shared_ptr<ParticleData> initializeParticles(std::shared_ptr<System> sys,
                                                  Parameters par) {
...
// data.main will be named "data.main" unless at least an argument is passed to
// the program, in which case the data.main name will be the first argument
UAMMD initialize(int argc, char *argv[]) {
  UAMMD sim;
  // System can optionally be handed argc/argv. In this case some command line
  // options can be passed (see wiki for a list). Things like the used GPU are
  // selected through this.
  auto sys = std::make_shared<System>(argc, argv);
  // System provides an uammd-wide random generator, which can be seeded like
  // this
  std::random_device r;
  sys->rng().setSeed(r());
...
/*
  The family of functions below are easily copy pastable and will create and
  return instances of Integrators/Interactors
 */
...
auto createIntegratorVerletNVE(UAMMD sim) {
  ...
  par.initVelocities = false; // If true, velocities will be initialized by the
                              // module to ensure the desired energy
  // Note that it does not make sense to pass an energy and prevent VerletNVE
  // from initializing velocities to match it.
...
auto createIntegrator(UAMMD sim) {
  ...
  if (sim.par.integrator.compare("BD") == 0) {
    ...
    System::log<System::CRITICAL>("Invalid integrator. Choose BD, BDHI, FIB, "
                                  "ICM, VerletNVT, VerletNVE, SPH or DPD");
  }
  return integrator;
}

auto createShortRangeInteractor(UAMMD sim) {
  // The potential used is defined in customizations.cuh and describes an LJ
...
auto createBondInteractor(UAMMD sim) {
  ...
  auto bf =
      std::make_shared<BF>(sim.pd, params, std::make_shared<Bond>(sim.par));
  return bf;
}
...
auto createElectrostaticInteractor(UAMMD sim) {
  // Similar to the hydrodynamics modules, electrostatics are available in two
  // algorithms, in this case exposed under the same module One works best for
  // dilute/big systems (Ewald splitting) and the other for dense/small systems
  // (with no Ewald splitting). Ewald splitting is automatically selected
  // according to some system size heuristic. As with BDHI, Ewald splitting is
  // slower but provides an algorithm that scales linearly with the number of
  // particles independently of system size. Non-Ewald splitted version scales
  // linearly with number of particles but also scales linear with system size.
  ...
  if (maxcells >= 128) {
    // Controls Ewald splitting, if the parameter is nor present no Ewald
    // splitting is used.
...
double sumTotalEnergy(std::shared_ptr<Integrator> integrator,
                      std::shared_ptr<ParticleData> particles) {
  {
    auto energy =
        particles->getEnergy(access::location::gpu, access::mode::write);
  ...
  double totalEnergy =
      thrust::reduce(thrust::cuda::par, energy.begin(), energy.end(), 0.0);
  return totalEnergy;
}
...
void writeSimulation(UAMMD sim) {
  // Particles might change ordering once in a while, this array always points
  // to the index of a particle given its name or id. This means that a particle
  // that started at index "i" (and thus has the id or name "i") will always be
  // accesible via id2index[i]
  ...
  if (eout.good())
    eout << std::setprecision(2 * sizeof(real))
         << "# Total energy: " << sumTotalEnergy(sim.integrator, sim.pd)
         << std::endl;
  ...
  auto vel =
      sim.pd->getVelIfAllocated(access::location::cpu, access::mode::read);

  auto energy =
      sim.pd->getEnergyIfAllocated(access::location::cpu, access::mode::read);
...
int main(int argc, char *argv[]) {
  ...
  forj(0, sim.par.relaxSteps) { bd->forwardTime(); }
  Timer tim;
  tim.tic();
...
void writeDefaultDatamain(std::string datamain) {
  ...
  out << "#data.main format is [option] [argument1] [argument2]..."
      << std::endl;
  ...
  out << "#The code generic_simulation.cu will read this file and run the "
         "simulation it describes"
      << std::endl;
  out << "#Below, you can tweak or turn off and on the different "
         "functionalities"
      << std::endl;
  ...
  out << "################################################################"
      << std::endl;
  out << "#Choose the integration scheme, it can be any of the following:"
      << std::endl;
  out << "#  -VerletNVT: Molecular dynamics with a thermostat." << std::endl;
  out << "#  -VerletNVE: Molecular dynamics with constant energy (starting "
         "with a velocity distribution to match the given temperature)."
      << std::endl;
  out << "#  -BD: Brownian dynamics" << std::endl;
  out << "#  -BDHI: Brownian dynamics with Hydrodynamic interactions (via "
         "positively split Ewald Rotne-Prager-Yamakawa or Force Coupling "
         "method)"
      << std::endl;
  ...
  out << "################################################################"
      << std::endl;
  out << "#Integrator-specific parameters. Only the parameters for the chosen "
         "integrator have to be present (they will be ignored for the rest)"
      << std::endl;
  ...
  out << "#Where density is computed as a function of the masses of the "
      ...
      << std::endl;
  out << "#Particle mass starts as 1, but you can change this in "
         "customizations.cuh"
      << std::endl;
  ...
  out << "#The hydrodynamic radius of the particles will be used only in BD "
         "and BDHI."
      << std::endl;
  ...
  out << "epsilon 1 #Strength of the LJ potential, 0 turns off the interaction"
      << std::endl;
  out << "cutOff  2.5 #CufOff of the LJ potential, 2^(1/6)*sigma will result "
         "in an WCA potential"
      << std::endl;
  out << "#################################################################"
      << std::endl;
  ...
  out << "#If not present no bonds will be placed between particles"
      << std::endl;
  out << "#The bond file is a list of bonds (joining particle \"i\" with \"j\" "
         "with an spring of constant \"Kspring\" and eq. distance \"r0\")."
      << std::endl;
  out << "#There can also be bonds joining particles with points in space, "
         "these are placed after the particle-particle bonds."
      << std::endl;
  ...
  out << "#If not present, no three-particle angular bonds will be placed, "
         "similar to the pair-bond file, the format of the angular bonds file "
         "is:"
      << std::endl;
  out << "#numberOfBonds" << std::endl;
  out << "#i j k Kspring equilibrium_angle" << std::endl;
  out << "#..." << std::endl;
  out << "#Notice that in this case the bond involves particles i,j and k "
         "(being j the middle one). i-j-k"
      << std::endl;
  out << "" << std::endl;
  out << "#torsionalBondFile tor.bonds" << std::endl;
  out << "#If not present, no four-particle torsional bonds will be placed, "
         "this time the file format is:"
      << std::endl;
  out << "#numberOfBonds" << std::endl;
  out << "#i j k l Kspring equilibrium_angle" << std::endl;
  out << "#..." << std::endl;
  out << "#Notice that in this case the bond involves particles i,j,k and l. "
         "An eq. angle of 0 will result in the 4 particles being in the same "
         "plane."
      << std::endl;
  out << "" << std::endl;
  ...
  out << "#Charges are modeled as gaussian charge clouds, a point charge will "
         "be equivalent to gaussianWidth=0 (a limit which the algorithm cannot "
         "handle)."
      << std::endl;
  out << "#gaussianWidth 0.5" << std::endl;
  out << "#Optionally charges can be read from a file (a simple list of "
         "charges for each particle)."
      << std::endl;
  out << "#Charge defaults to 1 for all particles if this file is not present, "
         "again, you may customize this further in customizations.cuh"
      << std::endl;
  out << "#chargeReadFile charges.dat" << std::endl;
  out << "" << std::endl;
  out << "#Enable gravity and wall" << std::endl;
  out << "#If present, a wall will be placed at z=-Lz*0.5 and gravity will "
         "pull the particles in the z direction"
      << std::endl;
  ...
  out << "numberParticles 16384  #Number of particles, this number of lines "
         "will be read when reading files like \"readFile\""
      << std::endl;
  out << "#readFile init.pos     #If the option is present, initial positions "
         "will be read from here. Otherwise particles will start in an FCC "
         "lattice"
      << std::endl;
  out << "#The format of this file is simply a sequence of \"numberParticle\" "
         "lines with the positions of each particle as: X Y Z"
      << std::endl;
  out << "#Notice that /dev/stdin is a valid file name, in which case initial "
         "positions can be piped"
      << std::endl;
  out << "" << std::endl;
  out << "L 32 32 32             #Domain size in the three dimensions"
      << std::endl;
  out << "outfile /dev/stdout    #Positions will be written to this file"
      << std::endl;
  out << "#outfileVelocities /dev/null #If present velocities will be written "
         "to this file (if the integrator uses velocity)"
      << std::endl;
  out << "#outfileEnergy /dev/null    #If present per particle energy will be "
         "written to this file for each spanshot"
      << std::endl;
  out << "numberSteps 100000     #Number of simulation steps" << std::endl;
  out << "printSteps 500         #Positions will be writen to \"outfile\" "
         "every printSteps steps"
      << std::endl;
  out << "relaxSteps  100       #Run this number of steps before writting"
      << std::endl;
  out << "dt 0.001               #Time discretization step" << std::endl;
  out << "" << std::endl;
  out << "#You can use the special shell option to run something in a shell "
         "when the data.main is processed"
      << std::endl;
  out << "shell echo hello from data.main > /dev/stderr" << std::endl;
}

// Reads parameters from file with name given in datamain. If the file is not
// present, a default will be printed and the execution will end. This function
// calls readCustomParamters in customizations.cuh before exiting.
Parameters readParameters(std::string datamain) {
  if (not std::ifstream(datamain).good()) {
    System::log<System::WARNING>(
        "Parameter file %s not found, autogenerating and exiting",
        datamain.c_str());
  ...
  in.getOption("outfileVelocities", InputFile::Optional) >>
      par.outfileVelocities;
  ...
  in.getOption("torsionalBondFile", InputFile::Optional) >>
      par.torsionalBondFile;
  ...
  if (par.integrator.compare("SPH") == 0) {
    ...
    in.getOption("gasStiffness_sph", InputFile::Required) >>
        par.gasStiffness_sph;
  ...
  if (par.integrator.compare("BD") == 0 or
      par.integrator.compare("BDHI") == 0 or
      par.integrator.compare("FIB") == 0 or
      par.integrator.compare("ICM") == 0) {
