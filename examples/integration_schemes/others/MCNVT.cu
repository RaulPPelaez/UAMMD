/*Raul P. Pelaez 2018
  ...
  Will perform a simulation of a LJ liquid at a certain temperature and output
  positions and total internal energy (without the thermal energy).
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 10) {
    sys->log<System::CRITICAL>("[System] Not enough input arguments!. Look for "
                               "argv[ in the source code");
  }
  ...
  Box box(std::stod(argv[2]));
  {
    ...
    fori(0, N) {
      pos.raw()[i] = make_real4(
          sys->rng().uniform3(-box.boxSize.x * 0.5, box.boxSize.x * 0.5), 0);
      pos.raw()[i].w = 0;
    }
  }
  ofstream outPos("pos.dat"), outEnergy("energy.dat");
  using LJ = Potential::LJ;
  auto pot = make_shared<LJ>();
  {
  ...
  forj(0, nsteps) {
    ...
    if (j % printSteps == 1) {
      ...
      resetEnergy(pd);
      outEnergy << mc->sumEnergy() << " " << mc->getCurrentAcceptanceRatio()
                << " " << mc->getCurrentStepSize() << std::endl;
      outPos << "#" << endl;
      fori(0, N) {
        p = make_real3(pos[i]);
        int type = pos[i].w;
