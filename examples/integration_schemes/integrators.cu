/* Raul P. Pelaez 2021
   ...
   This code contains a collection of functions that create and return an
   instance of the different UAMMD integrators. The code in itself does not do
   much, rather it is intended  to serve as a copy pastable collection of
   snippets.
...
std::shared_ptr<Integrator> createIntegratorBD(UAMMD sim) {
  ...
  par.temperature = 1.0;
  par.viscosity = 1.0;
  par.hydrodynamicRadius = 1.0; // Self diffusion coefficient will be D = T*M =
                                // T/(6*pi*vis*hydrodynamicRadius)
  par.dt = 0.001;
  // Optionally you can place a shear matrix, dX = M*F*dt + sqrt(2*D*dt)*dW +
  // K*R par.K = {{1,2,3},{1,2,3},{1,2,3}}; or par.K[0] = {1,2,3}; or par.K[1].x
  // = 1; All K elements start being zero.
...
std::shared_ptr<Integrator> createIntegratorVerletNVT(UAMMD sim) {
  ...
  par.friction = 1.0;
  par.dt = 0.1;
  // If present, all particles will have this mass, otherwise the individual
  // particle masses in ParticleData will be used If those masses have not been
  // set then the default mass is 1.0. par.mass = 1.0; If set to false particle
  // velocities will be left untouched during initialization If true (default)
  // velocities will be sampled from the equilibrium configuration
...
std::shared_ptr<Integrator> createIntegratorVerletNVE(UAMMD sim) {
  ...
  par.dt = 0.1;
  par.energy = 1; // Optionally a target energy can be passed that VerletNVE
                  // will set according to velocities keep constant
  // par.initVelocities = false; //If true, velocities will be initialized by
  // the module to ensure the desired energy Note that it does not make sense to
  // pass an energy and prevent VerletNVE from initializing velocities to match
  // it. If present, all particles will have this mass, otherwise the individual
  // particle masses in ParticleData will be used If those masses have not been
  // set then the default mass is 1.0. par.mass = 1.0;
...
auto createIntegratorICM(UAMMD sim) {
  ...
  par.temperature = 1;
  // Choose if the thermal drift should be taken into account. Best leave it at
  // the default. par.sumThermalDrift = true; Additionally you can choose if the
  // total momemtum of the fluid should be substracted each step or not
  // par.sumTotalMomemtum = true;
  auto icm = std::make_shared<ICM>(sim.pd, par);
  // You can request the fluid velocities and the number of fluid cells from
  // this integrator
  ...
  // Note that the velocities are defined in a staggered grid, corresponding to
  // cell face centers.
  return icm;
}
...
auto createIntegratorICM_Compressible(UAMMD sim) {
  ...
  // determine the number of fluid cells)
  par.cellDim = {
      32, 32,
      32}; // Number of fluid cells, if set the hydrodynamicRadius is ignored
  par.dt = 0.1;
  ...
  par.initialVelocityX = [](real3 position) {
    return sin(2 * M_PI * position.y);
  };
...
// Creates a triply periodic Brownian Dynamics with Hydrodynamic Interactions
// integration module
std::shared_ptr<Integrator> createIntegratorBDHI(UAMMD sim) {
  // There are several hydrodynamics modules, we choose between Positively Split
  // Ewald (PSE) or Force Coupling Method (FCM) here
  //  mainly for performance reasons. FCM is faster for small and/or dense
  //  systems, but it is limited in the system size by memory. PSE can be slower
  //  it temperature>0, but does not have that system size constraints.
  // FCM scales linearly with system size (so doubling the box size in the three
  // dimensions makes it 8 times slower) and number of particles PSE scales
  // linearly with the number of particles, independently of system size. But
  // the "psi" parameter must be tweaked to find the optimal performance for
  // each case. See the wiki for more information about these modules
  real3 L = make_real3(32, 32, 32);
  real hydrodynamicRadius = 1.0;
  real maxL = std::max({L.x, L.y, L.z});
  int maxcells = maxL / hydrodynamicRadius;
  // In both modules, particle self diffusion coefficient will be
  // T/(6*pi*viscosity*hydrodynamicRadius) or close to it
  if (maxcells >= 128) {
    ...
    par.tolerance = 1e-4;
    // Balances the load of the algorithm, low values work best for dilute
    // and/or big systems.
...
std::shared_ptr<Integrator> createIntegratorFCM(UAMMD sim) {
  ...
  // In this mode, FCM can also compute angular displacements due to torques
  // acting on the particles See the wiki for more information about these
  // modules
  real3 L = make_real3(32, 32, 32);
  real hydrodynamicRadius = 1.0;
