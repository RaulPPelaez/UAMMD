/*Raul P. Pelaez 2019. SPH Dambreak problem example
  ...
  A cube of fluid particles falling from the upper left corner of a closed box,
  a wave is formed when the fluid front reaches the right wall. the fluid is
  modelated with Smoothed Particle Hydrodynamics.
...
int main(int argc, char *argv[]) {
  ...
  { // Initial positions
    ...
    std::transform(initial.begin(), initial.end(), pos.begin(), [&](real4 p) {
      p += make_real4(L.x / 8.0 - L.x * 0.49, 0, 0, 0);
      p.w = 0;
      return p;
    });
  }
  ...
  { // Add gravity+wall interactor
    auto gravity =
        make_shared<ExternalForces<Gravity>>(pd, make_shared<Gravity>(boxSize));
