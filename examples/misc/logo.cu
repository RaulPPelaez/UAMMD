...
Runs a Brownian Hydrodynamics simulation with particles starting in a periodic
box at low temperature.

Reads some parameters from a "data.main.logo" file, which will be autogenerated
if not present. You can visualize the reuslts with superpunto
...
struct Faller : public ParameterUpdatable {
  ...
  __device__ ForceEnergyVirial sum(Interactor::Computables comp,
    ...
    real fz = 0;
    if (pos.z <= zwall)
      fz += k * pow(fabs(pos.z - zwall), 2.0f);
...
int main(int argc, char *argv[]) {
  ...
  Box box(boxSize);
  {
    ...
    while (i < numberParticles) {
      ...
      real fac = 1.01;
      pos_t = make_real2(fac * pos_t.x * boxSize.x,
                         fac * pos_t.y * boxSize.x * (h / (real)w) +
                             boxSize.z * 0.5 -
                             fac * 0.5 * boxSize.x * (h / (real)w));
      pos[i] =
          make_real4(pos_t.x, sys->rng().uniform(-0.5, 0.5) * boxSize.y * 0.25,
                     pos_t.y, 0);
      i++;
    }
  }
  ...
  {
    auto extForces = make_shared<ExternalForces<Faller>>(
        pd, make_shared<Faller>(-box.boxSize.z * 0.5));
  ...
  forj(0, numberSteps) {
    if (j % printSteps == 0) {
      ...
      out << "#Lx=" << boxSize.x * 0.5 << ";Ly=" << boxSize.y * 0.5
          << ";Lz=" << boxSize.z * 0.5 << ";" << endl;
      real3 p;
