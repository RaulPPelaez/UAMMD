...
This makes the particles form a kind of pillars going from one type to the
other.
...
// An HarmonicWall functor to be used in a ExternalForces module (See
// ExternalForces.cuh)
struct HarmonicWall : public ParameterUpdatable {
  ...
  // This function must return a list of arrays whose elements force/energy will
  // need
  auto getArrays(ParticleData *pd) {
    auto pos = pd->getPos(access::gpu, access::read);
    return pos.begin();
    // If there are more arrays:
    // return std::make_tuple(pos.begin(), otherthing1.begin(),
    // otherthing2.begin());
  }

  // void updateSimulationTime(real time){
  // You can be aware of changes in some parameters by making the functor
  // ParameterUpdatable and overriding the update function you want, see
  // misc/ParameterUpdatable.h for a list
  //}
};
...
// Initialize the basic UAMMD structures and read parameters from the
// data.main.lj file
...
void initializeParticles(UAMMD sim) {
  ...
  // Start in an fcc lattice, pos.w contains the particle type, lets make it
  // ranfomly 0 or 1
...
void addExternalPotentialInteractions(std::shared_ptr<Integrator> verlet,
                                      UAMMD sim) {
  ...
  auto pg2 =
      make_shared<ParticleGroup>(particle_selector::Type(0), sim.pd, "12");
  auto pg3 =
      make_shared<ParticleGroup>(particle_selector::Type(1), sim.pd, "11");
  ...
  auto extForces = make_shared<ExternalForces<HarmonicWall>>(
      pg2, make_shared<HarmonicWall>(-Lz * 0.25));
  auto extForces2 = make_shared<ExternalForces<HarmonicWall>>(
      pg3, make_shared<HarmonicWall>(Lz * 0.25));
...
std::shared_ptr<Potential::LJ> createLJPotential(UAMMD sim) {
  ...
  // The needed ones are in InputPairParameters inside each potential, in this
  // case:
  ...
  par.cutOff = 2.5 * par.sigma;
  // the pair 1,0 is registered as well with this call, and assumed to be the
  // same
...
void addShortRangeInteraction(std::shared_ptr<Integrator> verlet, UAMMD sim) {
  // You can force the use of a certain neighbour list passing its name as a
  // second template argument The default list is a CellList
...
void printSimulation(UAMMD sim, std::ofstream &out) {
  ...
  // Accesing the arrays using this indices will ensure the traversing order is
  ...
  std::for_each(index2id, index2id + pos.size(), [&](int id) {
    real4 pc = pos[id];
...
int main(int argc, char *argv[]) {
  ...
  // You can issue a logging event like this, many log levels exists (see
  // System.cuh). A maximum log level is set in System.cuh or through the
  // MAXLOGLEVEL macro every logging event with a level superior to the max will
  // be ignored by the compiler so dont be afraid to write System::DEBUGX log
  // calls.
  ...
  sim.sys->log<System::MESSAGE>("mean FPS: %.2f",
