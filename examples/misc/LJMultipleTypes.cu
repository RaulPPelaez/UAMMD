...
This example showcases the performance when using a "large" number of particle
types. It does this by setting particles types randomly from 0 to an input
number and then configuring the potential to handle each particle pair. In this
case all particle pairs have the same parameters, but the potential is oblivious
to this.
...
int main(int argc, char *argv[]) {
  ...
  auto verlet = make_shared<NVT>(pd, par);
  {
    ...
    auto pot = make_shared<Potential::LJ>();
    {
      ...
      // All interaction parameters are the same, though the potential does not
      // know this.
      fori(0, ntypes) {
