...
Reads some parameters from a file called "data.main.benchmark", if not present
it will be auto generated with some default parameters.
...
int main(int argc, char *argv[]) {
  ...
  {
    // Modules working with pairs of particles usually ask for a Potential
    // object PairForces decides if it should use a neighbour list or treat the
    // system as NBody, You can force the use of a certain neighbour list
    // passing its name as a second template argument
...
void readParameters(std::shared_ptr<System> sys, std::string file) {
  ...
  in.getOption("periodicity", InputFile::Required) >> periodicityX >>
      periodicityY >> periodicityZ;
}
