/*Raul P. Pelaez 2017. An example of how to use HydroGrid in UAMMD.
  ...
  HG is called every printSteps to update according to hydroGridOptions.nml
(which must be present at the current folder) and write to disk.
  ...
  You can visualize the particle results with superpunto. HG will output
whatever was asked in the .nml.
...
int main(int argc, char *argv[]) {
  if (argc != 7) {
    cerr << "ERROR: Not the right arguments!, try to run me with: ./a.out 15 "
            "256 0.0001 1000000 1000 0.5"
         << endl;
  ...
  // mid-simulation If UAMMD is to be used as a plugin for other enviroment or
  // custom code, ParticleData should accept references to
  //  properties allocated and handled by the user, this is a non-implemented
  //  work in progress as of now though.
  ...
  // faster than a custom one. See ParticleGroup.cuh for a list
