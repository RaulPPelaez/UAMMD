/* Raul P. Pelaez 2021. Python wrapper example
   ...
   We will create a PairForces module, specialized for a LJ potential and expose
   to python a function that computes forces between particles.

   Given a list of positions the resulting python library will expose a function
   that computes LJ forces between them.

   UAMMD stores positions and forces as real4, but lets make the python library
   work for tightly packed positions/forces by allowing real3 vectors as input.

 */
// The pybind11 library takes care of interfacing python and c++, it has much
// more functionality than what is shown here. Look up its documentation if you
// need.
...
__device__ real3 lj_force(real3 rij, real epsilon, real sigma, real rc) {
  ...
  if (r2 > 0 and r2 < rc * rc) {
    ...
    const real fmoddivr =
        epsilon * (real(-48.0) * invr6 + real(24.0)) * invr6 * invr2 / sigma;
    return fmoddivr * rij;
  }
  return real3();
}
...
struct ToReal4 {
  template <class vectype> __host__ __device__ real4 operator()(vectype i) {
    auto pr4 = make_real4(i);
    return pr4;
  }
};

// A simple LJ Potential, see advanced/customPotential.cu or the wiki page for
// PairForces if you need more info about UAMMD Potentials.
struct LJPotential {
  real epsilon, sigma, rc;

  LJPotential(Parameters par)
      : epsilon(par.epsilon), sigma(par.sigma), rc(par.cutOff) {}
  ...
  struct LJTransverser {
    ...
    Box box;
    real rc;
    LJTransverser(Box i_box, real i_rc, real4 *i_force, real epsilon,
                  real sigma)
...
// This is the struct that will get exposed to Python, it holds the UAMMD state
// (a System and a ParticleData)
//  and a PairForces module specialized with the above LJ Potential
// We will also expose the sumForce function
// From the python side a user would create an instance of this class and then
// call sumForce as many times as needed.
struct UAMMD {
  ...
  void sumForce(py::array_t<real> h_pos, py::array_t<real> h_F) {
    ...
    { // Transform and upload the input vectors to UAMMD
      ...
      thrust::copy((real3 *)h_pos.data(),
                   (real3 *)h_pos.data() + numberParticles, tmp.begin());
      thrust::transform(thrust::cuda::par.on(st), tmp.begin(), tmp.end(),
                        pos.begin(), ToReal4());
      auto forces = pd->getForce(access::location::gpu, access::mode::write);
      thrust::copy((real3 *)h_F.data(), (real3 *)h_F.data() + numberParticles,
                   tmp.begin());
      thrust::transform(thrust::cuda::par.on(st), tmp.begin(), tmp.end(),
                        forces.begin(), ToReal4());
    }
    // Compute LJ forces
    pf->sum({.force = true, .energy = false, .virial = false}, st);
    // Copy particle forces to the input array, transformed again into a real3
    // layout
...
// This is the interfacing code. If you are familiar with pybind11 you will see
// it is a quite basic module.
...
PYBIND11_MODULE(uammd, m) {
  ...
  py::class_<UAMMD>(m, "LJ")
      . // Two functions will be available in python: The constructor and the
        // sumForce method (the destructor is called automatically when needed)
      def(py::init<Parameters, int>(), "Parameters"_a, "numberParticles"_a)
      . // This is the constructor
      def("sumForce", &UAMMD::sumForce,
          "Computes LJ forces between particles given a list of positions", // This is the sumForce function, the third argument is the doc
          "positions"_a,
          "forces"_a); // These allow to have python-style named arguments
  // Lets expose also a part of the Box class just for the sake of it.
  py::class_<Box>(
      m, "Box",
      "Domain bounds, the box is periodic by default. Note that you can make "
      "the domain aperiodic in any direction by using an infinite length.")
      .def(py::init<real>())
  ...
  py::class_<Parameters>(m, "Parameters")
      .def(py::init([](real sigma, real epsilon, real cutOff,
                       Box box) { // This is the constructor
           ...
           "sigma"_a = 1.0, "epsilon"_a = 1.0, "cutOff"_a = 1.0,
           "box"_a = Box())
      ...
      .def(
          "__str__",
          [](const Parameters &p) { // This allows to print the object in python
            return "sigma = " + std::to_string(p.sigma) + "\n" +
                   ...
                   std::to_string(p.box.boxSize.y) + "," +
                   std::to_string(p.box.boxSize.z) + ")\n";
          });
}
