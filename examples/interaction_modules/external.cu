/*Raul P. Pelaez 2021
  ...
  ExternalForces in UAMMD will compute forces and energies acting on particles
  ...
  some examples with specializations for external interactions.
...
// particles experience gravity and there is a wall at the bottom
...
// An External interaction can be ParameterUpdatable (see
// advanced/ParameterUpdatable.cu) This allows you to define a family of
// functions that can make this object aware of changes in the simulation state,
// such as the simulation time In this example we will use it to move the height
// of the wall according to time
struct MovingWall : public ParameterUpdatable {
  ...
  // This function will be called for each particle
  // The arguments will be modified according to what was returned by getArrays
  // below comp contains wether the caller needs force, energy, virial or a
  // combination of them
  __device__ ForceEnergyVirial sum(Interactor::Computables comp,
                                   real4 pos /*, real mass */) {
    real3 f = real3();
  ...
  // This function will be called whenever the simulation time is modified (i.e
  // each step)
...
// You can use this function to create an interactor that can be directly added
// to an integrator
std::shared_ptr<Interactor> createExternalPotentialInteractor(UAMMD sim) {
  // You can pass an instance of the specialization as a shared_ptr, which
  // allows you to modify it from outside the interactor module at any time.
