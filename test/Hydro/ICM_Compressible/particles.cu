/* Raul P. Pelaez 2022. Compressible ICM particle-related test code.
   ...
   If LJ interactions (sterics) are enabled particles start in an FCC lattice, otherwise particles are distributed randomly.
...
auto createLJInteractor(std::shared_ptr<ParticleData> pd, Parameters ipar) {
  ...
  auto pot = std::make_shared<Potential::LJ>();
  {
    ...
    par.shift = false;
    par.sigma = ipar.boxSize.x/ipar.cellDim.x; //Approx twice the hydrodynamicRadius
...
auto initializeParticles(Parameters par, std::shared_ptr<System> sys){
  ...
  if(par.enableSterics){
    ...
    thrust::copy(init.begin(), init.end(), thrust::device_ptr<real4>(pos.begin()));
  }
  else{
