/* Raul P. Pelaez 2022. Compressible ICM test code.
   Can compute dynamic structure factors for the fluid as well as the VACF
...
public:
  VelocityAutocorrelation() { histogram = std::vector<int>(nbin, 0); }

  template <class Container> void addSample(Container &vel) {
    nsamples++;
    size = vel.size();
...
template <class Container1, class Container2>
void writeComplexSignals(Container1 &Sqw, Parameters par, int ntimes,
                         Container2 &sampleWaveNumbers, std::string name) {
...
int main(int argc, char *argv[]) {
  ...
  if (par.maxFreq < 0)
    par.maxFreq = 4 * par.speedOfSound * 2 * M_PI * 2 / par.boxSize.x;
  if (par.simulationTime < 0)
    par.simulationTime = 2 * M_PI * par.numberFreq / par.maxFreq;
  {
  ...
  //  for(auto s: sampleWaveNumbers) s = count++;
  {
    ...
    sampleWaveNumbers[0] =
        kn.x + (kn.y + kn.z * par.cellDim.y) * (par.cellDim.x / 2 + 1);
  }
  FourierTransform3D fft(par.cellDim);
  // DynamicStructureFactor Sqw_vxvy(nsamples, ntimes), Sqw_vxvx(nsamples,
  // ntimes); DynamicStructureFactor Sqw_rhovx(nsamples, ntimes),
  // Sqw_rhorho(nsamples, ntimes);
