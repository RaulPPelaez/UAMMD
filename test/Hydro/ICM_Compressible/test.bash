...
tar xvf plots.tar.xz #Unpack the theory data
function average(){
    local runs=$1
    local name=$3
...
nruns=100 #Number of identical runs to average
naverage=5 #Number of times to run $nruns for statistics
...
function savePlot(){
    local name=$1
    gracebat  plots/theory/$name.dat $folder/$name.dat -par plots/$name.par -hdevice EPS -hardcopy -printfile $name.eps
}
savePlot Srhorho
savePlot Srhovx
savePlot Svxvx
