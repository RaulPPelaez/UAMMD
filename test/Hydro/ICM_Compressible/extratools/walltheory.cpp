/* Raul P. Pelaez 2022. Prints the velocity of a fluid between two walls when one of
   the walls is moving with a velocity vx(z=0)=v0*cos(wt).
...
auto vz_theory(real z, real vamplitude, real w, real t, real delta, real L) {
  ...
  const real vz = vamplitude*(std::real(vzh)*cos(w*t) + std::imag(vzh)*sin(w*t));
  return vz;
}
