...
struct Convolution {
  ...
  template <class complex> __device__ complex operator()(complex a, complex b) {
    return {(a.x * b.x + a.y * b.y) / nomalization,
            (a.y * b.x - a.x * b.y) / nomalization};
  }
};
...
public:
  ...
  template <class T> using cached_container = uninitialized_cached_vector<T>;
  StaticStructureFactor3D(int3 ncells) : ncells(ncells) {
    cufftPlan3d(&plan, ncells.x, ncells.y, ncells.z,
  ...
  template <class RealIterator> auto compute(RealIterator a, RealIterator b) {
    ...
    cufftExecReal2Complex<real>(plan, a,
                                thrust::raw_pointer_cast(a_hat.data()));
    cufftExecReal2Complex<real>(plan, b,
                                thrust::raw_pointer_cast(b_hat.data()));
    auto cit = thrust::make_counting_iterator(0);
    thrust::transform(a_hat.begin(), a_hat.end(), b_hat.begin(),
                      structureFactor.begin(), Convolution(n));
...
struct ColMajorToRowMajor {
  ...
  __device__ int operator()(int i) { return ntimes * i + time; }
};
...
public:
  DynamicStructureFactor(int nsamples, int ntimes)
      : nsamples(nsamples), ntimes(ntimes) {
    ...
    int inembed[] = {0};
    int onembed[] = {0};
    cufftPlanMany(&plan, 1, n, inembed, istride, idist, onembed, ostride, odist,
                  CUFFT_Complex2Complex<real>::value, nsamples);
  }
  ...
  void addSamplesFourier(ComplexIterator samples_u, ComplexIterator samples_v,
                         int time) {
    auto cit = thrust::make_counting_iterator(0);
    auto indexit =
        thrust::make_transform_iterator(cit, ColMajorToRowMajor(ntimes, time));
  ...
  auto compute() {
    ...
    cufftExecComplex2Complex<real>(plan, thrust::raw_pointer_cast(uqt.data()),
                                   thrust::raw_pointer_cast(uqw.data()),
                                   CUFFT_FORWARD);
    cufftExecComplex2Complex<real>(plan, thrust::raw_pointer_cast(vqt.data()),
                                   thrust::raw_pointer_cast(vqw.data()),
                                   CUFFT_FORWARD);
    thrust::transform(uqw.begin(), uqw.end(), vqw.begin(), uqw.begin(),
                      Convolution(ntimes));
    return uqw;
  }
};
...
public:
  ...
  template <class RealIterator> auto transform(RealIterator a) {
    int nhat = (ncells.x / 2 + 1) * ncells.y * ncells.z;
    cached_container<complex> a_hat(nhat);
    cufftExecReal2Complex<real>(plan, a,
                                thrust::raw_pointer_cast(a_hat.data()));
    ...
    }
    {
      ...
      auto it =
          thrust::make_constant_iterator<complex>({real(ntot), real(ntot)});
      thrust::transform(a_hat.begin(), a_hat.end(), it, a_hat.begin(),
                        thrust::divides<complex>());
    }
    return a_hat;
  }
};
...
public:
  FourierTransformComplex1D(int nsamples, int ntimes)
      : nsamples(nsamples), ntimes(ntimes) {
    uqt.resize(nsamples * ntimes);
    int size = ntimes;
    int n[] = {size};
  ...
  auto compute() {
    ...
    cufftExecComplex2Complex<real>(plan, thrust::raw_pointer_cast(uqt.data()),
                                   ...
                                   CUFFT_FORWARD);
    auto it =
        thrust::make_constant_iterator<complex>({real(ntimes), real(ntimes)});
    thrust::transform(uqw.begin(), uqw.end(), it, uqw.begin(),
                      thrust::divides<complex>());
    return uqw;
  }
};
