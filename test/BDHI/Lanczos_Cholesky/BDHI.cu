...
int main(int argc, char *argv[]) {
  ...
  Box box(boxSize);
  {
    ...
    fori(0, N) {
      pos.raw()[i] =
          make_real4(sys->rng().uniform3(-boxSize.x * 0.5, boxSize.x * 0.5), 0);
  ...
  if (mode == "Lanczos")
    ...
    sys->log<System::CRITICAL>(
        "[System] Unrecognized mode! choose Lanczos or Cholesky");
  ...
  forj(0, nsteps) {
    if (printSteps > 0 and j % printSteps == 0) {
      ...
      fori(0, N) {
        ...
        out << std::setprecision(17) << p << " " << radius.raw()[sortedIndex[i]]
            << " " << type << "\n";
      }
    }
    bdhi->forwardTime();
