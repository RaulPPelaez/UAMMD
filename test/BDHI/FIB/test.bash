#This script runs several tests that ensure the correct results are provided when using the FIB BDHI module. Several tests exists tackilng the different parts of the algorithm separatedly and jointly.
...
then
    echo "FIB TEST FAILED: I need at least gnuplot 5.0!"
    exit 1
fi
...
function selfDiffusionCubicBox {
	...
	rh=$(echo $i | awk -F rh '{print $2}' | awk -F .dt '{print $1}')
	D0=$(echo 1 | awk '{l=1/'$L';print '$temperature'/(6*3.14159265358979*'$viscosity'*'$rh')*(1-2.837297*l+(4/3.0)*3.14159265358979*l**3-27.4*l**6);}')
