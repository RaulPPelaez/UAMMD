...
// References:
//  [1] Hydrodynamic fluctuations in quasi-two dimensional diffusion. RP Pelaez
//  et. al.
#include <cmath>
#include <iomanip>
#include <iostream>
...
double rh, vis;
double f(double r) {
  return (1.0 / (8.0 * M_PI * vis * r)) *
         ((1 + 2 * rh * rh / (M_PI * r * r)) * erf(r * sqrt(M_PI) / (2 * rh)) -
          2 * rh / (M_PI * r) * exp(-M_PI * r * r / (4 * rh * rh)));
}
double g(double r) {
  return (1.0 / (8.0 * M_PI * vis * r)) *
         ((1 - 6 * rh * rh / (M_PI * r * r)) * erf(r * sqrt(M_PI) / (2 * rh)) +
          6 * rh / (M_PI * r) * exp(-M_PI * r * r / (4 * rh * rh)));
}
