...
int main(int argc, char *argv[]) {
  ...
  if (!N || !Nframes) {
    cerr << "ERROR!! SOME INPUT IS MISSING!!!" << endl;
    print_help();
    exit(1);
  }
  ...
  if (isatty(STDIN_FILENO)) { // If there is no pipe
    ...
    fori(1, argc) { // exclude the exe file
      in = new ifstream(
          argv[i]); // There must be a filename somewhere in the cli
      if (in->good()) {
        good_file = true;
        break;
      }
    ...
    in = &cin; // If there is something being piped
  parse_as_plain(*in);
...
void parse_as_plain(istream &in) {
  ...
  cerr << "Computing..." << endl;
#pragma omp parallel for default(shared) private(r, r0, rij, msd_tid,          \
                                                     nmsd_tid) ordered
...
#pragma omp ordered
    {
      cout << dt << " ";
      fork(0, 3) cout << msd[3 * dt + k] / (float)N << " ";
      cout << nmsd[dt] << "\n";
    }
  }
