...
bool selfMobilityCubicBox_test() {
  ...
  fori(0, NL) {
    ...
    for (int j = 0; j < 9; j++)
      Mout << std::setprecision(15) << abs((1 * (j % 3 == j / 3) - M[j] / M0))
           << " ";
    Mout << endl;
  }
  return true;
...
void computePairMobilityMatrix(real3 L, double F, double3 dist, long double *M,
                               double &true_rh) {
  int N = 2;
  auto sys = make_shared<System>();
  sys->rng().setSeed(0xabefa129f9173 ^ time(NULL));
  ...
  for (int i = 0; i < Ntest; i++) {
    for (int alpha = 0; alpha < 3; alpha++) {
      double3 posprev;
      {
        auto pos = pd->getPos(access::location::cpu, access::mode::write);
        double3 ori =
            sys->rng().uniform3(-0.5, 0.5) * make_double3(box.boxSize);
...
bool pairMobilityCubicBox_test(double dist) {
  ...
  std::ofstream out("pairMobilityCubicBox.dist" + std::to_string(dist / rh) +
                    ".test");
  double F = 1;
  long double M[9];
...
bool idealParticlesDiffusion(int N, real3 L, double &true_rh,
  ...
  auto bdhi = make_shared<BDHI::FIB>(pd, par);
  true_rh = bdhi->getHydrodynamicRadius();
  std::ofstream out("pos.noise.boxSize" + std::to_string(L.z / true_rh) +
                    ".rh" + std::to_string(true_rh) + ".dt" +
                    std::to_string(par.dt) + "." + suffix);
  {
    auto pos = pd->getPos(access::location::cpu, access::mode::write);
...
void selfDiffusion_q2D_test() {
  ...
  forj(0, NL) {
    ...
    double true_rh;
    idealParticlesDiffusion(N, make_real3(Lx, Lx, Lz), true_rh,
                            "q2D.Lx" + std::to_string(Lx / rh) + ".test");
    double L = Lx;
    double lz = Lz;
    long double M0 = 1.0L / (6.0L * M_PIl * viscosity * true_rh);
    // From eq 21 and 23 in Vögele, M., & Hummer, G. (2016). Divergent Diffusion
    ...
    out << std::setprecision(15) << Lz / rh << " " << Mplane_near / M0 << " "
        << Mplane_far / M0 << " ";
...
void noiseVariance_test() {
  ...
  forj(0, NL) {
    ...
    out << std::setprecision(15) << L / true_rh << " " << noiseCorr.x << " "
        << noiseCorr.y << " " << noiseCorr.z << endl;
    CudaCheckError();
  }
}
...
int main(int argc, char *argv[]) {
  ...
  if (strcmp(argv[1], "noiseVariance") == 0)
    noiseVariance_test();
  if (strcmp(argv[1], "radialDistributionFunction") == 0)
