...
long double f(long double r, double rh, double viscosity) {
  return (1.0 / (8.0 * M_PIl * viscosity * r)) *
         ((1 + 2 * rh * rh / (M_PIl * r * r)) *
              erfl(r * sqrt(M_PIl) / (2 * rh)) -
          2 * rh / (M_PIl * r) * expl(-M_PIl * r * r / (4 * rh * rh)));
}
long double g(long double r, double rh, double viscosity) {
  return (1.0 / (8.0 * M_PIl * viscosity * r)) *
         ((1 - 6 * rh * rh / (M_PIl * r * r)) *
              erfl(r * sqrt(M_PIl) / (2 * rh)) +
          6 * rh / (M_PIl * r) * expl(-M_PIl * r * r / (4 * rh * rh)));
}
// Self mobility with PBC corrections up to sixth order
long double computePBCCorrection(double L, double rh) {
  return (1.0l - 2.837297l * rh / L + (4.0l / 3.0l) * M_PIl * pow(rh / L, 3) -
          27.4l * pow(rh / L, 6.0l));
}
  ...
  for (int i = 0; i < Ntest; i++) {
    for (int alpha = 0; alpha < 3; alpha++) {
      double3 posprev;
      {
        ...
        pos.raw()[0] = make_real4(
            make_real3(sys->rng().uniform3(-0.5, 0.5)) * box.boxSize, 0);
...
void computeSelfMobilityMatrixRH(
    real3 L, double F, long double *M, long double &M0, long double &real_rh,
    real3 r0, shared_ptr<BDHI::EulerMaruyama<BDHI::FCM>> &bdhi,
    int3 cells = {-1, -1, -1}) {
  int N = 1;
...
bool hydrodynamicRadiusVariance_test() {
  ...
  fori(0, Nt) {
    ...
    r0.x = dis(gen);
    r0.y = dis(gen);
    computeSelfMobilityMatrixRH(make_real3(L), F, M, M0, real_rh, r0, bdhi,
                                make_int3(L));
    CudaCheckError();
    Mout << std::setprecision(15) << r0.x + 0.5 * L << " " << r0.y + 0.5 * L
         << " ";
    for (int j = 0; j < 3; j++)
      Mout << std::setprecision(15) << (M[j] / M0) << " ";
    Mout << endl;
  }
  return true;
}
...
bool selfMobilityCubicBox_test() {
  ...
  fori(0, NL) {
    ...
    // Substract 1 to the diagonal terms, which should be one so a matrix of
    // zeroes should be printed abs to be able to plot log
    for (int j = 0; j < 9; j++)
      Mout << std::setprecision(15)
           << abs((1.0L * (j % 3 == j / 3) - M[j] / M0)) << " ";
    Mout << endl;
  }
  return true;
}
...
bool pairMobilityCubicBox_test(double dist) {
  ...
  while (dir.x == 0 or dir.y == 0 or dir.z == 0)
    dir = make_real3(rng.gaussian3(0, 1));
  real3 rij = dist * dir / sqrt(dot(dir, dir));
  std::ofstream out("pairMobilityCubicBox.dist" + std::to_string(dist) +
                    ".test");
  double F = 1;
  long double M[9];
  ...
  fori(0, NL) {
    ...
    for (int j = 0; j < 9; j++)
      out << abs(1.0l - M[j] / (M_theo_Linf[j])) << " ";
    out << endl;
    CudaCheckError();
  }
  return true;
...
bool pairMobility_q2D_test(double dist) {
  ...
  while (dir.x == 0 or dir.y == 0 or dir.z == 0)
    dir = make_real3(rng.gaussian3(0, 1));
  real3 rij = dist * dir / sqrt(dot(dir, dir));
  std::ofstream out("pairMobility_q2D.dist" + std::to_string(dist / rh) +
                    ".test");
  double F = 1;
  long double M[9];
  out << "#rij " << rij.x << " " << rij.y << " " << rij.z << endl;
...
bool selfMobility_q2D_test() {
  ...
  fori(0, NL) {
    ...
    CudaCheckError();
    // From eq 21 and 23 in Vögele, M., & Hummer, G. (2016). Divergent Diffusion
    // Coefficients in Simulations of Fluids and Lipid Membranes. The Journal of
    // Physical Chemistry B, 120(33), 8722–8732. doi:10.1021/acs.jpcb.6b05102
    ...
    for (int j = 0; j < 9; j++)
      Mout << std::setprecision(15) << M[j] / M0 << " ";
    Mtheoout << std::setprecision(15) << Mplane_near / M0 << " "
             << Mplane_far / M0 << " " << Mperp_near / M0 << " "
             << Mperp_far / M0 << endl;
    Mout << endl;
  }
  return true;
...
bool idealParticlesDiffusion(int N, real3 L, long double &M0,
                             long double &real_rh,
  ...
  out << "# " << L.x / real_rh << " " << L.z / real_rh << " "
      << temperature * M0 << " " << par.dt << " " << real_rh << endl;
  {
    auto pos = pd->getPos(access::location::cpu, access::mode::write);
    fori(0, pd->getNumParticles()) {
      pos.raw()[i] =
          make_real4(make_real3(sys->rng().uniform3(-0.5, 0.5)) * L, 0);
    }
  }
  fori(0, 3000) {
    bdhi->forwardTime();
    ...
    if (i > 0)
      out << "#" << endl;
    forj(0, pd->getNumParticles()) { out << make_real3(p[j]) << "\n"; }
  }
  sys->finish();
  return true;
}
...
void selfDiffusion_q2D_test() {
  ...
  forj(0, NL) {
    ...
    idealParticlesDiffusion(N, make_real3(Lx, Lx, Lz), M0, real_rh,
                            "q2D.Lx" + std::to_string(Lx) + ".test");
    ...
    out << std::setprecision(15) << lz / real_rh << " " << Mplane_near / M0
        << " " << Mplane_far / M0 << " ";
    out << std::setprecision(15) << Mperp_near / M0 << " " << Mperp_far / M0
        << endl;
    CudaCheckError();
  }
}
...
// Returns Var(noise)
double3 singleParticleNoise(real T, real3 L, long double &M0,
                            long double &real_rh) {
  int N = 1;
  auto sys = make_shared<System>();
  ...
  double3 prevp;
  {
    auto pos = pd->getPos(access::location::cpu, access::mode::write);
    pos.raw()[0] =
        make_real4(make_real3(sys->rng().uniform3(-0.5, 0.5)) * box.boxSize, 0);
...
void noiseVariance_test() {
  ...
  forj(0, NL) {
    ...
    out << std::setprecision(15) << L / real_rh << " "
        << noiseCorr.x / (2 * selfDiffusion) << " "
        << noiseCorr.y / (2 * selfDiffusion) << " "
        << noiseCorr.z / (2 * selfDiffusion) << endl;
    CudaCheckError();
  }
}
...
int main(int argc, char *argv[]) {
  ...
  if (strcmp(argv[1], "selfMobilityCubicBox") == 0)
    selfMobilityCubicBox_test();
  if (strcmp(argv[1], "hydrodynamicRadiusVariance") == 0)
    hydrodynamicRadiusVariance_test();
  ...
  if (strcmp(argv[1], "selfMobility_q2D") == 0)
    selfMobility_q2D_test();
  if (strcmp(argv[1], "selfDiffusionCubicBox") == 0)
    selfDiffusionCubicBox_test();
  if (strcmp(argv[1], "selfDiffusion_q2D") == 0)
    selfDiffusion_q2D_test();
