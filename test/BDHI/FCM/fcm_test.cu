...
auto initializeKernelTorque(real hydrodynamicRadius, real tolerance, real3 L,
                            int3 n) {
...
TEST(FCM_impl, CanBeCreated) {
  ...
  par.box = Box(L);
  par.cells = {128, 128,
               128}; // createGrid(L, hydrodynamicRadius, par. tolerance);
  par.kernel = initializeKernel(par.tolerance, L, par.cells);
  par.kernelTorque =
      initializeKernelTorque(hydrodynamicRadius, par.tolerance, L, par.cells);
  ...
  for (int j = 0; j < ntest; j++) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      auto disp = fcm->computeHydrodynamicDisplacements(
          pos.data().get(), force.data().get(), torque, numberParticles,
