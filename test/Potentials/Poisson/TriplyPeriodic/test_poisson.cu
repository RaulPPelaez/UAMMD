...
real calculateTheoreticalField(real r, real gw) {
    const auto pi = M_PIl;
    return -exp(-r*r/(4.0*gw*gw))/(4*pi*sqrt(pi)*gw*r) - erf(r/(2.0*gw))/(4*pi*r*r);
}
...
public:
    ...
    static real fitAndGetConstantTerm(const std::vector<real>& x, const std::vector<real>& y) {
        ...
        // Return the constant term (a), which represents the field as x approaches infinity
        return coefficients[0];
    }

private:
    // Simple Gaussian elimination to solve the linear system
    static std::vector<real> solveLinearSystem(std::vector<std::vector<real>> A, std::vector<real> b) {
        int n = b.size();
...
protected:
    ...
    real4 runPoissonSimulation(real L, real r, real tolerance, real gw, real split) {
        int N = 2;
...
TEST_F(PoissonTest, SingleSimulationTest) {
    ...
    real relativeDifference = std::abs(1.0 - std::abs(force.x / theoreticalField));
