...
template<class NVT> void addInteractors(UAMMD sim, std::shared_ptr<NVT> mc);

int main(int argc, char *argv[]){
  auto sim = initialize(argc, argv);
  Timer tim;
  if(sim.parameters.scheme.compare("forceBiased") == 0){
    auto mc = createIntegratorForceBiased(sim);
    addInteractors(sim, mc);
    tim.tic();  
    runSimulation(sim, mc);
  }
  else{
    auto mc = createIntegrator(sim);
    addInteractors(sim, mc);
    tim.tic();  
    runSimulation(sim, mc);
  }
...
void initializeFromFile(UAMMD sim){
  ...
  fori(0, sim.parameters.numberParticles){
    real3 p;
    in>>p.x>>p.y>>p.z>>charge[i];
    Box box(sim.parameters.boxSize);
    p = box.apply_pbc(p);
    pos[i] = make_real4(p, 0);
  }
}
...
std::shared_ptr<Integrator> createIntegrator(UAMMD sim){
  ...
  else if(sim.parameters.scheme.compare("forceBiased") == 0){
...
template<class NVT>
void addInteractors(UAMMD sim, std::shared_ptr<NVT> mc){
...
template<class NVT> void writeCurrentMCJumpSize(UAMMD sim, std::shared_ptr<NVT> mc){

}

template<> void writeCurrentMCJumpSize<MC::ForceBiased>(UAMMD sim, std::shared_ptr<MC::ForceBiased> mc){
  sim.sys->log<System::WARNING>("Current Jump Size: %g", mc->getCurrentStepSize());
}
...
Parameters readParameters(std::string datamain, shared_ptr<System> sys){
  ...
  if(par.scheme.compare("forceBiased") == 0){
