...
// Encodes the matrix dot product of the diagonal matrix M_{ij} =
// (i+1)\delta_{ij}
struct IdentityMatrixDot : public lanczos::MatrixDot {
  ...
  void operator()(real *v, real *mv) override {
    ...
    thrust::transform(thrust::cuda::par, cit, cit + m_size, v, mv,
  ...
  for (int size = 1; size < 128; size++) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      ASSERT_THAT(h_mv_i, ::testing::DoubleNear(theory, tol))
          << "Failed at index " << i << " for size " << size << " after "
          << lanczos->getLastRunRequiredSteps() << " steps" << std::endl;
    }
  }
}
...
struct DiagonalMatrixDot : public lanczos::MatrixDot {
  ...
  void operator()(real *v, real *mv) override {
    thrust::transform(thrust::cuda::par, m, m + m_size, v, mv,
                      thrust::multiplies<real>());
  }
};
...
TEST(Lanczos, WorksForDiagonalMatrixAndVectorOfOnes) {
  ...
  for (int size = 1; size < maxSize; size++) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (theory == 0)
        err = abs(h_mv_i);
      ASSERT_LE(err, tol) << "Failed at index " << i << " for size " << size
                          << " after " << lanczos->getLastRunRequiredSteps()
                          << " steps, found " << h_mv_i << " while expecting "
                          << theory << std::endl;
    }
  }
}
...
struct DenseMatrixDot : public lanczos::MatrixDot {
  ...
  DenseMatrixDot(real *m) : m(m) { cublasCreate_v2(&handle); }

  ~DenseMatrixDot() { cublasDestroy_v2(handle); }

  void operator()(real *v, real *mv) override {
    real alpha = 1;
    real beta = 0;
    cublasgemv(handle, CUBLAS_OP_N, m_size, m_size, &alpha, m, m_size, v, 1,
               &beta, mv, 1);
  }
};
...
auto generateRandomSymmetricPositiveMatrix(int n) {
  ...
  for (int i = 0; i < n; i++)
    for (int j = 0; j < n; j++) {
      vecmp[i + n * j] =
          0.5 * (vecm[i + n * j] + vecm[j + n * i]) + 5 * n * (i == j);
    }
  return vecmp;
}
...
auto computeSquaredMatrix(thrust::device_vector<real> &M, int n) {
  ...
  real alpha = 1;
  real beta = 0;
  cublasgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, n, n, n, &alpha, M.data().get(),
             n, M.data().get(), n, &beta, M2.data().get(), n);
  cublasDestroy_v2(handle);
  return M2;
}

auto computeMatrixVectorProduct(thrust::device_vector<real> &M,
                                thrust::device_vector<real> &v, int n) {
  cublasHandle_t handle;
  cublasCreate_v2(&handle);
  auto mv = v;
  real alpha = 1;
  real beta = 0;
  cublasgemv(handle, CUBLAS_OP_N, n, n, &alpha, M.data().get(), n,
             v.data().get(), 1, &beta, mv.data().get(), 1);
  cublasDestroy_v2(handle);
  return mv;
}
