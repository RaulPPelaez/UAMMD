...
void initializePositions(UAMMD sim) {
  ...
  std::generate(pos.begin(), pos.end(), [&]() {
    return make_real4(
        make_real3(sim.pd->getSystem()->rng().uniform3(-0.5, 0.5)) *
            sim.parameters.boxSize,
        0);
  });
}
void initializeCharges(UAMMD sim) {
  ...
  std::generate(charges.begin(), charges.end(),
                [i, N]() mutable { return pow(-1, ++i); });
...
std::shared_ptr<Integrator> createIntegrator(UAMMD sim) {
  if (sim.parameters.scheme.compare("EulerMaruyama") == 0) {
    ...
    System::log<System::CRITICAL>("Unrecognized scheme, use EulerMaruyama, "
                                  "MidPoint, AdamsBashforth or Leimkuhler");
    exit(1);
    return nullptr;
  }
}
...
std::shared_ptr<Interactor> createShortRangeInteractor(UAMMD sim) {
  if (sim.parameters.potential.compare("LJ") == 0) {
    ...
    System::log<System::CRITICAL>(
        "Unrecognized potential. Use LJ, soft or repulsive");
    return nullptr;
  }
}
