...
struct RepulsivePotentialFunctor {
  ...
  __device__ __host__ real force(real r2, PairParameters params) {
    ...
    real fmod =
        real(-4.0) * U0 * p * sdivrp * (real(2.0) * sdivrp - real(1.0)) / rr;
    return fmod / r;
  }

  __device__ real energy(real r2, PairParameters params) { return 0; }
