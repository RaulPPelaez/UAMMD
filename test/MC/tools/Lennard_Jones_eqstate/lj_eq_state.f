      ...
      double precision dens,temp,u,p,cv, epsilon
      double precision coef_adia,cs,eta,thc,bulkv,sa
      logical lrc               !long range corrections

c     WARNING: Do not change this
c     lrc = true means an infinite range LJ potential
c     lrc = false means the potential is truncated and shifted at a certain cut off rc, which means the force is F_LJ(r) = -du_lj(r)/dr + du(r)/dr|_rc
c     Things change a lot with the cut off distance, so leave lrc=false and put the 2.5      
      lrc=.false.



      read(*,*) dens,temp, rcutfluid, sigma, epsilon
...
c     &       coef_adia,cs,eta,thc,bulkv,sa)
      ...
      write(*,*) dens,temp,u/epsilon,p/epsilon
      end
...
* la fuente es kj. karl johnson, john a. zollweg and keith e gubbins, mol. phys v78, no3, 591-618
      ...
      double precision x28,x29,x30,x31,x32
      integer i,j,k
      logical lrc
