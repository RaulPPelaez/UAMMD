/*Raul P. Pelaez 2018-2022. MonteCarlo NVT test.
  ...
  It allows for testing of the LJ potential and the integrator by using it to
  reproduce the eq of state.
...
int main(int argc, char *argv[]) {
  ...
  Box box(boxSize);
  sys->log<System::MESSAGE>("[System] Box size: %e %e %e", boxSize.x, boxSize.y,
                            boxSize.z);
  {
    ...
    if (printSteps > 0 && j % printSteps == 0) {
      sys->log<System::DEBUG1>("[System] Writing to disk...");
      {
        ...
        fori(0, numberParticles) {
          ...
          p = make_real3(pc);
          int type = pc.w;
          out << std::setprecision(2 * sizeof(uammd::real)) << p << " "
              << 0.5 * (type == 1 ? 2 : 1) << " " << type << endl;
        }
      }
      {
        double U = mc->sumEnergy();
        auto energy = pd->getEnergy(access::location::gpu, access::mode::read);
        U += thrust::reduce(thrust::cuda::par, energy.begin(), energy.end(),
                            uammd::real(0.0));
...
void readParameters(std::string datamain) {
  ...
  in.getOption("desiredAcceptanceRatio", InputFile::Required) >>
      desiredAcceptanceRatio;
}
