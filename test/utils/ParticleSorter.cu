...
TEST(Sorter, CanBeCreated){
  auto sorter = std::make_shared<ParticleSorter>();
}

struct TrivialHash{
  inline __host__ __device__ uint operator()(uint i) const{
    return i;
  }
};
...
TEST(Sorter, SortsCorrectly){
  ...
  thrust::sequence(uammd::cached_device_execution_policy.on(st), vec.rbegin(), vec.rend(), 0);
