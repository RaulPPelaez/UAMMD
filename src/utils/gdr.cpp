...
void read_frame(istream &in, double *r);
void rdf(const double *pos, bool write);
...
#include<limits>
void rdf(const double *r, bool write=false){
  static vector<unsigned long long int> gdr(RDF_SAMPLES, 0);
  static int avg_counter = 0;

  double rijmod;                                // distance between two particles
  double ratio = rcut/(double)(RDF_SAMPLES); //dr, distance interval of one bin
  int bin=0; 
  ...
  if(write){
    ...
    double normalization = (double)avg_counter*2.0f*3.14159265359f*ratio*N*N/V;
    double R;
    for(int i=1; i<RDF_SAMPLES; i++){
      R = (i-0.5)*ratio;
      double b = gdr[i-1];
      double err = sqrt(b*(1.0-b/N/N/avg_counter/avg_counter));
      err /= normalization*R*R;
      cout<<R<<" "<<b/(normalization*R*R)<<" "<< err <<" "<< gdr[i-1] <<"\n";
    }
    cout<<flush;
  }
...
void parse_as_plain(istream &in, int Nsteps){
  vector<double> pos(3*N, 0);// = new double[3*N];
  read_frame(in, &pos[0]);
...
inline void apply_pbc(double *r){
