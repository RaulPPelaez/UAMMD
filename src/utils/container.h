...
public:
  ...
  thrust::reverse_iterator<iterator> rbegin() const {
    return thrust::make_reverse_iterator(end());
  }

  thrust::reverse_iterator<iterator> rend() const {
    return thrust::make_reverse_iterator(begin());
  }
...
template <class T>
using uninitialized_cached_vector = detail::UninitializedCachedContainer<T>;
} // namespace uammd
#endif
