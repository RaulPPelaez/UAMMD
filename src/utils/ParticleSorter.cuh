/* Raul P. Pelaez 2019-2022. ParticleSorter.
   ...
   A helper class to sort particles according to their positions following a
certain rule. This rule can be a morton hash, so the particle positions are
sorted to follow a Z-order curve, a cell hash, particle ID...
   ...
   ps.updateOrderByHash<Sorter::MortonHash>(pos, numberParticles, BoxSize,
cudaStream);
   //Apply new order to some array
   ps.applyCurrentOrder(array_in, array_out, numberParticles, cudaStream);

   //Order the id array (without changing it) as keys and a 0..numberparticles
array as values,
   ...
   I tried to use thrust::sort_by_key, but it is not smart enough to leverage
the end bit of the maximum hash and is approx 30% slower.
...
struct MortonHash {
  ...
  // Interleave a 10 bit number in 32 bits, fill one bit and leave the other 2
  // as zeros. See [1]
...
// The hash is the cell 1D index, this pattern is better than random for
// neighbour transverse, but worse than Morton
...
__global__ void assignHash(HashIterator hasher, int *__restrict__ index,
...
template <typename T> inline int msb(T n) {
  ...
  for (T i = std::numeric_limits<T>::digits - 1, mask = 1 << i; i >= 0;
       --i, mask >>= 1) {
    if ((n & mask) != 0)
      return i;
  }
...
public:
  ...
  template <class HashIterator>
  void
  updateOrderWithCustomHash(HashIterator &hasher, uint N,
                            uint maxHash = std::numeric_limits<uint>::max(),
                            cudaStream_t st = 0) {
    System::log<System::DEBUG1>(
        "[ParticleSorter] Updating with custom hash iterator: %s",
        type_name<HashIterator>().c_str());
    System::log<System::DEBUG1>(
        "[ParticleSorter] Assigning hash to %d elements", N);
    System::log<System::DEBUG2>(
        "[ParticleSorter] Maximum hash 0x%x ( dec: %u ) last bit: %d", maxHash,
        maxHash, 32 - Sorter::clz(maxHash));
    try {
      tryToUpdateOrderWithCustomHash(hasher, N, maxHash, st);
    } catch (...) {
      System::log<System::ERROR>(
          "ParticleSorter raised an exception in updateOrderWithCustomHash");
      throw;
    }
  }

  // Sets the current sorting using the linear index of the cell of the given
  // positions
  template <class CellHasher = Sorter::MortonHash, class InputIterator>
  void updateOrderByCellHash(InputIterator pos, uint N, Box box, int3 cellDim,
                             cudaStream_t st = 0) {
  ...
  void updateOrderById(int *id, int N, cudaStream_t st = 0) {
    try {
      ...
      System::log<System::ERROR>(
          "Exception raised in ParticleSorter::updateOrderById");
      throw;
    }
  }
  ...
  void applyCurrentOrder(InputIterator d_property_unsorted,
                         OutputIterator d_property_sorted, int N,
                         cudaStream_t st = 0) {
    auto pi =
        thrust::make_permutation_iterator(d_property_unsorted, index.begin());
    thrust::copy_n(uammd::cached_device_execution_policy.on(st), pi, N,
                   d_property_sorted);
    CudaCheckError();
  }
  ...
  int *getSortedIndexArray(int N) {
    try {
      ...
      System::log<System::ERROR>(
          "Exception raised in ParticleSorter::getSortedIndexArray");
      throw;
    }
  }

  uint *getSortedHashes() { return thrust::raw_pointer_cast(hash.data()); }
  ...
  int *getIndexArrayById(int *id, int N, cudaStream_t st = 0) {
    try {
      ...
      System::log<System::ERROR>(
          "Exception raised in ParticleSorter::getIndexArrayById");
      throw;
    }
  }
...
private:
  ...
  void tryToUpdateOrderById(int *id, int N, cudaStream_t st) {
    ...
    thrust::copy_n(uammd::cached_device_execution_policy.on(st), id, N,
                   hash.begin());
    auto db_index =
        cub::DoubleBuffer<int>(thrust::raw_pointer_cast(original_index.data()),
                               thrust::raw_pointer_cast(index_alt.data()));
    auto db_hash = cub::DoubleBuffer<int>(
        d_hash, (int *)thrust::raw_pointer_cast(hash_alt.data()));
  ...
  template <class HashIterator>
  void tryToUpdateOrderWithCustomHash(HashIterator &hasher, uint N,
                                      uint maxHash, cudaStream_t st) {
    init = true;
    hash.resize(N);
    index.resize(N);
    int Nthreads = 128;
    int Nblocks = N / Nthreads + ((N % Nthreads) ? 1 : 0);
    Sorter::assignHash<<<Nblocks, Nthreads, 0, st>>>(
        hasher, thrust::raw_pointer_cast(index.data()),
        thrust::raw_pointer_cast(hash.data()), N);
    CudaCheckError();
    hash_alt.resize(N);
    index_alt.resize(N);
    auto db_index =
        cub::DoubleBuffer<int>(thrust::raw_pointer_cast(index.data()),
                               thrust::raw_pointer_cast(index_alt.data()));
    auto db_hash =
        cub::DoubleBuffer<uint>(thrust::raw_pointer_cast(hash.data()),
  ...
  void sortByKey(cub::DoubleBuffer<int> &index,
                 cub::DoubleBuffer<hashType> &hash, int N, cudaStream_t st = 0,
                 int end_bit = sizeof(uint) * 8) {
    if (N > maxRequestedElements) {
      maxRequestedElements = N;
      cub_temp_storage_bytes = 0;
      CudaSafeCall(cub::DeviceRadixSort::SortPairs(
          nullptr, cub_temp_storage_bytes, hash, index, N, 0, end_bit, st));
    }
    auto alloc = System::getTemporaryDeviceAllocator<char>();
    std::shared_ptr<char> d_temp_storage(
        alloc.allocate(cub_temp_storage_bytes),
        [=](char *ptr) { alloc.deallocate(ptr); });
    void *d_temp_storage_ptr = d_temp_storage.get();
    CudaSafeCall(cub::DeviceRadixSort::SortPairs(d_temp_storage_ptr,
                                                 cub_temp_storage_bytes, hash,
                                                 index, N, 0, end_bit, st));
  }
};
} // namespace uammd
#endif
