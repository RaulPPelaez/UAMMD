...
public:
  ...
  Xorshift128plus(uint64_t s0, uint64_t s1) { setSeed(s0, s1); }
  explicit Xorshift128plus(uint64_t s0) { setSeed(s0); }
  ...
  double3 gaussian3(double mean, double std) {
    return make_double3(gaussian(mean, std), gaussian(mean, std),
                        gaussian(mean, std));
  }

  double2 gaussian2(double mean, double std) {
    return make_double2(gaussian(mean, std), gaussian(mean, std));
  }
  ...
  void setSeed(uint64_t s0) {
    s[0] = s0;
    s[1] = (s0 + 15438657923749336752ULL) % RANDOM_MAX;
  }
};
