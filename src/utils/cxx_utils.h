/*Raul P. Pelaez 2017. C++ useful constructions.
  ...
  Code to ease working with variadic templates, C++14+ features not present in
  C++11...
...
#define SFINAE_DEFINE_HAS_MEMBER(X)                                            \
  ...
  public:                                                                      \
    static constexpr bool value =                                              \
        std::is_same<decltype(test<T>(0)), one>::value;                        \
  };
...
template <class... B> inline constexpr bool disjunction_v() {
  return disjunction<B...>::value;
}
template <typename T, typename... Ts> constexpr bool is_one_of() {
  return disjunction_v<std::is_same<T, Ts>...>();
}
