...
struct alignas(real3) tensor3 {
  ...
  VECATTR tensor3(real xx, real xy, real xz, real yx, real yy, real yz, real zx,
                  real zy, real zz)
      : xx(xx), xy(xy), xz(xz), yx(yx), yy(yy), yz(yz), zx(zx), zy(zy), zz(zz) {
  }

  VECATTR tensor3(real init)
      : xx(init), xy(init), xz(init), yx(init), yy(init), yz(init), zx(init),
        zy(init), zz(init) {}
...
VECATTR void operator/=(tensor3 &a, const real &b) { a *= real(1.0) / b; }
...
struct alignas(real4) tensor4 {
  ...
  VECATTR tensor4(real xx, real xy, real xz, real xw, real yx, real yy, real yz,
                  real yw, real zx, real zy, real zz, real zw, real wx, real wy,
                  real wz, real ww)
      : xx(xx), xy(xy), xz(xz), xw(xw), yx(yx), yy(yy), yz(yz), yw(yw), zx(zx),
        zy(zy), zz(zz), zw(zw), wx(wx), wy(wy), wz(wz), ww(ww) {}

  VECATTR tensor4(real init)
      : xx(init), xy(init), xz(init), xw(init), yx(init), yy(init), yz(init),
        yw(init), zx(init), zy(init), zz(init), zw(init), wx(init), wy(init),
        wz(init), ww(init) {}
