/*Raul P.Pelaez 2017-2021. Smoothed Particle Hydrodynamics
  ...
  SPH is an Interactor module, any simplectic integrator can be used with it
(i.e VerletNVE).
  ...
  Where:
    ...
    W: Interpolation kernel, a smooth decaying function with a close support.
See SPH_ns::Kernel
...
SPH::SPH(shared_ptr<ParticleGroup> pg, Parameters par)
    : Interactor(pg, "SPH/"), box(par.box), support(par.support),
      viscosity(par.viscosity), gasStiffness(par.gasStiffness),
      restDensity(par.restDensity), nl(par.nl) {
...
SPH::~SPH() { sys->log<System::MESSAGE>("[SPH] Destroyed."); }
namespace SPH_ns {

// This is a Transverser, see NeighbourList.cuh, PairForces.cuh or
// NBodyForces.cuh for a guide on this concept Computes the density on each
// particle
template <class Kernel> struct DensityTransverser {
  Kernel kernel;
  Box box;
  real *density;
  real support;
  ...
  inline __device__ void accumulate(real &total, real current) {
    total += current;
  }
  // Write the result
...
struct Density2Pressure {
  ...
  Density2Pressure(real gasStiffness, real restDensity)
      : gasStiffness(gasStiffness), restDensity(restDensity) {}
...
struct ArtificialViscosity {
  ...
  ArtificialViscosity(real pre, real e = 0.001)
      : viscosityPrefactor(-pre), epsilon(e) {}

  inline __device__ real operator()(real3 vij, real3 rij, real support) const {
    return viscosityPrefactor *
           (dot(vij, rij) / (dot(rij, rij) + epsilon * support * support));
  }
};
...
template <class Kernel> struct ForceTransverser {
  ...
  ForceTransverser(Box box, real4 *force, real3 *vel, real *density,
                   real *pressure, real *mass, ArtificialViscosity viscosity,
                   real support)
  ...
  // Compute Fij
  inline __device__ real3 compute(real4 ri, real4 rj, InfoType infoi,
                                  InfoType infoj) {
    real3 rij = box.apply_pbc(make_real3(rj) - make_real3(ri));
    ...
    return mi * mj * (infoi.Pdivrhosq + infoj.Pdivrhosq + vis) *
           kernel.gradient(rij, support);
  }

  inline __device__ void accumulate(real3 &total, real3 current) {
    total += current;
  }
  // Write result
  inline __device__ void set(int i, const real3 &total) {
    force[i] += make_real4(total);
  }
};
...
void SPH::sum(Interactor::Computables comp, cudaStream_t st) {
  ...
  auto densityTrans =
      SPH_ns::DensityTransverser<Kernel>(box, d_density, d_mass, support);
  // Compute Density
  nl->transverseList(densityTrans, st);
  sys->log<System::DEBUG3>("[SPH] Computing Pressure");
  pressure.resize(numberParticles);
  // Compute Pressure
  thrust::transform(thrust::cuda::par.on(st), d_density,
                    d_density + numberParticles, pressure.begin(),
  ...
  auto forceTrans = SPH_ns::ForceTransverser<Kernel>(
      box, force.raw(), vel.raw(), d_density, d_pressure, d_mass, vis, support);
  // Compute Force
