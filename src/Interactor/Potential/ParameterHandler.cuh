...
template <class Functor> class BasicParameterHandler {
...
public:
  ...
  real getCutOff() { return this->cutOff; }

  struct Iterator {
    PairParameters *globalMem;
    int ntypes;

    Iterator(PairParameters *globalMem, int ntypes)
        : globalMem(globalMem), ntypes(ntypes) {}

    inline __device__ PairParameters operator()(int ti, int tj) {
      if (ntypes == 1)
        return this->globalMem[0];
      cub::CacheModifiedInputIterator<cub::LOAD_CA, PairParameters> itr(
          globalMem);
      if (ti > tj)
        thrust::swap(ti, tj);
