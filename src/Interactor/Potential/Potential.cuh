...
examples/customPotentials.cu See RadialPotential.cuh for an additional example.
 */
...
// this class is meant to be used to specialize RadialPotential, which can then
// to be used as the Potential for PairForces RadialPotential expects a functor
// with the rules of this one:
//    -InputPairParameters, a type with the necessary parameters to
//    differentiate between type pairs -PairParameters, a type with type pair
//    parameters that the GPU computation will use (can be an alias of
//    InputPairParameters). -A force and energy functions taking a squared
//    distance and a PairParameters -A processPairParameters function that
//    transforms between InputPairParameters and PairParameters
//* Notice that the force function in a RadialPotential must, in fact, return
// the modulus of the force divided by the distance, |f|/r.
struct LJFunctor {
  ...
  static inline __device__ real force(real r2, PairParameters params) {
    if (r2 >= params.cutOff2)
      return 0;
    const real invr2 = params.sigma2 / r2;
    const real invr6 = invr2 * invr2 * invr2;
    real fmod = params.epsilonDivSigma2 * (real(-48.0) * invr6 + real(24.0)) *
                invr6 * invr2;
  ...
  static inline __device__ real energy(real r2, PairParameters params) {
    ...
    real E = params.epsilonDivSigma2 * params.sigma2 * real(4.0) * invr6 *
                 (invr6 - real(1.0)) -
             params.shift;
    ...
    //   // E += -(sqrt(r2)-rc)*params.shift -
    //   real(4.0)*params.epsilonDivSigma2*params.sigma2*invrc6*(invrc6-real(1.0));
    // }
    return real(0.5) * E;
  }
  ...
  processPairParameters(InputPairParameters in_par) {
    ...
    if (in_par.shift) {
      ...
      // params.shift = params.epsilonDivSigma2*(real(48.0)*invrc13 -
      // real(24.0)*invrc7);
      params.shift = in_par.epsilon * real(4.0) * invrc6 * (invrc6 - real(1.0));
    } else
      params.shift = real(0.0);
    return params;
  }
};
