/*
  Raul P. Pelaez 2016-2021. Bonded pair forces Interactor implementation. i.e
two body springs
...
BondedForces<BondType, particlesPerBond>::BondedForces(
    shared_ptr<ParticleData> pd, Parameters par,
    std::shared_ptr<BondType> bondCompute)
    : Interactor(pd, "BondedForces/" + type_name<BondType>()),
      bondCompute(bondCompute), TPP(64) {
  // BondedForces does not care about any parameter update, but the BondType
  // might.
...
template <class Bond> class BondProcessor {
...
public:
  void hintNumberBonds(int nbonds) { bondList.reserve(nbonds); }
  ...
  std::vector<Bond> getBondListOfParticle(int index) const {
    ...
    fori(0, blst.size()) { blst[i] = bondList[isInBonds[index][i]]; }
    return blst;
  }
};
...
Bond readNextBond(std::ifstream &in, int currentBonds, int nbonds,
  ...
  for (int i = 0; i < particlesPerBond; i++) {
    if (!(in >> bond.ids[i])) {
      System::log<System::EXCEPTION>("[BondedForces] ERROR! Bond file ended "
                                     "too soon! Expected %d lines, found %d",
                                     nbonds, currentBonds);
...
Bond readNextBondFixedPoint(std::ifstream &in, int currentBonds, int nbonds,
  ...
  Bond bond;
  if (!(in >> bond.ids[0])) {
    System::log<System::EXCEPTION>("[BondedForces] ERROR! Bond file ended too "
                                   "soon! Expected %d lines, found %d",
                                   nbonds, currentBonds);
    throw std::ios_base::failure("File unreadable");
  }
...
template <class Bond>
auto buildBondList(const BondProcessor<Bond> &processor, int particlesPerBond,
                   int nbonds) {
  ...
  std::vector<Bond> blst;
  // Create a compact list of bonds for each particle that has at least one
  // bond. The bond list of particle with id=particlesWithBonds[i] starts at
  // bondStart[i] and ends at bondEnd[i]. So the first bond is
  // bondList[bondStart[i]]
  fori(0, NparticleswithBonds) {
    ...
    forj(0, nbondsi) { bondListCPU[offset + j] = blst[j]; }
...
void BondedForces<BondType, particlesPerBond>::readBonds(std::string fileName) {
  ...
  if (nbonds > 0) {
    ...
    processor.hintNumberBonds(nbonds); // This makes the processor reserve the
                                       // space for the bond list in advance
    for (int b = 0; b < nbonds; b++) {
      auto bond = readNextBond<Bond, BondType>(in, b, nbonds, particlesPerBond);
      processor.registerBond(bond, particlesPerBond);
    }
  }
  // Fixed point bonds are treated as pair bonds were one of the particles is
  // attached to a point in space
  if (particlesPerBond == 2) {
    ...
    if (nbondsFP) {
      ...
      System::log<System::MESSAGE>(
          "[BondedForces] Detected: %d fixed point bonds", nbondsFP);
      processor.hintNumberBonds(nbonds); // This makes the processor reserve the
                                         // space for the bond list in advance
      ...
      for (int b = 0; b < nbondsFP; b++) {
        real3 pos;
        auto bond =
            readNextBondFixedPoint<Bond, BondType>(in, b, nbondsFP, pos);
        this->fixedPointPositions[b + 1] = make_real4(pos);
        processor.registerBond(bond, particlesPerBond);
      }
    }
  }
  std::tie(this->bondList, this->bondStart, this->bondEnd) =
      buildBondList(processor, particlesPerBond, nbonds);
  this->particlesWithBonds = processor.getParticlesWithBonds();
  System::log<System::MESSAGE>(
      "[BondedForces] %d particles are involved in at least one bond.",
...
__global__ void computeBondedThreadPerParticle(
    real4 *force, real *energy, real *virial, Interactor::Computables comp,
    const real4 *pos, const Bond *bondList, const int *bondStart,
    const int *bondEnd, const int *particlesWithBonds,
    const real4 *fixedPointPositions, BondCompute bondCompute,
...
#pragma unroll 3
  for (int b = first; b < last; b++) {
    ...
    int indexes[particlesPerBond]; // Current indexes of the particles involved
                                   // in the bond
    real3 positions[particlesPerBond]; // Positions of the particles involved in
                                       // the bond
#pragma unroll particlesPerBond
    for (int i = 0; i < particlesPerBond; i++) {
      const int id_j = bond.ids[i];
      // Fixed point bonds are treated as a special case of two particles, with
      // one of them having a negative index
      const int index_j = (fpb and id_j < 0) ? -1 : id2index[id_j];
      indexes[i] = index_j;
      positions[i] = make_real3((fpb and id_j < 0) ? fixedPointPositions[-id_j]
                                                   : pos[index_j]);
    }
    const auto ctt =
        bondCompute.compute(index, indexes, positions, comp, bond.bond_info);
    ct.force += ctt.force;
    ct.virial += ctt.virial;
    ct.energy += ctt.energy;
  }
  if (comp.force)
    force[index] += make_real4(ct.force);
  if (comp.energy)
    energy[index] += ct.energy;
  if (comp.virial)
    virial[index] += ct.virial;
}

template <int particlesPerBond, class... T>
void dispatchComputeBonded(int Nparticles_with_bonds, bool fpb, int TPB,
                           cudaStream_t st, T... args) {
  int Nthreads = 128;
  int Nblocks = Nparticles_with_bonds / Nthreads +
                ((Nparticles_with_bonds % Nthreads) ? 1 : 0);
  if (fpb)
    computeBondedThreadPerParticle<particlesPerBond, true>
        <<<Nblocks, Nthreads, 0, st>>>(args...);
  else
    computeBondedThreadPerParticle<particlesPerBond, false>
        <<<Nblocks, Nthreads, 0, st>>>(args...);
  // A block per particle might be benefitial, but lets leave it off for now
  //  if(TPB<=32 or Nparticles_with_bonds < 5000){ //Empirical magic numbers,
  //  could probably be chosen better
  //  }
  //  else if(TPB>=128){
  //  	//This is due to cub having blocksize as template parameter, I hate it
  //  	//computeBondedBlockPerParticle<128><<<Nparticles_with_bonds, 128, 0,
  //  st>>>(args...);
  //  }
  //  else{
  //  	//computeBondedBlockPerParticle<64><<<Nparticles_with_bonds, 64, 0,
  //  st>>>(args...);
  //  }
}
} // namespace BondedForces_ns

// This function chooses which version of the kernel computeBondedForces to use
// as a function of the number of bonds per particle
template <class BondType, int particlesPerBond>
void BondedForces<BondType, particlesPerBond>::callComputeBonded(
    real4 *f, real *e, real *v, cudaStream_t st) {
  ...
  auto d_particlesWithBonds =
      thrust::raw_pointer_cast(particlesWithBonds.data());
  auto d_fixedPointPositions =
      thrust::raw_pointer_cast(fixedPointPositions.data());
  uint Nparticles_with_bonds = bondStart.size();
  Interactor::Computables comp{
      .force = f != nullptr, .energy = e != nullptr, .virial = v != nullptr};
  bool fixedPointsPresent = d_fixedPointPositions;
  BondedForces_ns::dispatchComputeBonded<particlesPerBond>(
      Nparticles_with_bonds, fixedPointsPresent, TPP, st, f, e, v, comp,
      pos.raw(), d_bondList, d_bondStart, d_bondEnd, d_particlesWithBonds,
      d_fixedPointPositions, *bondCompute, id2index, Nparticles_with_bonds);
}

template <class BondType, int particlesPerBond>
void BondedForces<BondType, particlesPerBond>::sum(Computables comp,
                                                   cudaStream_t st) {
  auto force = comp.force ? pd->getForce(access::gpu, access::readwrite).begin()
                          : nullptr;
  auto energy = comp.energy
                    ? pd->getEnergy(access::gpu, access::readwrite).begin()
                    : nullptr;
  auto virial = comp.virial
                    ? pd->getVirial(access::gpu, access::readwrite).begin()
                    : nullptr;
  if (nbonds > 0) {
    System::log<System::DEBUG3>(
        "[BondedForces] Computing Particle-Particle...");
