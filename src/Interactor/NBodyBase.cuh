/*Raul P. Pelaez 2021. NBody base submodule. Does not need a particleData or
particleGroup
...
nbody.transverse(particle_list, index_list, myTransverser, numberParticles,
cudaStream);
...
If it is a list of real4* the standard Transverser interface can be used,
otherwise the first two arguments of the compute function will require to have
the type of the elements in particle_list (
std::iterator_traits<decltype(particle_list)>::value_type ).

index_list is an iterator containing integer types that will be used as indices
for the particle_list and transverser. This can be used to work only on a subset
of the particles. If not present all particles will be processed.
...
__global__ void
transverseGPU(const Iterator particle_list, IndexIterator threadId2Index,
              ...
              Transverser tr, int N, size_t sharedInfoOffset,
  ...
  // but when N is not a multiple of 32 some threads are assigned a particle
  // i>N. This threads cant return, so they are masked to not do any work
  bool active = true;
  if (tid >= N)
    active = false;
  ...
  // Storing blockDim.x values in shared memory and processing all of them in
  // parallel
  SFINAE::TransverserAdaptor<Transverser> adaptor;
  extern __shared__ char shMem[];
  PerParticleValue *shValue =
      (PerParticleValue *)(shMem + transverserShMemSize);
  ...
  // Distribute the N particles in numTiles tiles. Storing in each tile
  // blockDim.x values in shared memory This way all threads are accesing the
  // same memory addresses at the same time
  for (int tile = 0; tile < numTiles; tile++) {
    // Load this tiles particles values to shared memory
    int i_load = tile * blockDim.x + threadIdx.x;
    if (i_load < N) { // Even if im not active, my thread may load a value each
    ...
    for (uint counter = 0; counter < blockDim.x; counter++) {
      ...
      if (cur_j < N) { // If the current particle exists, compute and accumulate
        ...
        Adaptor::accumulate(tr, quantity,
                            adaptor.computeSharedMem(
                                tr, valuei, shValue[counter], shInfo, counter));
      }
...
class NBodyBase {
public:
  NBodyBase() { System::log<System::DEBUG>("[NBody] Created"); }

  // For each particle in the list (only in the elements given by the argument
  // indices), applies the transverser to every other.
  ...
  static void transverse(Iterator particle_list, IndexIterator &indices,
                         Transverser &a_tr, int numberParticles,
                         cudaStream_t st = 0) {
    using PerParticleValue =
        typename std::iterator_traits<Iterator>::value_type;
    System::log<System::DEBUG2>("[NBody] Transversing with %s",
    ...
    constexpr size_t info_align =
        alignof(typename SFINAE::Delegator<Transverser>::InfoType);
    ...
    System::log<System::DEBUG4>("[NBody] %zu sh mem",
                                Nthreads * sh_size + extra_alignment);
    System::log<System::DEBUG4>("[NBody] %zu Additional sh mem", extra_sh);
    nbody_ns::
        transverseGPU<<<Nblocks, Nthreads,
                        Nthreads * sh_size + extra_sh + extra_alignment, st>>>(
            particle_list, indices, numtiles, a_tr, numberParticles,
            extra_alignment + Nthreads * sizeof(PerParticleValue), extra_sh);
    CudaCheckError();
  }

  // For each particle in the list, applies the transverser to every other.
  template <class Iterator, class Transverser>
  static inline void transverse(Iterator &particle_list, Transverser &a_tr,
