/*Raul P. Pelaez 2020. Near field section of the Doubly Periodic Poisson
 * algorithm
 */
...
__host__ __device__ double GreensFunctionNearPotential(double r2, double split,
                                                       double gw,
                                                       double permitivity) {
  ...
  if (sqrt(r2) > gw * 1e-2) {
    double r = sqrt(r2);
    G = (1.0 / (4.0 * M_PI * permitivity * r) *
         (erf(r / (2 * gw)) - erf(r / sqrt(4 * gw2 + 1 / (split * split)))));
  } else {
...
__host__ __device__ double GreensFunctionNearField(double r2, double split,
                                                   double gw,
                                                   double permitivity) {
  double r = sqrt(r2);
  double gw2 = gw * gw;
...
template <class Parameters> double computeCutOffDistance(Parameters par) {
  real rn = par.gw;
  const real dr = par.gw * 0.01;
  real ratioAtRn;
  do {
    rn += dr;
    real dGdr_split = GreensFunctionNearField(
        rn * rn, par.split, par.gw / sqrt(2.0), par.permitivity.inside);
    real dGdr = GreensFunctionNearField(rn * rn, 1e-10, par.gw / sqrt(2.0),
                                        par.permitivity.inside);
...
void throwIfInvalidConfiguration(double nearFieldCutOff, double H) {
  ...
  if (nearFieldCutOff > maximumAllowedCutOff) {
    System::log<System::EXCEPTION>(
        "[DPPoissonSlab] Close range cut off is too large (%g, max is %g), "
        "increase splitting parameter or lower tolerance",
        nearFieldCutOff, maximumAllowedCutOff);
...
public:
  NearField(shared_ptr<ParticleGroup> pg, Parameters par)
    ...
    System::log<System::MESSAGE>("[DPPoissonSlab] Near field cut off: %g",
                                 rcut);
...
void NearField::initializeTabulatedGreensFunctions() {
  ...
  int Ntable = std::max(
      1 << 16,
      std::min(1 << 20, 2 * int(rcut / (par.gw * par.tolerance * 1e2))));
  System::log<System::MESSAGE>("[Poisson] Elements in near field table: %d",
                               Ntable);
...
struct NearFieldTransverser {
  ...
  NearFieldTransverser(real *energy_ptr, real4 *force_ptr, real *charge,
                       TabulatedFunction<real2> greenTables, Box box, real H,
                       real rcut, Permitivity perm, real split, real gw,
                       real4 *field_ptr)
      : energy_ptr(energy_ptr), force_ptr(force_ptr), charge(charge),
        split(split), gw(gw), box(box), H(H), rcut(rcut), perm(perm),
        GreensFunctionFieldAndPotential(greenTables), field_ptr(field_ptr) {}
  ...
  __device__ returnInfo compute(real4 pi, real4 pj, Info infoi,
                                Info infoj) const {
    auto FandE =
        infoj.charge * computeFieldPotential(make_real3(pi), make_real3(pj));
    ...
    if (hasImageCharge) {
      ...
      real epratio =
          isinf(ep) ? real(1.0) : ((ep - perm.inside) / (ep + perm.inside));
...
private:
  ...
  __device__ returnInfo computeFieldPotential(real3 pi, real3 pj) const {
    ...
    // real2 greensFunctions =
    // make_real2(nearField_ns::GreensFunctionNearPotential(r2, split, gw,
    // perm.inside),
    // nearField_ns::GreensFunctionNearField(r2, split, gw,
    // perm.inside));
...
void NearField::compute(cudaStream_t st, real4 *fieldAtParticles) {
  if (par.split) {
    System::log<System::DEBUG2>(
        "[DPPoissonSlab] Near field energy computation");
    ...
    auto tr = NearFieldTransverser(
        energy.begin(), force.begin(), charge.begin(), *greenTables, box, par.H,
        rcut, par.permitivity, split, gw, fieldAtParticles);
