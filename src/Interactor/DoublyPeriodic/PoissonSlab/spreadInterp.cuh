/*Raul P. Pelaez 2020-2025. Spreading and interpolation for the Doubly Periodic
...
struct Gaussian {
  int3 support;
  Gaussian(real tolerance, real width, real h, real H, real He, real nz,
           int supportxy)
      : nz(nz) {
    ...
    this->Htot = H + 6 * He;
    int czmax =
        ceil((nz - 1) * (acos((0.5 * H + He) / (0.5 * Htot)) / real(M_PI)));
    support.z = 2 * czmax + 1;
  }
...
template <class PosIterator, class ChargeIterator> struct ChargeGroup {
...
ChargeGroup<PosIterator, ChargeIterator>
make_charge_group(const PosIterator &pos, const ChargeIterator &charge,
                  int numberParticles) {
...
template <class PositionIterator> struct HasImageChargeWithWall {
...
public:
  ...
  void separate(const Container &positions, real H, real thresholdDistance,
                cudaStream_t st) {
    detail::IsPositionCloseToWall predicate(H, thresholdDistance,
                                            positions.begin());
...
private:
  ...
  void identifyImageCharges(const Container &positions, real H,
                            real thresholdDistance, int nnear,
                            cudaStream_t st) {
    ...
    {
      detail::HasImageChargeWithWall<decltype(positionsNearWalls)>
          hasImageCharge(0.5 * H, thresholdDistance, positionsNearWalls);
...
struct toReal4 {
  inline __device__ real4 operator()(real3 a) const { return make_real4(a); }
};
...
//   FieldPotential2ForceEnergy(real4* f, real* e, real *q, real4*
//   field):force(f), energy(e), charge(q), field(field), i(-1){}
...
struct FieldPotential2ForceEnergy {
  ...
  __device__ __host__ FieldPotential2ForceEnergy(real4 *f, real *e, real *q,
                                                 real4 *field)
      : force(f), energy(e), charge(q), field(field), i(0) {}

  __device__ __host__ FieldPotential2ForceEnergy operator[](int offset) const {
    FieldPotential2ForceEnergy result = *this;
    result.i = i + offset;
    return result;
  }
...
public:
  ...
  SpreadInterpolateCharges(std::shared_ptr<System> sys,
                           std::shared_ptr<ParticleData> pd, Parameters par)
      : sys(sys), pd(pd), grid(par.grid), H(par.H) {
    sys->log<System::DEBUG>("[SpreadInterpolate] Initialized");
    initializeKernel(par.tolerance, par.gaussianWidth, par.He,
                     par.maximumSupport, par.support);
    initializeQuadratureWeights();
  }

  cached_vector<real> spreadChargesNearWalls(SeparatedCharges &sep,
                                             cudaStream_t st) {
    sys->log<System::DEBUG>("Spreading %d particles near walls",
                            sep.nearWalls.size());
    auto n = grid.cellDim;
    cached_vector<real> gridCharges((2 * (n.x / 2 + 1)) * n.y * (2 * n.z - 2));
    thrust::fill(thrust::cuda::par.on(st), gridCharges.begin(),
                 gridCharges.end(), real());
    int *indices = thrust::raw_pointer_cast(sep.nearWalls.data());
    spreadFromIndices(indices, thrust::raw_pointer_cast(gridCharges.data()),
                      sep.nearWalls.size(), st);
    return gridCharges;
  }

  void spreadChargesFarFromWallAdd(SeparatedCharges &sep,
                                   cached_vector<real> &gridCharges,
                                   cudaStream_t st) {
    sys->log<System::DEBUG>("Spreading %d particles far from walls",
                            sep.farFromWalls.size());
  ...
  void spreadImageChargesAdd(SeparatedCharges &sep,
                             cached_vector<real> &gridCharges,
                             Permitivity permitivity, cudaStream_t st) {
    sys->log<System::DEBUG>(
        "Spreading %d (top) and %d (bottom) particle images",
  ...
  void interpolateFieldsToParticles(Real4Container &gridFieldPotential,
                                    cudaStream_t st, real4 *fieldAtParticles) {
    sys->log<System::DEBUG2>(
        "[DPPoissonSlab] Interpolating forces and energies");
    ...
    auto energies =
        pd->getEnergy(access::location::gpu, access::mode::readwrite);
    ...
    IBM_ns::DefaultWeightCompute
        wc; // If non-default QuadratureWeights are used, a weight compute must
            // also be passed.
    ibm.gather(pos.begin(), f_tr, d_gridForcesEnergies, *qw, wc,
               numberParticles, st);
    CudaCheckError();
  }

private:
  void initializeKernel(real tolerance, real gaussianWidth, real He,
                        int maximumSupport, int support) {
    sys->log<System::DEBUG>("[DPPoissonSlab] Initialize kernel");
    double h = grid.cellSize.x;
    auto n = grid.cellDim;
    this->kernel = std::make_shared<Kernel>(tolerance, gaussianWidth, h, H, He,
                                            n.z, support);
    sys->log<System::MESSAGE>(
        "[DPPoissonSlab] Kernel support %g (%d %d %d cells)",
        kernel->support.x * h, kernel->support.x, kernel->support.y,
        kernel->support.z);
    sys->log<System::MESSAGE>("[DPPoissonSlab] Maximum support allowed: %d",
                              maximumSupport);
    ...
    sys->log<System::MESSAGE>("[DPPoissonSlab] Spread weight at h: %g",
                              kernel->phi(h, make_real3(h)));
    sys->log<System::MESSAGE>(
        "[DPPoissonSlab] Spread at maximum distance (%g) : %g",
        h * (kernel->support.x / 2),
        kernel->phi(h * (kernel->support.x / 2) - 1e-5, make_real3(h)));
  }

  void initializeQuadratureWeights() {
    sys->log<System::DEBUG>(
        "[DPPoissonSlab Spread] Initialize quadrature weights");
  ...
  void spreadBottomWallImages(SeparatedCharges &sep,
                              cached_vector<real> &gridCharges,
                              Permitivity permitivity, cudaStream_t st) {
    int *bottomImagesIndices =
        thrust::raw_pointer_cast(sep.imagesBottom.data());
    spreadImagesWithIndices(bottomImagesIndices, gridCharges,
                            permitivity.bottom / permitivity.inside,
                            sep.imagesBottom.size(), -0.5 * H, st);
  }

  void spreadTopWallImages(SeparatedCharges &sep,
                           cached_vector<real> &gridCharges,
                           Permitivity permitivity, cudaStream_t st) {
    int *topImagesIndices = thrust::raw_pointer_cast(sep.imagesTop.data());
    spreadImagesWithIndices(topImagesIndices, gridCharges,
                            permitivity.top / permitivity.inside,
                            sep.imagesTop.size(), 0.5 * H, st);
  }

  void spreadImagesWithIndices(int *indices, cached_vector<real> &gridCharges,
                               real permitivityRatio, int numberImages,
                               real wallZ, cudaStream_t st) {
    real *d_gridCharges = thrust::raw_pointer_cast(gridCharges.data());
    auto charges = pd->getCharge(access::location::gpu, access::mode::read);
    auto chargeImages = thrust::make_transform_iterator(
        indices, ImageChargeTransform(charges.raw(), permitivityRatio));
    auto pos = pd->getPos(access::location::gpu, access::mode::read);
    auto pos_perm = thrust::make_permutation_iterator(pos, indices);
    auto posImages = thrust::make_transform_iterator(
        pos_perm, ImagePositionTransform(wallZ));
    auto group = make_charge_group(posImages, chargeImages, numberImages);
    spreadGroup(d_gridCharges, group, st);
  }

  void spreadFromIndices(int *indices, real *gridCharges, int numberParticles,
                         cudaStream_t st) {
    ...
    auto pos_perm = thrust::make_permutation_iterator(pos, indices);
    auto charge_perm =
        thrust::make_permutation_iterator(charges.begin(), indices);
  ...
  void spreadGroup(real *d_gridCharges, ChargeGroup group, cudaStream_t st) {
    if (group.numberParticles == 0)
      return;
    sys->log<System::DEBUG>("Spreading %d particles", group.numberParticles);
    auto minusChargeIterator =
        thrust::make_transform_iterator(group.charge, thrust::negate<real>());
    int3 n = grid.cellDim;
    IBM<Kernel, Grid> ibm(kernel, grid,
                          IBM_ns::LinearIndex3D(2 * (n.x / 2 + 1), n.y, n.z));
    ibm.spread(group.pos, minusChargeIterator, d_gridCharges,
               group.numberParticles, st);
    CudaCheckError();
  }
};
} // namespace DPPoissonSlab_ns
