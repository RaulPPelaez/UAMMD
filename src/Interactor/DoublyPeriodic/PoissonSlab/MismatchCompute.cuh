/*Raul P. Pelaez 2020. Potential and field mismatch at the boundaries for the
 * Doubly Periodic Poisson solver. Slab geometry
 */
...
public:
  MismatchGPU(int nz, real H, Permitivity perm, real thetaTop, real thetaBottom)
      : nz(nz), H(H), perm(perm), thetaTop(thetaTop), thetaBottom(thetaBottom) {
    this->metallicTop = std::isinf(perm.top);
    this->metallicBottom = std::isinf(perm.bottom);
  }

  template <class Iterator1, class Iterator2>
  __device__ MismatchVals computeMismatch(Iterator1 insideSolution,
                                          Iterator2 outsideSolution,
                                          cufftComplex2 surfaceValueFourier) {
    auto evalthetaInside =
        evaluateThetas(insideSolution, thetaTop, thetaBottom, nz);
    auto evalthetaOutside =
        evaluateThetas(outsideSolution, thetaTop, thetaBottom, nz);
    ...
    if (not metallicTop) {
      ...
      mismatch.mEH = perm.top * et * evalthetaOutside.w -
                     perm.inside * evalthetaInside.w -
                     surfaceChargeAtTopWallFourier;
    } else {
    ...
    if (not metallicBottom) {
      ...
      mismatch.mE0 = perm.bottom * eb * evalthetaOutside.z -
                     perm.inside * evalthetaInside.z +
                     surfaceChargeAtBottomWallFourier;
    } else {
  ...
  __device__ cufftComplex2 computeLinearModeCorrection(
      cufftComplex mismatchFieldZBottom, cufftComplex mismatchFieldZTop,
      cufftComplex mismatchPotentialBottom, cufftComplex mismatchPotentialTop,
      Iterator outsideSolutionMode0) {
    auto evalThetaOutside =
        evaluateThetas(outsideSolutionMode0, thetaTop, thetaBottom, nz);
    ...
    if (metallicTop and not metallicBottom) {
      ...
      A0 = (E * eb - mismatchFieldZBottom / perm.inside + C * et -
            mismatchFieldZTop / perm.inside) *
           real(0.5);
      B0 = cufftComplex();
    }
    return {A0, B0};
...
private:
  ...
  __device__ cufftComplex4 evaluateThetas(SolutionIterator sol, real thetaTop,
                                          real thetaBot, int Nz) {
...
namespace detail {
__device__ void storeMismatch(MismatchVals mis, MismatchPtr ptr, int2 ik,
                              Index3D indexer) {
  *make_third_index_iterator(ptr.potentialBottom, ik.x, ik.y, indexer) =
      mis.mP0;
...
__global__ void computeMismatchD(const cufftComplex4 *insideSolution,
                                 ...
                                 MismatchPtr mismatch, MismatchGPU mcomp,
  ...
  auto inside_ik =
      make_third_index_iterator(insideSolution, ik.x, ik.y, indexer);
  auto outside_ik =
      make_third_index_iterator(outsideSolution, ik.x, ik.y, indexer);
  cufftComplex2 sfc =
      *make_third_index_iterator(surfaceValuesFourier, ik.x, ik.y, indexer);
  auto m = mcomp.computeMismatch(inside_ik, outside_ik, sfc);
  detail::storeMismatch(m, mismatch, ik, indexer);
  if (id == 0) {
    mismatch.linearModeCorrection[0] = mcomp.computeLinearModeCorrection(
        m.mE0, m.mEH, m.mP0, m.mPH, outside_ik);
  }
}
...
public:
  ...
  // surfaceValues hold surface charges of the walls. If the permittivity is
  // infinite (metallic boundary) at some wall, surfaceValues hold the potential
  // at the wall instead.
  ...
  MismatchPtr compute(Container &insideSolution, Container &outsideSolution,
    ...
    computeMismatchD<<<nblocks, blockSize, 0, st>>>(
        i_ptr, o_ptr, surfaceValuesFourier, mismatch_ptr, gpuCompute, n.x, n.y,
        n.z);
    CudaCheckError();
    return mismatch_ptr;
  }
  ...
  MismatchPtr getRawPointers() {
    ...
    ptrs.linearModeCorrection =
        thrust::raw_pointer_cast(linearModeCorrection.data());
    return ptrs;
  }
};
