...
setUpBVPKernelTemporalStorage(int numberConcurrentThreads, int nz) {
  ...
  tmp.potentialChebyshevCoefficients =
      mem.registerStorageRequirement<cufftComplex>(nz);
  tmp.secondDerivativeChebyshevCoefficients =
      mem.registerStorageRequirement<cufftComplex>(nz);
  tmp.firstDerivativeChebyshevCoefficients =
      mem.registerStorageRequirement<cufftComplex>(nz);
...
__device__ void firstDerivativeChebyshevCoefficients(FNIterator &fn,
                                                     OutputIterator &out,
                                                     int nz, real H) {
  ...
  for (int i = nz - 1; i >= 0; i--) {
    ...
    if (i <= nz - 2)
      Dfni = Dfnp2 + real(2.0) * (i + 1) * fn[i + 1] / H;
    if (i == 0)
      Dfni *= real(0.5);
    out[i] = Dfni;
    Dfnp2 = Dfnp1;
...
// TODO: There is no need for firstDerivativeChebyshevCoefficients, it could be
// directly stored in gridFieldPotential[i].z
...
__global__ void solveBVPFieldPotential(BVPSolver bvp, int nkx, int nky, int nz,
                                       ...
                                       BVPKernelTemporalStorage tmp,
                                       char *tmp_storage, real permitivity) {
  ...
  auto gridFieldPotential =
      make_third_index_iterator(gridFieldPotential_raw, ik.x, ik.y, indexer);
  ...
  fori(0, nz) {
    ...
    gridFieldPotential[i].x =
        -cufftComplex({-kvec.x * poti.y, kvec.x * poti.x}) * isPairedX;
    gridFieldPotential[i].y =
        -cufftComplex({-kvec.y * poti.y, kvec.y * poti.x}) * isPairedY;
...
public:
  ...
  solveFieldPotential(Container &gridChargesFourier, cudaStream_t st) {
    System::log<System::DEBUG2>("[DPPoissonSlab] BVP solve");
    cufftComplex *d_gridChargesFourier =
        thrust::raw_pointer_cast(gridChargesFourier.data());
    const int3 n = cellDim;
    cached_vector<cufftComplex4> gridFieldPotentialFourier((n.x / 2 + 1) * n.y *
                                                           (2 * n.z - 2));
    cufftComplex4 *d_gridFieldPotentialFourier =
        thrust::raw_pointer_cast(gridFieldPotentialFourier.data());
    ...
    bvp_ns::solveBVPFieldPotential<<<numberBlocks, blockSize, 0, st>>>(
        gpuSolver, n.x, n.y, n.z, Lxy, H, d_gridChargesFourier,
        d_gridFieldPotentialFourier, tmp, tmp_storage_ptr, permitivity);
    CudaCheckError();
...
private:
  void initializeBoundaryValueProblemSolver() {
    ...
    auto topBC = thrust::make_transform_iterator(
        thrust::make_counting_iterator<int>(0),
        BoundaryConditionsDispatch<TopBoundaryConditions, decltype(klist)>(
            klist, H));
    auto bottomBC = thrust::make_transform_iterator(
        thrust::make_counting_iterator<int>(0),
        BoundaryConditionsDispatch<BottomBoundaryConditions, decltype(klist)>(
            klist, H));

    int numberSystems = (nk.x / 2 + 1) * nk.y;
    this->bvpSolver = std::make_shared<BVP::BatchedBVPHandlerReal>(
        klist, topBC, bottomBC, numberSystems, H, cellDim.z);
    CudaCheckError();
  }
};
} // namespace DPPoissonSlab_ns
