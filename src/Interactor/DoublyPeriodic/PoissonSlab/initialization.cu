...
void throwIfInvalidSplit(DPPoissonSlab::Parameters par) {
  constexpr real upperRangeFactor =
      3.2; // TODO: Hardcoded for 1e-3 tolerance, should change
  real minimumSplit =
      upperRangeFactor / (par.H + par.numberStandardDeviations * par.gw);
  real maximumSplit = 1 / (par.gw * sqrt(12));
  if (par.split < minimumSplit or par.split > maximumSplit) {
    System::log<System::ERROR>(
        "Splitting parameter is outside of valid range [%g:%g] (got %g)",
...
DPPoissonSlab::DPPoissonSlab(shared_ptr<ParticleGroup> pg, Parameters par)
  ...
  if (par.split <= 0) {
    ...
    sys->log<System::EXCEPTION>(
        "[DPPoissonSlab] Pass only split OR Nxy, not both");
...
void DPPoissonSlab::printStartingMessages(Parameters par) {
  ...
  sys->log<System::MESSAGE>(
      "[DPPoissonSlab] permitivity: %g (inside), %g (top), %g (bottom)",
      par.permitivity.inside, par.permitivity.top, par.permitivity.bottom);
  sys->log<System::MESSAGE>("[DPPoissonSlab] Gaussian source width: %g",
                            par.gw);
  sys->log<System::WARNING>(
      "[DPPoissonSlab] Not subtracting adding phi(0,0,0)");
  bool isMetallicTop = std::isinf(par.permitivity.top);
  bool isMetallicBottom = std::isinf(par.permitivity.bottom);
  if (isMetallicTop)
    sys->log<System::MESSAGE>(
        "[DPPoissonSlab] Top wall is metallic, surface values will be "
        "interpreted as potential for the top wall.");
  if (isMetallicBottom)
    sys->log<System::MESSAGE>(
        "[DPPoissonSlab] Bottom wall is metallic, surface values will be "
        "interpreted as potential for the bottom wall.");
  if (isMetallicBottom and not isMetallicTop) {
    sys->log<System::ERROR>("[DPPoissonSlab] If only one wall is metallic, it "
                            "must be the top wall");
    throw std::runtime_error(
        "[DPPoissonSlab] Bottom metallic wall not implemented");
  }
}
...
void DPPoissonSlab::initializeFarField(Parameters par) {
  ...
  this->farField =
      std::make_shared<DPPoissonSlab_ns::FarField>(sys, pd, pg, ffpar);
}
} // namespace uammd
