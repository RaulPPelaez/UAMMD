...
public:
  virtual real top(real x, real y) { return 0; }

  virtual real bottom(real x, real y) { return 0; }
};

namespace detail {
// This is a very barebones container. Its purpose is only to avoid the
// unnecessary unninitialized_fill kernel that thrust issues on device_vector
// creation. Thus it mascarades as a thrust::device_vector.
...
class UninitializedCachedContainer {
  ...
  Container create(size_t s) {
    if (s > 0) {
      try {
        return Container(Allocator().allocate(s),
                         [](T *ptr) { Allocator().deallocate(ptr); });
      } catch (...) {
        System::log<System::EXCEPTION>("[UninitializedCachedContainer] Could "
                                       "not allocate buffer of size %zu",
                                       s);
        throw;
      }
    } else {
...
public:
  ...
  thrust::device_ptr<T> data() const { return thrust::device_ptr<T>(begin()); }
};
} // namespace detail
#ifndef UAMMD_DEBUG
template <class T> using gpu_container = thrust::device_vector<T>;
// template<class T>  using cached_vector = thrust::device_vector<T,
// System::allocator_thrust<T>>;
template <class T>
using cached_vector = detail::UninitializedCachedContainer<T>;
#else
template <class T>
...
using Iterator = thrust::permutation_iterator<
    RandomAccessIterator,
    thrust::transform_iterator<ThirdIndexIteratorTransform,
                               thrust::counting_iterator<int>>>;

template <class RandomAccessIterator>
inline __host__ __device__ Iterator<RandomAccessIterator>
