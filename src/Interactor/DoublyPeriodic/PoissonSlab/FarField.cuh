/*Raul P. Pelaez 2020. Far field section of the Doubly Periodic Poisson solver.
 * Slab geometry
 */
...
int3 proposeCellDim(real farFieldGaussianWidth, real upsampling, real3 L) {
  constexpr int minimumNumberCells = 16; // Just an arbitrary number. It does
                                         // not really help to have <=16 cells
  ...
  while (cd.z % 2 != 0) {
    cd.z++;
    cd = nextFFTWiseSize3D(cd);
    // cd.z must be even so that cd.z = ((2*cd.z-2)+2)/2 and 2*cd.z-2 is still a
    // friendly number
  }
  cd.z = (cd.z + 2) / 2;
  return cd;
...
void throwIfInvalidConfiguration(double He, double minBoxSize) {
  if (He > minBoxSize) {
    System::log<System::EXCEPTION>(
        "[DPPoissonSlab] Extra height is too high (%g, max is %g), increase "
        "splitting parameter or lower tolerance",
        He, minBoxSize);
...
public:
  FarField(shared_ptr<System> sys, shared_ptr<ParticleData> pd,
           shared_ptr<ParticleGroup> pg, Parameters par)
    ...
    this->He = detail::computeExtraHeight(par.numberStandardDeviations, par.gw,
                                          par.split);
    ...
    this->cellDim = detail::proposeCellDim(farFieldGaussianWidth,
                                           par.upsampling, totalBoxSize);
    sys->log<System::MESSAGE>("[DPPoissonSlab] cells: %d %d %d", cellDim.x,
                              cellDim.y, cellDim.z);
    sys->log<System::MESSAGE>(
        "[DPPoissonSlab] box size: %g %g %g (enlarged to %g)", par.Lxy.x,
        par.Lxy.y, par.H, par.H + He * 6);
    ...
    if (par.split > 0) {
      ...
      sys->log<System::MESSAGE>(
          "[DPPoissonSlab] Far field width: %g, (%g times original width)",
    ...
    this->correction = std::make_shared<Correction>(par.permitivity, cellDim,
                                                    par.Lxy, par.H, He);
  ...
  void compute(cudaStream_t st, real4 *fieldAtParticles = nullptr) {
    ...
    auto gridCharges =
        ibm->spreadChargesNearWalls(separatedCharges, st); // Step 2
    // DPPoissonSlab_ns::writeRealField<real>(gridCharges, cellDim,
    // "gf_C1.dat");
    auto gridChargesFourier = fct->forwardTransform(gridCharges, st); // Step 2
    auto outsideSolution =
        bvp->solveFieldPotential(gridChargesFourier, st); // Step 2
    // DPPoissonSlab_ns::writeComplex4Field(outsideSolution, cellDim,
    // par.H,"solOut.dat");
    ibm->spreadChargesFarFromWallAdd(separatedCharges, gridCharges,
                                     st); // Step 3
    ibm->spreadImageChargesAdd(separatedCharges, gridCharges, par.permitivity,
                               st); // Step 3
    // DPPoissonSlab_ns::writeRealField<real>(gridCharges, cellDim,
    // "gf_C1pC2pC1star.dat");
    gridChargesFourier = fct->forwardTransform(gridCharges, st); // Step 3
    // DPPoissonSlab_ns::writeComplexField(gridChargesFourier, cellDim, par.H,
    // "gf_C1pC2pC1starFourier.dat");
    auto insideSolution =
        bvp->solveFieldPotential(gridChargesFourier, st); // Step 3
    // DPPoissonSlab_ns::writeComplex4Field(insideSolution, cellDim,
    // par.H,"solIn.dat");
    auto surfaceValues_ptr =
        thrust::raw_pointer_cast(surfaceValuesFourier.data());
    correction->correctSolution(insideSolution, outsideSolution,
                                surfaceValues_ptr, st); // Steps 4,5,6
    // DPPoissonSlab_ns::writeComplex4Field(insideSolution, cellDim,
    // par.H,"solCorrected.dat");
    if (par.printK0Mode) {
      solutionZeroMode.resize(cellDim.z);
      Index3D indexer(cellDim.x / 2 + 1, cellDim.y, cellDim.z);
      auto k0z =
          make_third_index_iterator(insideSolution.begin(), 0, 0, indexer);
      thrust::copy(thrust::cuda::par.on(st), k0z, k0z + cellDim.z,
                   solutionZeroMode.begin());
    }
    auto gridFields = fct->inverseTransform(insideSolution, st);
    // DPPoissonSlab_ns::writeRealField<real4>(gridFields, cellDim,
    // "solGrid.dat");
  ...
  auto getK0Mode() {
    ...
    thrust::copy(solutionZeroMode.begin(), solutionZeroMode.end(),
                 h_zeroMode.begin());
    return h_zeroMode;
  }
...
private:
  std::shared_ptr<SpreadInterpolateCharges> createImmersedBoundary() {
    ...
    if (par.split > 0) {
      ...
      int czmax = int((cellDim.z - 1) *
                      (acos(2.0 * (0.5 * par.H + 2 * He) / Htot) / real(M_PI)));
...
gpu_container<cufftComplex2>
takeSurfaceValueDensityToFourier(cached_vector<real> &surfaceValueTop,
                                 cached_vector<real> &surfaceValueBottom,
                                 int2 gridSize) {
  int2 n = gridSize;
  ...
  CufftSafeCall(cufftPlanMany(&cufft_plan_forward, 2, &cdtmp.x, &inembed.x, 1,
                              1, &oembed.x, 2, 1,
                              CUFFT_Real2Complex<real>::value, 1));
  real *i_data = thrust::raw_pointer_cast(surfaceValueTop.data());
  cufftComplex *o_data =
      (cufftComplex *)thrust::raw_pointer_cast(surfaceValuesFourier.data());
  CufftSafeCall(
      cufftExecReal2Complex<real>(cufft_plan_forward, i_data, o_data));
  i_data = thrust::raw_pointer_cast(surfaceValueBottom.data());
  CufftSafeCall(
      cufftExecReal2Complex<real>(cufft_plan_forward, i_data, o_data + 1));
  return surfaceValuesFourier;
}

void FarField::setUpSurfaceValues(
    std::shared_ptr<SurfaceValueDispatch> surfaceValue) {
  ...
  forj(0, n.y) {
    fori(0, n.x) {
      ...
      surfaceValueBottom[i + 2 * (n.x / 2 + 1) * j] =
          surfaceValue->bottom(x, y);
    }
  }
  real totalWallCharge =
      std::accumulate(surfaceValueTop.begin(), surfaceValueTop.end(), 0.0);
  totalWallCharge += std::accumulate(surfaceValueBottom.begin(),
                                     surfaceValueBottom.end(), 0.0);
  real totalSystemCharge = qtot + totalWallCharge;
  if (totalSystemCharge != 0) {
    System::log<System::WARNING>("[DPPoissonSlab] The system is not "
                                 "electroneutral (found %g total charge)",
                                 totalSystemCharge);
    System::log<System::WARNING>(
        "[DPPoissonSlab] To ensure electroneutrality half an opposite charge "
        "will be placed on each wall. This is an assumption of the algorithm "
