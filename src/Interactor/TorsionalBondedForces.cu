/*Raul P. Pelaez 2019. Torsional bonds (bonds between three particles).

 */

#include"TorsionalBondedForces.cuh"
...
namespace uammd{
  ...
  TorsionalBondedForces<BondType>::BondProcessor TorsionalBondedForces<BondType>::readBondFile(std::string bondFile){
	...
	try{
	  ...
	  sys->log<System::ERROR>("[TorsionalBondedForces] ERROR! Bond file ended too soon! Expected %d lines, found %d", nbonds, b);
	  throw;
	}
  ...
  void TorsionalBondedForces<BondType>::generateBondList(const BondProcessor &bondProcessor){
    ...
    sys->log<System::DEBUG>("[TorsionalBondedForces] Generating list");
    sys->log<System::MESSAGE>("[TorsionalBondedForces] %d particles are involved in at least one bond.", h_particlesWithBonds.size());
    ...
    sys->log<System::DEBUG>("[TorsionalBondedForces] Uploading list");
  ...
  // real TorsionalBondedForces::sumVirial(){
  //   return 0;
  // }
