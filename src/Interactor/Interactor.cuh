...
Interactor is an interface for modules that can compute forces, energies and/or
virials according to a certain interaction. For a class to be a valid
Interactor, it must override sum() member function.

Interactor is also ParameterUpdatable, which means that any Interactor can
override any of the available update*() functions.
...
public:
  ...
  // This struct exposes the different targets of computation that can be
  // requested from an Interactor.
  struct Computables {
    bool force = false;
    bool energy = false;
    bool virial = false;
