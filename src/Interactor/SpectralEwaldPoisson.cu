/*Raul P. Pelaez 2019-2025. Spectral Poisson solver with Ewald splitting.
TODO:
100- Heuristics for selecting cut offs with tolerance need reevaluation
50- In place C2R cufft does not work. I believe cufft just does not allow it,
but I am not sure.
 */
...
__host__ __device__ real greensFunction(real r2, real gw, real split,
                                        real epsilon) {
  double G = 0;
  if (r2 > gw * gw * gw * gw) {
    double r = sqrt(r2);
    G = (1.0 / (4.0 * M_PI * epsilon * r) *
         (erf(r / (2 * gw)) -
          erf(r / sqrt(4 * gw * gw + 1 / (split * split)))));
  } else {
    const double pi32 = pow(M_PI, 1.5);
    const double gw2 = gw * gw;
    const double invsp2 = 1.0 / (split * split);
    const double selfterm =
        1.0 / (4 * pi32 * gw) - 1.0 / (2 * pi32 * sqrt(4 * gw2 + invsp2));
    const double r2term = 1.0 / (6.0 * pi32 * pow(4.0 * gw2 + invsp2, 1.5)) -
                          1.0 / (48.0 * pi32 * gw2 * gw);
    const double r4term = 1.0 / (640.0 * pi32 * gw2 * gw2 * gw) -
                          1.0 / (20.0 * pi32 * pow(4 * gw2 + invsp2, 2.5));
    G = 1.0 / epsilon * (selfterm + r2 * r2term + r2 * r2 * r4term);
  }
  return G;
}

__device__ __host__ real greensFunctionField(real r, real gw, real split,
                                             real epsilon) {
  double r2 = r * r;
  double gw2 = gw * gw;
  ...
  double fmod = 0;
  if (r2 > gw * gw * gw * gw) {
    double invrterm = exp(-0.25 * r2 / newgw2) / sqrt(M_PI * newgw2) -
                      exp(-0.25 * r2 / gw2) / sqrt(M_PI * gw2);
    ...
    double r3term = 1 / (160 * pi32) *
                    (1.0 / (newgw2 * newgw2 * newgw) - 1.0 / (gw2 * gw2 * gw));
...
template <class T>
std::shared_ptr<T> allocateTemporaryArray(size_t numberElements) {
...
Poisson::Poisson(shared_ptr<ParticleGroup> pg, Parameters par)
    : Interactor(pg, "IBM::Poisson"), epsilon(par.epsilon), box(par.box),
      split(par.split), gw(par.gw), tolerance(par.tolerance) {
  double h;
  double farFieldGaussianWidth = par.gw;
  if (par.split > 0)
    farFieldGaussianWidth =
        sqrt(par.gw * par.gw + 1.0 / (4.0 * par.split * par.split));
  if (par.upsampling > 0)
    h = 1.0 / par.upsampling;
  else
    h = (1.3 - std::min((-log10(par.tolerance)) / 10.0, 0.9)) *
        farFieldGaussianWidth;
  ...
  this->kernel =
      std::make_shared<Kernel>(par.tolerance, farFieldGaussianWidth, h);
  if (kernel->support > grid.cellDim.x / 2 - 1) {
    sys->log<System::ERROR>("[Poisson] Kernel support (%d) is too large for "
                            "this configuration (max is %d), try increasing "
                            "splitting parameter or decrasing tolerance",
                            kernel->support, grid.cellDim.x / 2 - 1);
  ...
  if (par.split > 0) {
    ...
    if (nearFieldCutOff > box.boxSize.x / 2.0) {
      sys->log<System::ERROR>("[Poisson] Near field cut off is too large, "
                              "increase splitting parameter.");
  ...
  sys->log<System::MESSAGE>("[Poisson] cells: %d %d %d", grid.cellDim.x,
                            grid.cellDim.y, grid.cellDim.z);
  sys->log<System::MESSAGE>("[Poisson] box size: %g %g %g", box.boxSize.x,
  ...
  if (par.split > 0) {
    ...
    sys->log<System::MESSAGE>(
        "[Poisson] Far field width: %g, (%g times original width)",
        farFieldGaussianWidth,
        1 / par.gw * sqrt(1 / (4 * par.split * par.split) + par.gw * par.gw));
    sys->log<System::MESSAGE>("[Poisson] Near field cut off: %g",
                              nearFieldCutOff);
  }
  initCuFFT();
  if (par.split > 0) {
    ...
    sys->log<System::MESSAGE>("[Poisson] Elements in near field table: %d",
                              Ntable);
...
void Poisson::initCuFFT() {
  ...
  CufftSafeCall(cufftMakePlan3d(cufft_plan_forward, cdtmp.x, cdtmp.y, cdtmp.z,
                                ...
                                &cufftWorkSizef));
  sys->log<System::DEBUG>("[BDHI::Poisson] cuFFT grid size: %d %d %d", cdtmp.x,
                          cdtmp.y, cdtmp.z);
  // Force and energy in a single transform
  /*I want to make four 3D FFTs, each one using one of the three interleaved
   * coordinates*/
  CufftSafeCall(cufftMakePlanMany(
      ...
      4, 1, // Each element separated by four others fx0 fy0 fz0 e0 fx1 fy1 fz1
            // e1...
      &oembed.x, 4, 1, CUFFT_Complex2Real<real>::value, 4, &cufftWorkSizei));
  size_t cufftWorkSize = std::max(cufftWorkSizef, cufftWorkSizei);
  size_t free_mem, total_mem;
  CudaSafeCall(cudaMemGetInfo(&free_mem, &total_mem));
  sys->log<System::DEBUG>("[BDHI::Poisson] Necessary work space for cuFFT: %s, "
                          "available: %s, total: %s",
  ...
  try {
    ...
    sys->log<System::ERROR>("[BDHI::Poisson] Not enough memory in device to "
...
struct NearFieldEnergyTransverser {
  ...
  NearFieldEnergyTransverser(real *energy_ptr, real *charge,
                             TabulatedFunction<real> gf, Box box)
...
struct NearFieldForceTransverser {
  ...
  NearFieldForceTransverser(real4 *force_ptr, real *charge,
                            TabulatedFunction<real> gff, Box box)
      : force_ptr(force_ptr), greensFunctionField(gff), charge(charge),
        box(box) {}
  ...
  __device__ returnInfo compute(const real4 &pi, const real4 &pj, real chargei,
                                real chargej) const {
    real3 rij = box.apply_pbc(make_real3(pj) - make_real3(pi));
    real r2 = dot(rij, rij);
    real r = sqrt(r2);
  ...
  __device__ void accumulate(returnInfo &total,
                             const returnInfo &current) const {
    total += current;
  }
  __device__ void set(uint pi, const returnInfo &total) const {
    force_ptr[pi] += make_real4(total);
  }
...
struct NearFieldFieldPotentialTransverser {
  ...
  NearFieldFieldPotentialTransverser(real4 *field_potential_ptr, real *charge,
                                     TabulatedFunction<real> gf,
                                     TabulatedFunction<real> gff, Box box)
      : field_potential_ptr(field_potential_ptr), charge(charge),
        greensFunction(gf), greensFunctionField(gff), box(box) {}

  inline __device__ returnInfo zero() const { return make_real4(0, 0, 0, 0); }
...
void Poisson::farField(cudaStream_t st, real4 *fieldPotentialAtParticles) {
  ...
  int3 n = grid.cellDim;
  auto gridCharges =
      Poisson_ns::allocateTemporaryArray<real>(2 * (n.x / 2 + 1) * n.y * n.z);
  auto gridFieldPotentialFourier =
      Poisson_ns::allocateTemporaryArray<cufftComplex4>((n.x / 2 + 1) * n.y *
                                                        n.z);
  thrust::fill(thrust::cuda::par.on(st), gridCharges.get(),
               gridCharges.get() + 2 * (n.x / 2 + 1) * n.y * n.z, 0);
  spreadCharges(gridCharges.get(), st);
  forwardTransformCharge(gridCharges.get(), (cufftComplex *)gridCharges.get(),
                         st);
  convolveFourier((cufftComplex *)gridCharges.get(),
                  gridFieldPotentialFourier.get(), st);
  gridCharges.reset();
  auto gridFieldPotential =
      Poisson_ns::allocateTemporaryArray<real4>(2 * (n.x / 2 + 1) * n.y * n.z);
...
void Poisson::nearFieldFieldPotential(cudaStream_t st,
                                      real4 *fieldPotentialAtParticles) {
  if (split > 0) {
    ...
    nl->update(box, nearFieldCutOff, st);
    auto charge = pd->getCharge(access::location::gpu, access::mode::read);
    auto tr = Poisson_ns::NearFieldFieldPotentialTransverser(
        fieldPotentialAtParticles, charge.begin(),
        *nearFieldPotentialGreensFunction, *nearFieldGreensFunction, box);
    nl->transverseList(tr, st);
  }
}
...
void Poisson::nearFieldEnergy(cudaStream_t st) {
  if (split > 0) {
    ...
    auto tr = Poisson_ns::NearFieldEnergyTransverser(
        energy.begin(), charge.begin(), *nearFieldPotentialGreensFunction, box);
    nl->transverseList(tr, st);
  }
}
...
inline __device__ vec3 cellToWaveNumber(const int3 &cell, const int3 &cellDim,
  ...
  if (cell.x >= (cellDim.x / 2 + 1))
    k.x -= real(cellDim.x) * pi2invL.x;
  if (cell.y >= (cellDim.y / 2 + 1))
    k.y -= real(cellDim.y) * pi2invL.y;
  if (cell.z >= (cellDim.z / 2 + 1))
...
__global__ void
chargeFourier2FieldAndPotential(cufftComplex *gridCharges,
                                cufftComplex4 *gridFieldPotential, real epsilon,
                                Grid grid) {
  int3 cell;
  ...
  const int i_icell =
      cell.x + (cell.y + cell.z * grid.cellDim.y) * (grid.cellDim.x / 2 + 1);
  ...
  if (not nyquist) {
    ...
    fieldPotential.x.x = k.x * fk.y * B;
    fieldPotential.x.y = -k.x * fk.x * B;
    fieldPotential.y.x = k.y * fk.y * B;
    fieldPotential.y.y = -k.y * fk.x * B;
    fieldPotential.z.x = k.z * fk.y * B;
    fieldPotential.z.y = -k.z * fk.x * B;
...
void Poisson::forwardTransformCharge(real *gridCharges,
  ...
  CufftSafeCall(cufftExecReal2Complex<real>(cufft_plan_forward, gridCharges,
                                            gridChargesFourier));
}

void Poisson::convolveFourier(cufftComplex *gridChargesFourier,
  ...
  NblocksCells.x =
      (ncellsx / NthreadsCells.x + ((ncellsx % NthreadsCells.x) ? 1 : 0));
  ...
  Poisson_ns::
      chargeFourier2FieldAndPotential<<<NblocksCells, NthreadsCells, 0, st>>>(
          gridChargesFourier, gridFieldPotentialFourier, epsilon, grid);
  CudaCheckError();
}

void Poisson::inverseTransform(cufftComplex *gridFieldPotentialFourier,
  ...
  CufftSafeCall(cufftExecComplex2Real<real>(
      cufft_plan_inverse, gridFieldPotentialFourier, gridFieldPotential));
  CudaCheckError();
}
...
struct UnZip2Real4 {
  ...
  __device__ __host__ void operator+=(real4 fande) const {
    if (force)
      force[i] += charges[i] * make_real4(fande.x, fande.y, fande.z, 0);
    if (energy)
      energy[i] += charges[i] * fande.w;
  }
};
...
void Poisson::interpolateFields(real4 *gridFieldPotential, cudaStream_t st) {
  ...
  auto f_tr =
      Poisson_ns::UnZip2Real4(charge.begin(), forces.begin(), energy.begin());
  static_assert(
      std::is_same<real4,
                   std::iterator_traits<decltype(f_tr)>::value_type>::value,
      "f_tr is not a real4 iterator");
  int3 n = grid.cellDim;
  IBM<Kernel> ibm(kernel, grid,
                  IBM_ns::LinearIndex3D(2 * (n.x / 2 + 1), n.y, n.z));
