/* Raul P. Pelaez 2019. Linear Bounding Volume Hierarchy neighbour list.
   ...
   This neighbour list works by partitioning the system by particles instead of
partitioning space (as CellList does). This means that it handles better systems
with great cut off or density disparities.


Here is a little break down of the algorithm, although it is beautifully
explained in the references.

   1-Sorts the particles by assigning a hash to each particle such that close
particles of the same type tend to be close in memory(based on a Z-order curve
and the types). Particles are sorted by type and then by Z-order hash

   2-Partitions the sorted hashes in a binary tree structure following Karra's
algorithm [2]. The special hashing ensures that a certain node exists in the
tree that is the father of all particles of a certain type. And which has no
leaf of a different type under it. This allows to construct a single tree and
then find the root nodes of every type subtree. Which scales great with the
number of types. As opposed to generate an entirely new tree for every type as
in [1], [4].

   3-Assigns an Axis Aligned Bounding Box (AABB) to each node of the tree that
joins the AABBs of the nodes below it (with the particles, AKA leaf nodes, being
at the innermost level). This is done using Karra's algorithm in [2]. The AABBs
are stored in a "quantized" manner that allows to store a node in a single int4,
which increases traversal time [4]. The joining of boxes stops at the root of
every type subtree.

   4-The neighbours of a given particle are found by traversing the AABB
subtrees of every type. The traversal is done with a top-down approach, each
particle starts at the root of a given subtree. If the particle overlaps the
node with a given cut off, the algorithm goes to the next child node, otherwise
it skips to the next node/periodic image/tree. The algorithm is detailed in [1]
and [3]. For a given particle, overlap with the 27 (in 3D) periodic images of
the current subtree is computed before traversal of a tree and encoded in a
single integer to reduce divergence (except the main box, which is traversed
first by default) (see [4]). This process is repeated for every type subtree.

   References:
   [1] Efficient neighbor list calculation for molecular simulation of colloidal
systems using graphics processing units. Michael P. Howard, et.al. 2014.
https://doi.org/10.1016/j.cpc.2016.02.003 [2] Maximizing Parallelism in the
...
https://doi.org/10.1016/j.commatsci.2019.04.004
   ...
   100- Handle 2D case
   100- Find a way to inform neighbour lists of cut-offs per type pair.
Currently kind of hackish. 90- NeighbourContainer is twice as slow as
transverseLBVHList, this should be improved. 80- Better type subtree heuristics
(currently every type spawns a subtree)
 */
#ifndef LBVH_CUH
#define LBVH_CUH
...
struct alignas(int4) QuantizedAABB {
  ...
  __host__ __device__ int pack(int3 bin) {
    return ((bin.x) << 20) + ((bin.y) << 10) + (bin.z);
  }
  ...
  // True if the minimum distance between a sphere of radius r centered at c and
  // this box is less or equal than r.
...
private:
  ...
  __host__ __device__ int3 unpack(int h) {
    return make_int3((h >> 20) & 0x3ffu, (h >> 10) & 0x3ffu, (h) & 0x3ffu);
  }
};
...
struct alignas(int4) AABB {
  ...
  __host__ __device__ void setlo(real3 low, real3 L) { lo = low; }
  __host__ __device__ void sethi(real3 high, real3 L) { hi = high; }
...
public:
  __device__ __host__ Delta(const uint *keys, int size, int i)
      : keys(keys), size(size), ki(keys[i]), i(i) {}
...
// Given a root node, this function returns an integer encoding which of the 27
// periodic images the position pib
//  overlaps.
...
__device__ int treeFlags(AABB root, real rc2, LBVHList nl, real3 pib) {
  int flag = 0;
  {
    ...
    fori(0, 27) {
      ...
      origin.z = (i / 9 - 1) * L.z;
      if (!(origin.x == 0 and origin.y == 0 and origin.z == 0) and
          root.overlaps(pib + origin, rc2, L)) {
        flag |= 1 << i;
      }
    }
  }
...
__global__ void transverseLBVHList(
    Transverser tr,
    ParticleGroup::IndexIterator
        globalIndex, // Transforms a group index into a global index
    LBVHList nl, int numberParticles) {
  ...
  for (int treecount = 0; treecount < nl.Ntrees; treecount++) {
    ...
    do { // For all overlapping periodic images
      ...
      // When the tree is processed, go to next image or fo to next tree if none
      // remain.
      if (periodicFlags != 0) {
        const int next_image =
            __ffs(periodicFlags) - 1; // least significant non zero bit
...
struct MortonPlusTypeHash {
  ...
  MortonPlusTypeHash(Grid grid, const real4 *pos) : grid(grid), pos(pos) {
    // The last bit set by the morton hash, there are 32-lastMortonBit bits
    // available to encode types.
  ...
  static inline __host__ __device__ uint interleave3(const int3 &cell) {
    return encodeMorton(cell.x) | (encodeMorton(cell.y) << 1) |
           (encodeMorton(cell.z) << 2);
  }
  ...
  int getMaxNumberTypes() { return (1 << (32 - lastMortonBit)) - 1; }
...
inline __device__ int2 findRange(int i, Delta delta_i) {
  ...
  int deltamin; // The common prefix between the hashes of key i and its leaf
                // sibiling
  ...
  int lmax = 2;
  while (delta_i(i + lmax * d) > deltamin)
    lmax = lmax << 1;
...
// Find the index of the primitive between first and last with the first
// different bit.
...
inline __device__ int findSplit(int first, int last, Delta delta_i) {
  // Length of the longest common prefix between the first and last keys in the
  // range
...
__global__ void genHierarchy(Node *nodes, int *lefts, const uint *sortedHashes,
                             int typeEncodeFirstBit, int numberParticles) {
  const int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= numberParticles - 1)
    return;

  // For a given particle delta_i(j) computes the length of the longest common
  // bit prefix between
  ...
  // split is the left child node, which is a leaf if the split corresponds to
  // the first primitive of the range. Similar with the right child
  const int left = split + (split == min(first, last) ? leafOffset : 0);
  const int right =
      split + 1 + (split + 1 == max(first, last) ? leafOffset : 0);
  ...
  nodes[left] = {.parent = parent, .sibiling = right};

  nodes[right] = {.parent = parent, .sibiling = -left};

  lefts[i] = left;

  if (i == 0) {
    nodes[0] = {.parent = -1, .sibiling = -1};
  }
}

// Find the root of every type subtree. Which are the innermost nodes with
// parent = -2
__global__ void findRoots(const Node *nodes, int *treeRoots,
                          const uint *sortedHashes, int lastMortonBit,
                          int numberParticles) {
...
__global__ void genAABB(const Node *nodes,
                        ...
                        float4 *uncompressedBoxes, int *visitCount, Box box,
                        const real4 *sortPos, int numberParticles) {
  const int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= numberParticles)
    return;
  ...
  int parent = nodes[node].parent;
  while (parent >= 0) {
    { // Only the second thread to arrive to a given node actually processes it,
      // this
    ...
    // Join the aabb of my sibiling with mine and store them in compressed
    // format
...
struct Bubble {
  template <class AABB> __device__ AABB operator()(AABB a, AABB b) {
    auto res = a.bubble(b);
    return res;
  }
};
...
// The rope is the next node to process after a particular branch is processed.
// See [3]
template <class AABB>
__global__ void computeRopes(const Node *nodes, const int *lefts,
                             const int *treeRoots, int Ntypes, AABB *aabb,
                             int numberParticles) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= 2 * numberParticles - 1)
    return;
  ...
  int rope = -1;
  {
    ...
    while (rope < 0 and node > 0) {
      ...
      if (nodes[node].parent == -2) {
        rope = -1;
        break;
      } //-nextTree(node, treeRoots, Ntypes); break;}
...
template <class IT, class T> __global__ void storeValue(IT it, T *it2) {
  *it2 = *it;
}
...
private:
  ...
  // If true, the next issued neighbour list update will always result in a
  // reconstruction of the list.
  bool force_next_update = true;
  bool rebuildNlist;

  // The box and cut off used in the current state of the cell list/neighbour
  // list
  Box currentBox;
  ...
  void handleNumParticlesChanged(int Nnew) {
    System::log<System::DEBUG>(
        "[LBVHList] Number particles changed signal handled.");
    force_next_update = true;
  }
  void handlePosWriteRequested() {
    System::log<System::DEBUG1>(
        "[LBVHList] Issuing a list update after positions were written to.");
    force_next_update = true;
  }
  ...
  int countParticleTypes(cudaStream_t st) {
    ...
    auto maxElement =
        thrust::max_element(thrust::cuda::par(alloc).on(st), groupPos,
                            groupPos + numberParticles, LBVH_ns::CompareType());

    auto temp = allocate_temporary_vector<real4>(1);
    LBVH_ns::storeValue<<<1, 1, 0, st>>>(maxElement, temp.get());
    real4 h_temp;
    CudaSafeCall(
        cudaMemcpy(&h_temp, temp.get(), sizeof(real4), cudaMemcpyDeviceToHost));
  ...
  int sortParticlesWithMortonPlusTypesHash(int Ntypes, cudaStream_t st) {
    ...
    // Choose a morton grid with a number of cells that allows to encode all the
    ...
    auto pd = pg->getParticleData();
    auto pos = pd->getPos(access::location::gpu, access::mode::read);
    const real maxL = std::max(
        {currentBox.boxSize.x, currentBox.boxSize.y, currentBox.boxSize.z});
    do {
    ...
    ps.updateOrderWithCustomHash(hashIterator, numberParticles, maxHash, st);
    ps.applyCurrentOrder(pos.begin(), sortPos.begin(), numberParticles, st);
    return lastMortonBit;
  }

  void generateHierarchy(Node *d_nodes, int *d_lefts, int lastMortonBit,
                         cudaStream_t st) {
    int numberParticles = sortPos.size();
    int Nthreads = 128;
    int Nblocks = (numberParticles - 1) / Nthreads +
                  (((numberParticles - 1) % Nthreads) ? 1 : 0);
    auto mhsh = ps.getSortedHashes();
    LBVH_ns::genHierarchy<<<Nblocks, Nthreads, 0, st>>>(
        d_nodes, d_lefts, mhsh, lastMortonBit, numberParticles);
    CudaCheckError();
    System::log<System::DEBUG3>("[LBVHList] Find roots");
    LBVH_ns::findRoots<<<Nblocks, Nthreads, 0, st>>>(
        d_nodes, thrust::raw_pointer_cast(treeRoots.data()), mhsh,
        lastMortonBit, numberParticles);
    CudaCheckError();
  }
  ...
  void generateAABB(Node *d_nodes, cudaStream_t st) {
    ...
    {
      // AABB of each node in uncompressed format, real4 for faster memory
      // access.
      auto uncompBoxes = allocate_temporary_vector<real4>(2 * NtotalNodes);
      int Nthreads = 128;
      int Nblocks =
          NInternalNodes / Nthreads + ((NInternalNodes % Nthreads) ? 1 : 0);
      LBVH_ns::genAABB<<<Nblocks, Nthreads, 0, st>>>(
          d_nodes, d_aabbs, uncompBoxes.get(), visitCount.get(), currentBox,
          thrust::raw_pointer_cast(sortPos.data()), numberParticles);
      CudaCheckError();
      if (treeRoots.size() > 1) {
        auto tr =
            thrust::make_permutation_iterator(aabbs.begin(), treeRoots.begin());
        auto alloc = System::getTemporaryDeviceAllocator<char>();
        AABB root =
            thrust::reduce(thrust::cuda::par(alloc).on(st), tr,
                           tr + treeRoots.size(), AABB(), LBVH_ns::Bubble());
        aabbs[0] = root;
        CudaCheckError();
      }
    }
...
public:
  LBVHList(shared_ptr<ParticleData> pd)
      : LBVHList(std::make_shared<ParticleGroup>(pd)) {}

  LBVHList(shared_ptr<ParticleGroup> pg) : pg(pg) {
    System::log<System::MESSAGE>("[LBVHList] Created");
    auto pd = pg->getParticleData();
    pd->getNumParticlesChangedSignal()->connect(
        [this](int Nnew) { this->handleNumParticlesChanged(Nnew); });
    pd->getPosWriteRequestedSignal()->connect(
        [this]() { this->handlePosWriteRequested(); });
  }
  ...
  // Use a transverser to transverse the tree structure directly (without
  // constructing a neighbour list)
  template <class Transverser>
  void transverseList(Transverser &tr, cudaStream_t st = 0) {
    int numberParticles = pg->getNumberParticles();
    System::log<System::DEBUG2>("[LBVHList] Transversing LBVH List with %s",
                                type_name<Transverser>().c_str());
    int Nthreads = 128;
    int Nblocks =
        numberParticles / Nthreads + ((numberParticles % Nthreads) ? 1 : 0);
    auto globalIndex = pg->getIndexIterator(access::location::gpu);
    size_t shMemorySize =
        SFINAE::SharedMemorySizeDelegator<Transverser>().getSharedMemorySize(
            tr);
    auto nl = this->getLBVHList();
    LBVH_ns::transverseLBVHList<<<Nblocks, Nthreads, shMemorySize, st>>>(
        tr, globalIndex, nl, numberParticles);
    CudaCheckError();
  }
  ...
  void transverseListWithNeighbourList(Transverser &tr, cudaStream_t st = 0) {
    ...
    System::log<System::CRITICAL>(
        "[LBVHList] The transverseListWithNeighbourList capability is not "
        "available yet");
    CudaCheckError();
  }
  ...
  void update(Box box, real cutOff = 0, cudaStream_t st = 0) {
    if (this->needsRebuild(box) == false)
      return;
    ...
    { // Compute traversal ropes between nodes. see [3]
      int Nthreads = 128;
      int Nblocks =
          (NtotalNodes) / Nthreads + (((NtotalNodes) % Nthreads) ? 1 : 0);
      LBVH_ns::computeRopes<<<Nblocks, Nthreads, 0, st>>>(
          nodes.get(), lefts.get(), thrust::raw_pointer_cast(treeRoots.data()),
          Ntypes, thrust::raw_pointer_cast(aabbs.data()), numberParticles);
      CudaCheckError();
    }
...
#if 0 // This code is just dectivated for now, it is twice as slow than
      // transverseLBVHList and I do not know how to
