/*Raul P. Pelaez 2020. Verlet List UAMMD interface
  A NeighbourList can compute and provide for each particle a list of particle
indices that are closer than a certain distance to it.
  ...
  3. Any additional mechanism own to the particular instance (a.i. you can get
the cell list binning structure with CellList)

  Methods 1. and 2. are tipically the fastest ones and ensure the code will work
for any UAMMD neighbour list.
  ...
  -Provide a "transverse" method that takes a Transverser and goes through every
pair of particles with it (See NBodyForces.cuh or RadialPotential.cuh) -Provide
a NeighbourContainer able to provide, for each particle, a forward iterator with
its neighbours.*

  *Note that this does not imply that a neighbour list is constructed, just that
the iterator is able to provide the neighbours somehow "as is".
...
USAGE:
 ...
 //Get a NeighbourContainer (Method 2.). This is equivalent to Method 1 (see
transverseList kernel), but instead of having to provide a Transverser you can
use this structure to get iterators to the neighbours of each particle (as the
aforementioned kernel does) and process them manually. This allows for more
versatility.
 //See examples/NeighbourListIterator.cu for an example
 auto nc = cl->getNeighbourContainer();

 //Get a Verlet list to use manually, which provides a spatial binning and a
list with the bin of each particle (Method 3.). auto verlet_list_data =
cl->getVerletList();

For using the Verlet list manually using getVerletList:
  The list for a certain particle i starts at particleOffset[i], after
numberNeighbours[i], the neighbour list for particle i contains undefined data.
i is the index of a particle referred to its group (particle id if the group
contains all particles).

See PairForces.cu or examples/NeighbourListIterator.cu for examples on how to
use a NL.
...
public:
  ...
  VerletList(shared_ptr<ParticleGroup> pg) : pg(pg) {
    ...
    posWriteConnection = pd->getPosWriteRequestedSignal()->connect(
  ...
  void update(Box box, real3 cutOff, cudaStream_t st = 0) {
    if (cutOff.x != cutOff.y or cutOff.x != cutOff.z) {
      System::log<System::ERROR>("[VerletList] Cannot work with a different "
  ...
  void transverseList(Transverser &tr, cudaStream_t st = 0) {
    System::log<System::DEBUG2>(
        "[VerletList] Transversing Neighbour List with %s",
    ...
    size_t shMemorySize =
        SFINAE::SharedMemorySizeDelegator<Transverser>().getSharedMemorySize(
            tr);
    NeighbourList_ns::transverseWithNeighbourContainer<<<Nblocks, Nthreads,
                                                         shMemorySize, st>>>(
        tr, globalIndex, this->getNeighbourContainer(), numberParticles);
    CudaCheckError();
  }

  VerletListBase::VerletListData getVerletList() { return nl.getVerletList(); }
...
private:
  void handlePosWriteRequested() {
    System::log<System::DEBUG1>(
        "[VerletList] Issuing a list update after positions were written to.");
    forceNextUpdate = true;
  }

  void handleReorder() {
    System::log<System::DEBUG1>(
        "[VerletList] Issuing a list update after a reorder.");
    forceNextUpdate = true;
    nl.forceNextUpdate();
  }
