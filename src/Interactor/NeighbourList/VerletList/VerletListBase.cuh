/*Raul P. Pelaez 2020. Verlet List base implementation.
  ...
  A Verlet neighbour list, keeps record of the neighbours of each particle. Each
time it is updated with a given cut off distance VerletListBase will compute the
neighbours of each particle within a distance a little higher than requested.
Subsequent calls to update will only incur the rebuild of this list when a
particle has traveled a certain distance related to this extra cut off.
...
nl.update(thrust::raw_pointer_cast(some_positions), numberPartinles, someBox,
someCutOff);
...
//Get the number of steps since last update (can be used to optimize the extra
cut off). nl.getNumberOfStepsSinceLastUpdate();

See VerletList.cuh for a UAMMD compatible interface to this utility.
Implementation details:

  VerletListBase uses BaseNeighbourList to construct a verlet list (which in
turn uses a cell list). BaseNeighbour List is equivalent to a Verlet list with
no extra cut off.
...
__global__ void checkMaximumDrift(PosIterator currentPos, real4 *storedPos,
                                  real maxDistAllowed, uint *errorFlag, Box box,
                                  int numberParticles) {
...
public:
  ...
  int getNumberOfStepsSinceLastUpdate() { return stepsSinceLastUpdate - 1; }

  void forceNextUpdate() { this->forceNextRebuild = true; }

private:
  template <class PositionIterator>
  void storeCurrentPos(PositionIterator pos, int numberParticles,
                       cudaStream_t st = 0) {
    storedPos.resize(numberParticles);
    thrust::copy(thrust::cuda::par.on(st), pos, pos + numberParticles,
                 storedPos.begin());
    CudaCheckError();
  }

  template <class PositionIterator>
  void updateSortedPositions(PositionIterator pos, int numberParticles,
                             cudaStream_t st = 0) {
    sortPos.resize(numberParticles);
    auto listData = nl.getBasicNeighbourList();
    auto pos_real4 =
        thrust::make_transform_iterator(pos, CellList_ns::detail::ToReal4());
    auto reorderIt =
        thrust::make_permutation_iterator(pos_real4, listData.groupIndex);
    thrust::copy(thrust::cuda::par.on(st), reorderIt,
                 reorderIt + numberParticles, sortPos.begin());
  }
  ...
  template <class PositionIterator>
  bool needsRebuild(PositionIterator pos, uint numberParticles, Box box,
                    real cutOff, cudaStream_t st = 0) {
    if (forceNextRebuild) {
      forceNextRebuild = false;
      return true;
    }
    if (box != currentBox or cutOff != currentCutOff or
        numberParticles != storedPos.size()) {
      return true;
    }
    if (isParticleDriftOverThreshold(pos, numberParticles, st)) {
      System::log<System::DEBUG2>(
          "[VerletList] Particle drift forced rebuild. Last one %d steps ago.",
          stepsSinceLastUpdate);
      return true;
    }
    return false;
  ...
  template <class PositionIterator>
  bool isParticleDriftOverThreshold(PositionIterator pos, int numberParticles,
                                    cudaStream_t st = 0) {
    real thresholdDistance =
        (verletRadiusMultiplier * currentCutOff - currentCutOff) / 2.0;
    ...
    VerletListBase_ns::checkMaximumDrift<<<nblocks, blockSize, 0, st>>>(
        pos, thrust::raw_pointer_cast(storedPos.data()), thresholdDistance,
        thrust::raw_pointer_cast(errorFlagsGPU.data()), currentBox,
        numberParticles);
    CudaCheckError();
    uint errorFlag = errorFlagsGPU[0];
    if (errorFlag > 0) {
      System::log<System::DEBUG2>(
          "[VerletList] Found %d particles over threshold.", errorFlag);
    }
