...
namespace detail {
struct ToReal4 {
  template <typename T> __device__ real4 operator()(const T &v) const {
    return make_real4(v);
  }
};
} // namespace detail
...
__global__ void fillCellList(InputIterator sortPos, uint *cellStart,
                             int *cellEnd, uint currentValidCell,
                             int *errorFlag, int N, Grid grid) {
...
public:
  ...
  void update(PositionIterator pos, int numberParticles, Grid in_grid,
              cudaStream_t st = 0) {
    System::log<System::DEBUG1>("[CellList] Updating list");
    if (!isGridValid(in_grid)) {
      throw std::runtime_error(
          "CellList encountered an invalid grid and/or cutoff");
    }
    this->grid = in_grid;
    System::log<System::DEBUG2>("[CellList] Using %d %d %d cells",
  ...
  // This accesor function is part of CellList only, not part of the
  // NeighbourList interface They allow to obtain a reference to the cell list
  // structures to use them outside
  struct CellListData {
    //[all particles in cell 0, all particles in cell 1,..., all particles in
    // cell ncells] cellStart[i] stores the index of the first particle in cell
    // i
    // (in internal index) cellEnd[i] stores the last particle in cell i (in
    // internal index) So the number of particles in cell i is
    // cellEnd[i]-cellStart[i]
    const uint *cellStart;
    const int *cellEnd;
    const real4 *sortPos; // Particle positions in internal index
    const int *
        groupIndex; // Transformation between internal indexes and group indexes
    Grid grid;
    uint VALID_CELL; // A value in cellStart less than VALID_CELL means the cell
                     // is empty
  };
...
private:
  bool isGridValid(Grid in_grid) {
    ...
    // if(in_grid.cellDim.x < 3 or in_grid.cellDim.y < 3 or (in_grid.cellDim.z <
    // 3 and not is2D)){
    //  	System::log<System::ERROR>("[CellList] I cannot work with less
    //  than 3 cells per dimension!");
    // 	return false;
    // }
    return true;
  }
  ...
  void resizeCellListToCurrentGrid() {
    try {
      ...
      System::log<System::ERROR>(
          "[CellList] Raised exception at cell list resize");
      throw;
    }
  }
  ...
  void updateCurrentValidCell(uint numberParticles) {
    ...
    const ullint nextStepMaximumValue =
        ullint(numberParticles) * (currentValidCell_counter + 2);
    constexpr ullint maximumStorableValue =
        ullint(std::numeric_limits<uint>::max()) - 1ull;
    const bool nextStepOverflows =
        (nextStepMaximumValue >= maximumStorableValue);
  ...
  template <class PositionIterator>
  void updateOrderAndStoreInSortPos(PositionIterator &pos, int numberParticles,
                                    cudaStream_t st) {
    ps.updateOrderByCellHash<Sorter::MortonHash>(pos, numberParticles, grid.box,
                                                 grid.cellDim, st);
    CudaCheckError();
  ...
  void fillCellList(cudaStream_t st) {
    System::log<System::DEBUG3>(
        "[CellList] fill Cell List, currentValidCell: %d", currentValidCell);
    const int numberParticles = sortPos.size();
    const int Nthreads = 512;
    const int Nblocks =
        numberParticles / Nthreads + ((numberParticles % Nthreads) ? 1 : 0);
    int *d_errorFlag = thrust::raw_pointer_cast(errorFlags.data());
    CellList_ns::fillCellList<<<Nblocks, Nthreads, 0, st>>>(
        thrust::raw_pointer_cast(sortPos.data()),
        thrust::raw_pointer_cast(cellStart.data()),
        thrust::raw_pointer_cast(cellEnd.data()), currentValidCell, d_errorFlag,
        numberParticles, grid);
#ifdef UAMMD_DEBUG
    CudaSafeCall(cudaDeviceSynchronize());
    if (d_errorFlag[0] > 0) {
      System::log<System::ERROR>(
          "[CellList] NaN positions found during construction");
