...
the cell list binning structure with CellList)
  ...
  Methods 1. and 2. are the fastest ones it the list is to be used once/twice
per construction, as both methods do not force the construction of a neighbour
list explicitly.
...
the iterator is able to provide the neighbours.
...
versatility.
 ...
 //Get a cell list to use manually, which provides a spatial binning and a list
with the bin of each particle (Method 3.) auto cell_list_data =
cl->getCellList();
...
protected:
  ...
  void handlePosWriteRequested() {
    System::log<System::DEBUG1>(
        "[CellList] Issuing a list update after positions were written to.");
  ...
  Grid createUpdateGrid(Box box, real3 cutOff) {
    ...
    if (L.x >= inf)
      L.x = maximumNumberOfCells * cutOff.x;
    if (L.y >= inf)
      L.y = maximumNumberOfCells * cutOff.y;
    if (L.z >= inf)
      L.z = maximumNumberOfCells * cutOff.z;
    Box updateBox(L);
...
public:
  ...
  CellList(shared_ptr<ParticleGroup> pg) : pg(pg) {
    ...
    posWriteConnection =
        pg->getParticleData()->getPosWriteRequestedSignal()->connect(
  ...
  void update(Box box, real3 cutOff, cudaStream_t st = 0) {
    if (needsRebuild(box, cutOff)) {
      ...
      auto pos = pg->getParticleData()->getPos(access::location::gpu,
                                               access::mode::read);
  ...
  void transverseList(Transverser &tr, cudaStream_t st = 0) {
    System::log<System::DEBUG2>("[CellList] Transversing Cell List with %s",
  ...
  CellListBase::CellListData getCellList() { return cl.getCellList(); }
...
private:
  bool needsRebuild(Box box, real3 cutOff) {
    if (force_next_update)
      return true;
    if (cutOff.x != currentCutOff.x)
      return true;
    if (cutOff.y != currentCutOff.y)
      return true;
    if (cutOff.z != currentCutOff.z)
      return true;
    if (box != currentBox)
      return true;
    return false;
