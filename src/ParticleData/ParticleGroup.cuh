...
Create a group:
  ...
  //In this case, it will result in a group with particles whose ID lies between
4 and 8
  ...
  auto pg = make_shared<ParticleGroup>(ids.begin(), ids.end(), pd, name); //From
a container with a list of particle IDs
...
//Or get a plain array with the indices directly, if it exists.
  ...
  pg->getIndicesRawPtr(access::location::gpu); //or cpu, it will be nullptr if
all particles are in the group


//You can also request an iterator that will read a ParticleData array using the
group indices directly.
//Optionally a cub load modifier can be passed, which will cause the iterator to
be read through a certain GPU cache
//This allows to iterate over the particles in a group without worrying about
groups, particle sorting, etc...
...
#include <vector>
namespace uammd {
/*Small structs that encode different ways of selecting a certain set of
  particles, i.e by type, spatial location, ID... A particle selector must have
  an isSelected method that takes a particle index (not ID) and a ParticleData
  reference. It will be called for all particles except when not needed (i.e
  with All).*/
...
public:
  All() {}
  static constexpr bool isSelected(int particleIndex,
...
public:
  IDRange(int first, int last) : firstID(first), lastID(last) {}

  bool isSelected(int particleIndex, std::shared_ptr<ParticleData> &pd) {
    int particleID =
        (pd->getId(access::cpu, access::read).raw())[particleIndex];
...
public:
  Domain(real3 origin, Box domain, Box simulationBox)
      : origin(origin), domain(domain), simulationBox(simulationBox) {}
  bool isSelected(int particleIndex, std::shared_ptr<ParticleData> &pd) {
    real3 pos =
        make_real3(pd->getPos(access::cpu, access::read).raw()[particleIndex]);
...
public:
  ...
  bool isSelected(int particleIndex, std::shared_ptr<ParticleData> &pd) {
    int type_i =
        int(pd->getPos(access::cpu, access::read).raw()[particleIndex].w);
...
// Updates the indices of the particles in a group using
// pd->getIdOrderedIndices()
__global__ void updateGroupIndices( // An array that stores the indices of the
                                    // particles in the group per id.
    ...
    const int *__restrict__ particlesIds, int numberParticles) {
...
// Asking for the raw memory is a risky bussiness, as this array may not even
// exists (i.e if all particles in the system are in the group, it might decide
// to not create it, as it would be unnecessary). In this case, you will get a
// nullptr.
class ParticleGroup {
  std::shared_ptr<ParticleData> pd;
  std::shared_ptr<System> sys;
  cudaStream_t st = 0;
  // A list of the particle indices and ids of the group (updated to current
  // order)
...
public:
  ...
  ParticleGroup(std::shared_ptr<ParticleData> pd,
                std::string name = std::string("noName"));

  // Create the group from a selector
  template <class ParticleSelector>
  ParticleGroup(ParticleSelector selector, std::shared_ptr<ParticleData> pd,
                std::string name = std::string("noName"));
  ...
  void clear() { this->numberParticles = 0; }
  // Add particles to the group via an array with ids
  void addParticlesById(access::location loc, const int *ids, int N);
  // Add particles to the group via an array with the current indices of the
  // particles in pd (faster)
  void addParticlesByCurrentIndex(access::location loc, const int *indices,
                                  int N);

  void handleReorder() {
    sys->log<System::DEBUG>(
        "[ParticleGroup] Handling reorder signal in group %s",
        this->name.c_str());
  ...
  const int *getGroupIndexMaskRawPtr(access::location loc) {
    ...
    if (this->allParticlesInGroup and updateMask) {
      ...
      std::fill(myParticlesIndexMaskCPU.begin(), myParticlesIndexMaskCPU.end(),
                1);
    ...
    if (updateMask) {
      ...
      fori(0, numberParticles) { myParticlesIndexMaskCPU[indexRaw[i]] = 1; }
  ...
  const int *getIndicesRawPtr(access::location loc) {
    if (this->allParticlesInGroup || numberParticles == 0)
      return nullptr;
  ...
  // Access the index array only if it is not a nullptr (AKA if the group does
  // not contain all particles)
  struct IndexAccess {
  ...
  using IndexIterator =
      thrust::transform_iterator<IndexAccess, thrust::counting_iterator<int>>;
  ...
  IndexIterator getIndexIterator(access::location loc) {
    return IndexIterator(thrust::make_counting_iterator<int>(0),
                         IndexAccess(this->getIndicesRawPtr(loc)));
  }

  // Access the mask array only if it is not a nullptr (AKA if the group does
  // not contain all particles) Otherwise always return true
  struct MaskAccess {
    MaskAccess(const int *masks) : masks(masks) {}

    inline __host__ __device__ bool operator()(int i) const { return masks[i]; }
  ...
  using MaskIterator =
      thrust::transform_iterator<MaskAccess, thrust::counting_iterator<int>>;

  // Get an iterator of size pd->getNumParticles() returning 1 if the particle
  // in that index is
  ...
  MaskIterator getGroupIndexMask(access::location loc) {
    return MaskIterator(thrust::make_counting_iterator<int>(0),
                        MaskAccess(this->getGroupIndexMaskRawPtr(loc)));
  }
...
public:
  // Returns an iterator that will have size pg->getNumberParticles() and will
  // iterate over the
  //  particles in the group.
  // For example, If a group contains only the particle with id=10, passing
  // pd->getPos(...).begin() to this function
  //  will return an iterator so that iterator[0] = pos[10]; and it will take
  //  into account any possible reordering of the pos array.
  template <class Iterator>
  ...
  template <class Iterator> auto getPropertyIterator(Iterator property) {
    // using value_type = typename std::iterator_traits<Iterator>::element_type;
    // static_assert(std::is_same<Iterator, typename
    // Property<value_type>::iterator>::value, 	       "You must specify a
    // location or call this function with a Property::iterator argument");
  ...
  int getNumberParticles() { return this->numberParticles; }
...
ParticleGroup::ParticleGroup(ParticleSelector selector,
  ...
  sys->log<System::MESSAGE>("[ParticleGroup] Group %s created with selector %s",
                            name.c_str(),
                            type_name<ParticleSelector>().c_str());
...
ParticleGroup::ParticleGroup(particle_selector::All selector,
  ...
  sys->log<System::MESSAGE>(
      "[ParticleGroup] Group %s created with All selector", name.c_str());
...
ParticleGroup::ParticleGroup(std::shared_ptr<ParticleData> pd, std::string name)
...
ParticleGroup::ParticleGroup(InputIterator begin, InputIterator end,
                             std::shared_ptr<ParticleData> pd, std::string name)
    : pd(pd), sys(pd->getSystem()), name(name) {
  sys->log<System::MESSAGE>("[ParticleGroup] Group %s created from ID list.",
                            name.c_str());
...
// Handle a reordering of the particles (which invalids the previous relation
...
void ParticleGroup::computeIndexList(bool forceUpdate) {
  ...
  if (this->needsIndexListUpdate || forceUpdate) { // Update only if needed
    sys->log<System::DEBUG>(
        "[ParticleGroup] Updating group %s after last particle sorting",
        name.c_str());
    ...
    int *myParticlesIndicesGPU_ptr =
        thrust::raw_pointer_cast(myParticlesIndicesGPU.data());
    int *myParticlesIdsGPU_ptr =
        thrust::raw_pointer_cast(myParticlesIdsGPU.data());
    ParticleGroup_ns::updateGroupIndices<<<Nblocks, Nthreads>>>(
        id2index, myParticlesIndicesGPU_ptr, myParticlesIdsGPU_ptr,
        numberParticles);
    this->needsIndexListUpdate = false;
    updateHostVector = true;
    sys->log<System::DEBUG1>("[ParticleGroup] Updating group %s DONE!",
                             name.c_str());
  }
}
// Add particles to the group via an array with ids
void ParticleGroup::addParticlesById(access::location loc, const int *ids,
                                     int N) {
  sys->log<System::DEBUG1>(
      "[ParticleGroup] Adding %d particles to group %s via ids!", N,
      name.c_str());
  ...
  ParticleGroup_ns::updateGroupIndices<<<Nblocks, Nthreads, 0, upSt>>>(
      id2index, myParticlesIndicesGPU_ptr + numberParticlesPrev, d_ids, N);
  CudaSafeCall(cudaStreamSynchronize(st));
}

namespace ParticleGroup_ns {
__global__ void IdsFromIndices(const int *indices, const int *index2Id,
                               int *groupParticleIds, int N) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  if (tid >= N)
    return;
...
// Add particles to the group via an array with the current indices of the
// particles in pd (faster)
void ParticleGroup::addParticlesByCurrentIndex(access::location loc,
                                               const int *indices, int N) {
  sys->log<System::DEBUG1>(
      "[ParticleGroup] Adding %d particles to group %s via indices!", N,
      name.c_str());
  if (N == 0)
    return;
  int numberParticlesPrev = numberParticles;
  ...
  CudaSafeCall(cudaMemcpyAsync(myParticlesIndicesGPU_ptr + numberParticlesPrev,
                               indices, N * sizeof(int), copyKind, st));
  ...
  ParticleGroup_ns::IdsFromIndices<<<Nblocks, Nthreads, 0, upSt>>>(
      d_indices, index2id.raw(), myParticlesIdsGPU_ptr + numberParticlesPrev,
      N);
  CudaSafeCall(cudaStreamSynchronize(st));
}
