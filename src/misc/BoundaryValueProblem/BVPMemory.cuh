...
template <class T> struct StorageHandle {
  using value_type = T;
  using pointer = T *;
  size_t offset;
  int numberElements;
...
public:
  ...
  StorageHandle<T> registerStorageRequirement(int numberElements) {
    const size_t extraAlignment =
        detail::computeExtraAlignment(currentOffset, sizeof(T));
    StorageHandle<T> stor;
    stor.offset = currentOffset + extraAlignment;
    stor.numberElements = numberElements;
    const size_t requestedAllocationSize =
        (numberCopies * numberElements) * sizeof(T);
    allocationSize += requestedAllocationSize + extraAlignment;
    currentOffset += requestedAllocationSize + extraAlignment;
    return stor;
  }

  size_t getRequestedStorageBytes() { return allocationSize; }
};
...
public:
  __host__ __device__ Contigous(int numberElements, int numberSystems,
                                int instance) {
    first = instance * numberElements;
  }

  __device__ __host__ int operator()(int i) { return first + i; };
};
...
public:
  __host__ __device__ Strided(int numberElements, int numberSystems,
                              int instance) {
    first = instance;
    stride = numberSystems;
  }

  __device__ __host__ int operator()(int i) { return first + stride * i; };
};
} // namespace layout
...
using MemoryAccessTransformIterator =
    thrust::transform_iterator<MemoryLayout, CountingIterator>;

template <class T>
using Iterator =
    thrust::permutation_iterator<typename StorageHandle<T>::pointer,
                                 MemoryAccessTransformIterator>;

template <class T>
__device__ __host__ Iterator<T> make_iterator(char *raw,
                                              const StorageHandle<T> &stor,
                                              int numberSystems, int id) {
  auto ptr =
      reinterpret_cast<typename StorageHandle<T>::pointer>(raw + stor.offset);
  auto layoutTransform = MemoryLayout(stor.numberElements, numberSystems, id);
  auto indexTransform =
      MemoryAccessTransformIterator(CountingIterator(0), layoutTransform);
...
public:
  __host__ __device__ StorageRetriever(int numberCopies, int instance,
                                       char *ptr)
      : raw(ptr), numberCopies(numberCopies), instance(instance) {}

  template <class T>
  __device__ __host__ Iterator<T>
  retrieveStorage(const StorageHandle<T> &stor) {
