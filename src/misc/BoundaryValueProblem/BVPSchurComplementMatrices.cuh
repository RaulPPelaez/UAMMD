...
public:
  ...
  real getElement(int i, int j) {
    ...
    if (std::max(i, j) >= 3) {
      if (i == j)
        return -(0.5 / i * (1 / (2.0 * i - 2) + (i < nz - 1) / (2.0 * i + 2)));
      if (i == j + 2)
        return 1.0 / (2.0 * j) * (j < nz - 2) / (2.0 * j + 2);
      if (i == j - 2)
        return 1.0 / (2.0 * j) * 1.0 / (2.0 * j - 2);
    } else if (i == j)
      return -((i > 0) / 8.0 + (i > 1) / 24.0);
    else if (j == 2 and i == 0)
      return 1.0 / 4.0;
    else if (j == 2 and i == 4)
      return 1.0 / 24.0;
  ...
  __host__ __device__ T computeSecondIntegralCoefficient(int i,
                                                         const AnIterator &a,
                                                         T c0, T d0) {
    if (i == 0)
      return c0;
    else if (i == 1)
      return d0 - real(1 / 8.0) * (a[1] - a[3]);
    else if (i == 2)
      return real(0.25) * (real(0.5) * (real(2.0) * a[i - 2] - a[i]) -
                           real(1 / 6.0) * (a[i] - a[i + 2]));
    const T di_m1 = real(0.5) / ((i - real(1.0))) * (a[i - 2] - a[i]);
    T di_p1 = T();
    if (i < nz - 1)
      di_p1 = real(0.5) / ((i + real(1.0))) *
              (a[i] - ((i < nz - 2) ? a[i + 2] : T()));
...
template <typename T> class SchurBoundaryCondition_impl {
  int nz;
  real H;
  BCRows bcs;
...
public:
  ...
  std::vector<T> computeBoundaryConditionMatrix(const TopBC &top,
                                                const BottomBC &bottom) {
...
using SchurBoundaryConditionComplex =
    SchurBoundaryCondition_impl<thrust::complex<real>>;
using SchurBoundaryConditionComplexDouble =
    SchurBoundaryCondition_impl<thrust::complex<double>>;
...
std::vector<T> computeSecondIntegralMatrix(T k, real H, int nz) {
  ...
  T kH2 = k * k * H * H;
  fori(0, nz) {
    forj(0, nz) { A[i + nz * j] = (i == j) - kH2 * sim.getElement(i, j); }
  }
  return std::move(A);
}
