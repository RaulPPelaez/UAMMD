...
ibm->spread(pos,      //An iterator with the position of the markers
            ...
            gridQuantity, //An iterator with the grid data (ibm will sum to the
existing data) grid,         //A Grid descriptor corresponding to gridQuantity
            numberMarkers,
            cudaStream);
...
ibm->gather(pos,      //An iterator with the position of the markers
            quantity, //An iterator with the quantity of each marker to gather
(ibm will sum to the existing values) gridQuantity, //An iterator with the grid
data grid,         //A Grid descriptor corresponding to gridQuantity
            //qw,  //Optional, a device functor that takes (cell, grid) and
returns the quadrature weight of cell. cellVolume() is the default.
            numberMarkers,
            cudaStream);

//The value types of the different iterators can be wildly different as long as
they are compatible, in the sense that the type of quantity*delta() must be
summable to the type of gridQuantity for spread, etc.
...
struct DefaultQuadratureWeights {
  template <class Grid>
  inline __host__ __device__ real operator()(int3 cellj,
...
struct DefaultWeightCompute {
  ...
  inline __device__ auto operator()(T1 value,
...
template <class Kernel, class Grid = uammd::Grid,
          class Index3D = IBM_ns::LinearIndex3D>
class IBM {
...
public:
  IBM(shared_ptr<Kernel> kern, Grid a_grid, Index3D cell2index)
    ...
    System::log<System::DEBUG2>("[IBM] Initialized with kernel: %s",
                                type_name<Kernel>().c_str());
  }

  IBM(shared_ptr<Kernel> kern, Grid a_grid)
      : IBM(kern, a_grid,
            Index3D(a_grid.cellDim.x, a_grid.cellDim.y, a_grid.cellDim.z)) {}

  template <bool is2D, class PosIterator, class QuantityIterator,
            class GridDataIterator,
            class WeightCompute = IBM_ns::DefaultWeightCompute>
  void spread(const PosIterator &pos, const QuantityIterator &v,
              GridDataIterator &gridData, WeightCompute &weightCompute,
    ...
    using KernelValueType =
        decltype(IBM_ns::detail::phiX(*kernel, real(), real3()));
    size_t shMemory =
        (support.x + support.y + (!is2D) * support.z) * sizeof(KernelValueType);
    IBM_ns::particles2GridD<is2D>
        <<<numberParticles, threadsPerParticle, shMemory, st>>>(
            pos, v, gridData, numberParticles, grid, cell2index, *kernel,
            weightCompute);
  }

  template <bool is2D, class PosIterator, class ResultIterator,
            class GridQuantityIterator, class QuadratureWeights,
            class WeightCompute>
  void gather(const PosIterator &pos, ResultIterator &Jq,
              const GridQuantityIterator &gridData, const QuadratureWeights &qw,
              const WeightCompute &wc, int numberParticles,
              cudaStream_t st = 0) const {
    System::log<System::DEBUG2>("[IBM] Gathering");
    int3 support = IBM_ns::detail::GetMaxSupport<Kernel>::get(*kernel);
    int numberNeighbourCells = support.x * support.y * ((is2D ? 1 : support.z));
    int threadsPerParticle =
        std::min(int(pow(2, int(std::log2(numberNeighbourCells) + 0.5))), 64);
    using KernelValueType =
        decltype(IBM_ns::detail::phiX(*kernel, real(), real3()));
    size_t shMemory =
        (support.x + support.y + (!is2D) * support.z) * sizeof(KernelValueType);
...
#define KERNEL(x)                                                              \
  if (threadsPerParticle <= x) {                                               \
    IBM_ns::callGather<x, is2D>(numberParticles, shMemory, st, pos, Jq,        \
                                gridData, numberParticles, grid, cell2index,   \
                                *kernel, wc, qw);                              \
    return;                                                                    \
  }
    KERNEL(32)
...
#undef KERNEL
  ...
  template <class... T> void gather(T... args) const {
    if (grid.cellDim.z == 1)
      gather<true>(args...);
    else
      gather<false>(args...);
  ...
  void gather(PosIterator pos, ResultIterator Jq, GridQuantityIterator gridData,
              int numberParticles, cudaStream_t st = 0) const {
  ...
  template <class... T> void spread(T... args) const {
    if (grid.cellDim.z == 1)
      spread<true>(args...);
    else
      spread<false>(args...);
  ...
  template <bool is2D, class PosIterator, class QuantityIterator,
            class GridDataIterator>
  void spread(const PosIterator &pos, const QuantityIterator &v,
              GridDataIterator &gridData, int numberParticles,
              cudaStream_t st = 0) const {
    IBM_ns::DefaultWeightCompute wc;
