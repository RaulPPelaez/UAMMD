...
public:
  ...
  ~KrylovSubspace() { CublasSafeCall(cublasDestroy(cublas_handle)); }
...
Solver::~Solver() { CublasSafeCall(cublasDestroy(cublas_handle)); }
...
real Solver::computeError(real *Bz, int N, int iter) {
  ...
  */
  ...
  /*oldBz = Bz-oldBz*/
  real a = -1.0;
  device_axpy(cublas_handle, N, &a, Bz, 1, d_oldBz, 1);
  /*yy = ||Bz_i - Bz_{i-1}||_2*/
  real yy;
  ...
  if (std::isnan(Error)) {
    throw std::runtime_error(
        "[Lanczos] Unknown error (found NaN in result guess) at iteration " +
        std::to_string(iter));
  }
  return Error;
}
