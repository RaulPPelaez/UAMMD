...
namespace uammd{
  ...
  Deserno::Deserno(shared_ptr<ParticleData> pd,
    ...
    }
    {
      ...
      bondedHarmonic = std::make_shared<BondedHarmonic>(pd, sys, params, BondedType::HarmonicPBC(par.box));
    }
    {
      BondedFENE::Parameters params;
      params.file = par.fileFENE;
      bondedFENE = std::make_shared<BondedFENE>(pd, sys, params, BondedType::FENEPBC(par.box));
    }
  ...
  real Deserno::sumEnergy(){
    return nonBonded->sumEnergy() +
      bondedHarmonic->sumEnergy() +
