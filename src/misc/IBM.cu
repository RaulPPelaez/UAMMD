...
__device__ int3 computeSupportShift(real3 pos, int3 celli, Grid grid,
  ...
  const auto distanceToLeftMostCell =
      abs(grid.distanceToCellCenter(pos, celli - P));
  const auto cellSize = grid.getCellSize(celli);
  if (cellSize.x > 0 and
      distanceToLeftMostCell.x > support.x * cellSize.x / real(2.0)) {
    P.x -= 1;
  }
  if (cellSize.y > 0 and
      distanceToLeftMostCell.y > support.y * cellSize.y / real(2.0)) {
    P.y -= 1;
  }
  if (cellSize.z > 0 and
      distanceToLeftMostCell.z > support.z * cellSize.z / real(2.0)) {
    P.z -= 1;
  }
  return P;
}
...
__device__ void fillSharedWeights(KernelValueType *weights, real3 pi,
                                  int3 support, int3 celli, int3 P, Grid &grid,
                                  Kernel &kernel) {
  auto *weightsX = &weights[0];
  const int tid = threadIdx.x;
  for (int i = tid; i < support.x; i += blockDim.x) {
    const auto cellj =
        make_int3(grid.pbc_cell_coord<0>(celli.x + i - P.x), celli.y, celli.z);
    if (cellj.x >= 0) {
  ...
  for (int i = tid; i < support.y; i += blockDim.x) {
    const auto cellj =
        make_int3(celli.x, grid.pbc_cell_coord<1>(celli.y + i - P.y), celli.z);
    if (cellj.y >= 0) {
  ...
  for (int i = tid; i < support.z; i += blockDim.x) {
    const auto cellj =
        make_int3(celli.x, celli.y, grid.pbc_cell_coord<2>(celli.z + i - P.z));
    if (cellj.z >= 0) {
...
__device__ auto computeWeightFromShared(KernelValueType *weights, int ii,
                                        int jj, int kk, int3 support) {
  return thrust::make_tuple(weights[ii], weights[support.x + jj],
                            weights[support.x + support.y + kk]);
}
} // namespace detail
...
*/
template <bool is2D, class Grid, class Index3D, class Kernel, class PosIterator,
...
__global__ void particles2GridD(const PosIterator pos,
                                ...
                                Index3D cell2index, Kernel kernel,
                                WeightCompute weightCompute) {
  const int id = blockIdx.x;
  const int tid = threadIdx.x;
  using GridQuantityType =
      typename std::iterator_traits<GridQuantityIterator>::value_type;
  using ParticleQuantityType =
      typename std::iterator_traits<ParticleQuantityIterator>::value_type;
  ...
  if (!is2D)
    numberNeighbourCells *= support.z;
  for (int i = tid; i < numberNeighbourCells; i += blockDim.x) {
    ...
    const int3 cellj =
        grid.pbc_cell(make_int3(celli.x + ii - P.x, celli.y + jj - P.y,
                                is2D ? 0 : (celli.z + kk - P.z)));
    if (cellj.x < 0 or cellj.y < 0 or cellj.z < 0)
      continue;
    if (cellj.x >= grid.cellDim.x or cellj.y >= grid.cellDim.y or
        cellj.z >= grid.cellDim.z)
      continue;
    const int jcell = cell2index(cellj);
    const auto kern =
        detail::computeWeightFromShared(weights, ii, jj, kk, support);
    const auto weight = weightCompute(vi, kern);
    atomicAdd(gridQuantity[jcell], weight);
  }
}

/*Interpolates a quantity (i.e velocity) from its values in the grid to the
  particles. J(z) q(x) = q(z) = \sum_{x\in G}{ q(x)*\delta(x-z) weight(x)} Where
  :
     ...
     - weight() is the quadrature weight of a cell. (cellsize^d in a regular
  grid) given by QuadratureWeights

  This is the discretization of an integral and thus requires quadrature weigths
  for each element. Which in a regular grid is just the cell size, h. But can in
  general be something that depends on the position.
*/
template <int TPP, bool is2D, class Grid, class InterpolationKernel,
          class ParticlePosIterator, class ParticleQuantityOutputIterator,
          class GridQuantityIterator, class WeightCompute, class Index3D,
          class QuadratureWeights>
__global__ void
grid2ParticlesDTPP(const ParticlePosIterator pos,
                   ParticleQuantityOutputIterator particleQuantity,
                   const GridQuantityIterator gridQuantity, int numberParticles,
                   Grid grid,
                   Index3D cell2index, // Index of a 3d cell in gridQuantity
                   InterpolationKernel kernel, WeightCompute weightCompute,
  ...
  using GridQuantityType =
      typename std::iterator_traits<GridQuantityIterator>::value_type;
  using ParticleQuantityType =
      typename std::iterator_traits<ParticleQuantityOutputIterator>::value_type;
...
template <int threadsPerParticle, bool is2D, class... T>
void callGather(int numberParticles, size_t shMemory, cudaStream_t st,
                T... args) {
  grid2ParticlesDTPP<threadsPerParticle, is2D>
      <<<numberParticles, threadsPerParticle, shMemory, st>>>(args...);
}
