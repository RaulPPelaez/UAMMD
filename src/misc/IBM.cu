...
#define ENABLE_PHI_IF_NOT_HAS(foo) template<class Kernel> __device__ inline SFINAE::enable_if_t<not has_phi##foo<Kernel>::value, real>
      ...
      /* Rotacional numérico, mejor en Fourier
      template<class Grid, class GridQuantityIterator>
      __device__ real3 curl(Grid grid, const GridQuantityIterator gridQuantity, int3 cell){

	int plusxcell = cell2index(grid.pbc_cell(make_int3(cell.x+1,cell.y,cell.z)));
	int minusxcell = cell2index(grid.pbc_cell(make_int3(cell.x+1,cell.y,cell.z)));
	int plusycell = cell2index(grid.pbc_cell(make_int3(cell.x,cell.y+1,cell.z)));
	int minusycell = cell2index(grid.pbc_cell(make_int3(cell.x,cell.y-1,cell.z)));
	int pluszcell = cell2index(grid.pbc_cell(make_int3(cell.x,cell.y,cell.z+1)));
	int minuszcell = cell2index(grid.pbc_cell(make_int3(cell.x,cell.y,cell.z-1)));

	real3 curlQuantity;
	curlGridQuantity.x = (gridQuantity[plusycell].z-gridQuantity[minusycell].z)/grid.cellSize.y;
	curlGridQuantity.x-= (gridQuantity[pluszcell].y+gridQuantity[minuszcell].y)grid.cellSize.z;

	curlGridQuantity.y = (gridQuantity[pluszcell].x-gridQuantity[minuszcell].x)/grid.cellSize.x;
	curlGridQuantity.y-= (gridQuantity[plusxcell].z+gridQuantity[minusxcell].z)grid.cellSize.z;

	curlGridQuantity.z = (gridQuantity[plusxcell].y-gridQuantity[minusxcell].y)/grid.cellSize.x;
	curlGridQuantity.z-= (gridQuantity[plusycell].x+gridQuantity[minusycell].x)grid.cellSize.y;

	return curlGridQuantity/2.0;
      }
      */
      template<class Grid>
	...
	for(int i = tid; i<support.z; i+=blockDim.x){
      ...
      template<class Grid, class Kernel>
      __device__ void fillSharedWeightsAndDirections(real* weights, real* directions, real3 pi, int3 support, int3 celli, int3 P,  Grid &grid, Kernel &kernel){
	real *weightsX = &weights[0];
	real *directionsX = &weights[0];
	const int tid = threadIdx.x;
	for(int i = tid; i<support.x; i+=blockDim.x){
	  const auto cellj = make_int3(grid.pbc_cell_coord<0>(celli.x + i - P.x), celli.y, celli.z);
	  const real rij = grid.distanceToCellCenter(pi, cellj).x;
	...
	real *weightsY = &weights[support.x];
	real *directionsY = &weights[support.x];
	for(int i = tid; i<support.y; i+=blockDim.x){
	  const auto cellj = make_int3(celli.x, grid.pbc_cell_coord<1>(celli.y + i -P.y), celli.z);
	  const real rij = grid.distanceToCellCenter(pi, cellj).y;
	  weightsY[i] = detail::phiY(kernel,rij);
	...
	real *weightsZ = &weights[support.x+support.y];
	real *directionsZ = &weights[support.x+support.y];
	for(int i = tid; i<support.z; i+=blockDim.x){
	  const auto cellj = make_int3(celli.x, celli.y, grid.pbc_cell_coord<2>(celli.z + i - P.z));
	  const real rij = grid.distanceToCellCenter(pi, cellj).z;
	  weightsZ[i] = detail::phiZ(kernel, rij);
	...
	return weights[ii]*weights[support.x+jj]*weights[support.x+support.y+kk];
      ...
      __device__ real3 computeWeightFromShared(real* weights, real *directions, int ii, int jj, int kk, int3 support){
	real3 kern = make_real3(directions[ii],directions[support.x+jj],directions[support.x+support.y+kk]);
	return kern*weights[ii]*weights[support.x+jj]*weights[support.x+support.y+kk];
      }
    ...
    */
    ...
    template<bool is2D, class Grid, class Index3D, class Kernel,
	     class PosIterator,
	     class ParticleQuantityIterator, class GridQuantityIterator>
    __global__ void particles2GridDAngular(const PosIterator pos,
				    const ParticleQuantityIterator  particleQuantity,
				    GridQuantityIterator  __restrict__ gridQuantity,
				    int numberParticles,
				    Grid grid,
      ...
      __shared__ int3 support;
      extern __shared__ real weights[];
      extern __shared__ real directions[];
      if(tid==0){
	pi = make_real3(pos[id]);
	vi = particleQuantity[id];
	celli = grid.getCell(pi);
	...
	if(is2D){
      ...
      __syncthreads();
      detail::fillSharedWeightsAndDirections(weights, directions, pi, support, celli, P, grid, kernel);
      __syncthreads();
      int numberNeighbourCells = support.x*support.y;
      if(!is2D)  numberNeighbourCells *= support.z;
      for(int i = tid; i<numberNeighbourCells; i+=blockDim.x){
	...
	const int3 cellj = grid.pbc_cell(make_int3(celli.x + ii - P.x, celli.y + jj - P.y, is2D?0:(celli.z + kk - P.z)));
	const int jcell = cell2index(cellj);
	const auto kern = detail::computeWeightFromShared(weights, directions, ii, jj, kk, support);
	const auto weight = -cross(vi,kern)*kernel.tau;
	if(kern)atomicAdd(&gridQuantity[jcell], weight);
      }
    }
    ...
    */
    ...
    /* 
      ...
      class Index3D,
      class QuadratureWeights>
    __global__ void grid2ParticlesDTPPAngular(const ParticlePosIterator pos,
					      ParticleQuantityOutputIterator particleQuantity,
					      const GridQuantityIterator gridQuantity,
					      int numberParticles,
					      Grid grid,
	...
	for(int i = tid; i<numberNeighbourCells; i+=blockDim.x){
	  ...
	  const real dV = qw(cellj, grid);
	  const int jcell = cell2index(cellj);
	  const real3 curlQuantity =curl(grid, gridQuantity, cellj);
	  const auto weight = curlQuantity*detail::computeWeightFromShared(weights, ii, jj, kk, support);
	  result += dV*weight;
	}
      }
      GridQuantityType total = BlockReduce(temp_storage).Sum(result);
    ...
    */
    ...
    template<int threadsPerParticle, bool is2D, class ...T> void callGatherAngular(int numberParticles, size_t shMemory, cudaStream_t st, T... args){
      grid2ParticlesDTPPAngular<threadsPerParticle, is2D><<<numberParticles, threadsPerParticle, shMemory, st>>>(args...);
    }
