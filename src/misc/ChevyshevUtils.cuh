...
struct QuadratureWeights {
  QuadratureWeights(real H, real cellSizex, real cellSizey, int nz)
      : isCopy(false) {
    CudaSafeCall(
        cudaMalloc((void **)&clencurtWeights, (nz + 1) * sizeof(real)));
    this->hxhy = cellSizex * cellSizey;
    std::vector<real> weights(nz + 1, 0);
    for (int i = 0; i < nz; i++) {
      weights[i] = 0.5 * H * clencurt(i, nz - 1);
    }
    CudaSafeCall(cudaMemcpy(clencurtWeights, weights.data(),
                            (nz + 1) * sizeof(real), cudaMemcpyHostToDevice));
  }

  // This copy constructor prevents cuda from calling the destructor after a
  // kernel call
  QuadratureWeights(const QuadratureWeights &_orig) {
    *this = _orig;
    isCopy = true;
  }
...
struct Grid {
  ...
  Grid(Box _box, int3 in_cellDim) : box(_box), cellDim(in_cellDim) {
  ...
  inline __host__ __device__ int3 getCell(const VecType &r) const {
    ...
    int cz = int((cellDim.z - 1) *
                 (acos(real(2.0) * pos_inBox.z / box.boxSize.z) / real(M_PI)));
    int3 cell = make_int3(
        (pos_inBox.x + real(0.5) * box.boxSize.x) * invCellSize.x,
        (pos_inBox.y + real(0.5) * box.boxSize.y) * invCellSize.y, cz);
    if (cell.x == cellDim.x)
      cell.x = 0;
    if (cell.y == cellDim.y)
      cell.y = 0;
  ...
  inline __host__ __device__ real3 getCellCenter(int3 cell) const {
    ...
    const real3 cellCenterPos =
        make_real3(real(-0.5) * box.boxSize.x + cellSize.x * (cell.x),
                   real(-0.5) * box.boxSize.y + cellSize.y * (cell.y), centerZ);
