/*Raul P. Pelaez 2017-2021.
  ...
  A parameter communication interface, anything that inherits from
  ParameterUpdatable can be called through update* to communicate a parameter
  change using a common interface. Parameters related with the particle data are
  communicated using ParticleData (like number of particles).

  Parameters like a simulation box or the current simulation time are updated
  through ParameterUpdatable.

  Interactors are ParameterUpdatable objects.

  If a module needs to be aware of a parameter change, it should override the
  particular virtual method. Wich will do nothing by default.

  If a module needs to delegate the ParameterBehavior to a member (i.e
  ExternalForces) it must then inherit from ParameterUpdatableDelegate and call
  setDelegate(&member). From that moment, calls to update*() will be called on
  member. This will work even when member is not ParameterUpdatable (SFINAE).
 */
...
#endif
// Add here any parameter you want along its type, after adding it here the
// function updateWHATEVER(type) will be available for all ParameterUpdatable
// modules
#define PARAMETER_LIST                                                         \
  ((TimeStep, real))((SimulationTime, real))((Box, Box))((Temperature, real))( \
...
#define DECLARE_UPDATE_FUNCTION_NAME_T(type, name)                             \
  DECLARE_UPDATE_FUNCTION_NAME_R(type, name)
#define DECLARE_UPDATE_FUNCTION_NAME_R(type, name)                             \
  virtual void update##name(type) {}
#define DECLARE_UPDATE_FUNCTION(r, data, tuple)                                \
  DECLARE_UPDATE_FUNCTION_NAME_T(PARTYPE(tuple), PARNAME(tuple))

#define DECLARE_UPDATE_FUNCTION_NAME_DELEGATE_T(type, name)                    \
  DECLARE_UPDATE_FUNCTION_NAME_DELEGATE_R(type, name)
#define DECLARE_UPDATE_FUNCTION_NAME_DELEGATE_R(type, name)                    \
  virtual void update##name(type val) {                                        \
    if (delegate)                                                              \
      delegate->update##name(val);                                             \
  }
...
template <class T, bool isParameterUpdatable =
                       std::is_base_of<ParameterUpdatable, T>::value>
