/*Raul P. Pelaez 2019. Allocators, this file defines some memory_resources and
 * allocators compatible with c++17's std::pmr system. Thrust also provides
 * similar ones with very recent versions (1.8.1+) but atm they are quite
 * experimental and have some downsides. In theory these classes could be
 * reduced to some aliases when thrust provides stable memory_resource options
 * and/or c++17 pmr is available.
 */
...
template <class MR>
struct pool_memory_resource_adaptor
    : public detail::memory_resource<typename MR::pointer> {
private:
...
// pointers
template <class T, class MR = detail::memory_resource<void *>,
...
// thrust versions are not reliable atm, std::pmr is C++17 which CUDA 10.1 does
// not support yet and is not thrust::cuda compatible (thrust expects device
// allocators to return thurst::cuda::pointers) template<class T> using
// memory_resource = thrust::mr::memory_resource<T>; template<class T> using
// polymorphic_resource_adaptor = thrust::mr::polymorphic_adaptor_resource<T>;
// template<typename T, class MR> using allocator = thrust::mr::allocator<T,
// MR>; template<class T> using polymorphic_allocator =
// thrust::mr::polymorphic_allocator<T>;
