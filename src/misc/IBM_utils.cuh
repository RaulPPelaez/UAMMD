...
#define ENABLE_PHI_IF_HAS(foo) template<class Kernel> __device__ inline SFINAE::enable_if_t<has_phi##foo<Kernel>::value, decltype(std::declval<Kernel>().phiX(real(), real3()))>
#define ENABLE_PHI_IF_NOT_HAS(foo) template<class Kernel> __device__ inline SFINAE::enable_if_t<not has_phi##foo<Kernel>::value, decltype(std::declval<Kernel>().phi(real(), real3()))>
