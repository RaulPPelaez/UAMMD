...
This integrator solves the Brownian Dynamics with Hydrodynamic interactions
equation where particles are restricted to move in a 2D enviroment. The
hydrodynamic kernel is free to choose as a kernel argument, mainly: 1- True2D ->
Pure 2D hydrodynamics, using the 2D FCM kernel (just 3D with z=0) 2- Quasi2D ->
3D hydrodynamics acting on 2D particles. 3D FCM kernel integrated in z. 3-
Saffman -> Saffman hydrodynamic kernel.

Any hydrodynamic kernel can be provided as a functor with the following
prototype (i.e for True2D):

struct True2D{
//This function returns the relation between the gaussian kernel variance and
...
//This function returns f_k and g_k in the Fourier representation of the
hydrodynamic Green function:
// \hat{G}_\vec{k} = 1/\eta [ g_k(k*a)  \vec{k_perp}\dyadic\vec{k_perp} +
f_k(k*a\vec{k}\dyadic\vec{k} ]
//See eq.25 in [1].
...
USAGE:
    ...
    //par.cells = make_int2(cells_x, cells_y); //Cell dimensions control
accuracy, can be forced par.tolerance = 1e-5; //Controls accuracy throguh the
number of cells par.box = box; //boxSize.z will be ignored if != 0
    //par.hydroKernel = std::make_shared<Kernel>(...) //Optionally you can pass
an instance of the hydrodynamic kernel to be used. auto q2D =
make_shared<Hydro2D>(pd, pg, sys, par);
...
References:
[1] Hydrodynamic fluctuations in quasi-two dimensional diffusion. Raul P.
Pelaez, et. al. JSTAT 2018
*/
...
struct Quasi2D {
  ...
  inline __device__ real2 operator()(real k2, real a) {
    ...
    const real fk =
        real(0.5) * invk3 *
        (erfc(kp) * (real(0.5) + kp * kp) * exp(kp * kp) - kp * inv_sqrtpi);
...
template <int direction> struct GaussianThermalDrift {
  int support;
  ...
  inline __device__ real phiZ(real r, real3 pos) const { return real(1.0); }
...
template <class HydroKernel> class BDHI2D : public Integrator {
public:
