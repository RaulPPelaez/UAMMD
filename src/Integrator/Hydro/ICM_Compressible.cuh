...
par.temperature = 0;
//par.hydrodynamicRadius = 1.0; //Particle hydrodynamic radius (used to
determine the number of fluid cells) par.cellDim = {32,32,32}; //Number of fluid
cells, if set the hydrodynamicRadius is ignored par.dt = 0.1; par.boxSize =
{32,32,32}; //Simulation domain par.seed = 1234; //0 will take a value from the
UAMMD generator
...
1- I want to fiddle with the boundary conditions:
    -Check the function pbc_cells and fetchScalar in file
ICM_Compressible/utils.cuh, which handles what happens when trying to access the
information of a cell -You can also influence the solver itself (for instance to
define special rules for the surfaces of the domain) in the functions of the
file FluidSolver.cuh.

2- I want to chenge the spreading kernel:
    -Change the line "using Kernel" below to the type of your kernel. You might
also have to change the initialization in the spreading and interpolation
functions in ICM_Compressible.cu. You will also have to change the relation
between the hydrodynamic radius and the number of fluid cells, do this in the
ICM_Compressible constructor.

3- I want to add some special fluid forcing:
    -The function addFluidExternalForcing in ICM_Compressible.cu was created for
this.
...
References:
   ...
   [1] Inertial coupling for point particle fluctuating hydrodynamics. F. Balboa
et. al. 2013 [2] STAGGERED SCHEMES FOR FLUCTUATING HYDRODYNAMICS. F. Balboa et.
al. 2012 [3] Ph.D. manuscript. Florencio Balboa.
 */
...
// Default walled behavior is no walls, which translates into a periodic in the
// three directions. Note that the walls can be ParameterUpdatable
...
public:
  ...
  __host__ __device__ static constexpr bool isEnabled() { return false; }

  // Applies the boundary conditions at the bottom z wall for the fluid
  __device__ static void applyBoundaryConditionZBottom(FluidPointers fluid,
                                                       int3 ghostCell, int3 n) {
    const int ighost =
  ...
  __device__ static void applyBoundaryConditionZTop(FluidPointers fluid,
                                                    int3 ghostCell, int3 n) {
    const int ighost =
        ghostCell.x + (ghostCell.y + ghostCell.z * (n.y + 2)) * (n.x + 2);
...
template <class Walls> class ICM_Compressible_impl : public Integrator {
public:
  template <class T> using cached_vector = icm_compressible::cached_vector<T>;
  ...
  ICM_Compressible_impl(std::shared_ptr<ParticleData> pd, Parameters par)
    ...
    if (par.walls) {
      System::log<System::MESSAGE>(
          "[ICM_Compressible] Using walls provided in parameters");
  ...
  // Returns the number of cells in the fluid grid in each direction without
  // including ghost cells
  int3 getGridSize() const { return this->grid.cellDim; }

  // Returns the fluid density in GPU memory.
  auto getCurrentDensity() const {
    System::log<System::DEBUG1>(
        "[ICM_Compressible] Returning a copy of the current density");
    auto densGPU =
        icm_compressible::deghostifyDensity(currentFluid.density, grid.cellDim);
    return densGPU;
  }
  ...
  auto getCurrentVelocity() const {
    System::log<System::DEBUG1>(
        "[ICM_Compressible] Computing collocated current velocity");
    auto collocatedVelocityGPU = icm_compressible::computeCollocatedVelocity(
        currentFluid.velocity, grid.cellDim);
  ...
  auto getCurrentBottomGhostCellVelocity() const {
    System::log<System::DEBUG1>(
        "[ICM_Compressible] Copying ghost cell velocity for returning");
    auto n = getGhostGridSize();
    int ncells = n.x * n.y;
    DataXYZ ghostVels(ncells);
    // Zbottom ghost cells are the first in the array.
    thrust::copy(thrust::cuda::par, currentFluid.velocity.xyz(),
                 currentFluid.velocity.xyz() + ncells, ghostVels.xyz());
    return ghostVels;
  }
...
private:
  ...
  cached_vector<int3>
      ghostCells; // A list with cells that lie in the halo of the domain grid
  std::shared_ptr<Walls> walls;

  // Returns the number of cells in the fluid grid in each direction including
  // ghost cells The simulation domain is located in the range i_x =
  // 1:(getGhostGridSize().x-3) and similar for y and z.
  ...
  void checkInputValidity(Parameters par) {
    if (par.shearViscosity <= 0)
      throw std::runtime_error("[ICM_Compressible] Invalid shear viscosity");
    if (par.bulkViscosity < 0)
      throw std::runtime_error("[ICM_Compressible] Invalid bulk viscosity");
    if (par.temperature < 0)
      throw std::runtime_error("[ICM_Compressible] Invalid temperature");
    if (par.dt < 0)
      throw std::runtime_error("[ICM_Compressible] Invalid dt");
    if (par.speedOfSound <= 0)
      throw std::runtime_error("[ICM_Compressible] Invalid speed of sound");
    if (par.boxSize.x <= 0)
      throw std::runtime_error("[ICM_Compressible] Invalid box size");
    if ((par.cellDim.x <= 0 and par.hydrodynamicRadius <= 0) or
        (par.cellDim.x > 0 and par.hydrodynamicRadius > 0)) {
      throw std::runtime_error("[ICM_Compressible] I need either an "
                               "hydrodynamic radius or a number of cells");
    }
  }

  void printInitialMessages(Parameters par) const {
    System::log<System::MESSAGE>("[ICM_Compressible] dt: %g", dt);
    System::log<System::MESSAGE>("[ICM_Compressible] shear viscosity: %g",
                                 shearViscosity);
    System::log<System::MESSAGE>("[ICM_Compressible] bulk viscosity: %g",
                                 bulkViscosity);
    System::log<System::MESSAGE>(
        "[ICM_Compressible] isothermal speed of sound: %g", par.speedOfSound);
    System::log<System::MESSAGE>("[ICM_Compressible] temperature: %g",
                                 temperature);
    System::log<System::MESSAGE>("[ICM_Compressible] Box size: %g %g %g",
                                 par.boxSize.x, par.boxSize.y, par.boxSize.z);
    System::log<System::MESSAGE>("[ICM_Compressible] Fluid cells: %d %d %d",
    ...
    System::log<System::MESSAGE>("[ICM_Compressible] Expected long time "
                                 "particle self mobility coefficient : %g",
                                 m0);
    System::log<System::MESSAGE>("[ICM_Compressible] Expected long time "
                                 "particle self diffusion coefficient : %g",
                                 d0);
    System::log<System::MESSAGE>("[ICM_Compressible] seed: %u", seed);
    const real h = grid.cellSize.x;
    real a_c = par.speedOfSound * par.dt / h;
    real maxvx = *thrust::max_element(
        thrust::device_ptr<real>(currentFluid.velocity.x()),
        thrust::device_ptr<real>(currentFluid.velocity.x()) +
            currentFluid.velocity.size());
    real maxvy = *thrust::max_element(
        thrust::device_ptr<real>(currentFluid.velocity.y()),
        thrust::device_ptr<real>(currentFluid.velocity.y()) +
            currentFluid.velocity.size());
    real maxvz = *thrust::max_element(
        thrust::device_ptr<real>(currentFluid.velocity.z()),
        thrust::device_ptr<real>(currentFluid.velocity.z()) +
            currentFluid.velocity.size());
    ...
    real selfDiffusion = par.temperature / (6 * M_PI * par.shearViscosity *
  ...
  void initializeFluid(Parameters par) {
    ...
    for (int i = 0; i < ncells; i++) {
      ...
      if (cell.x == 0 or cell.x == cellDim.x - 1)
        continue;
      if (cell.y == 0 or cell.y == cellDim.y - 1)
        continue;
      if (cell.z == 0 or cell.z == cellDim.z - 1)
        continue;
      real3 pos = icm_compressible::cell2CenterPos(
          cell - make_int3(1, 1, 1), grid.cellDim, grid.box.boxSize);
      if (par.initialDensity)
        d_h[i] = par.initialDensity(pos);
      if (par.initialVelocityX)
        d_vx[i] = par.initialVelocityX(pos);
      if (par.initialVelocityY)
        d_vy[i] = par.initialVelocityY(pos);
      if (par.initialVelocityZ)
        d_vz[i] = par.initialVelocityZ(pos);
    }
    thrust::copy(d_h.begin(), d_h.end(), currentFluid.density.begin());
    thrust::copy(d_vx.begin(), d_vx.end(),
                 thrust::device_ptr<real>(currentFluid.velocity.x()));
    thrust::copy(d_vy.begin(), d_vy.end(),
                 thrust::device_ptr<real>(currentFluid.velocity.y()));
    thrust::copy(d_vz.begin(), d_vz.end(),
                 thrust::device_ptr<real>(currentFluid.velocity.z()));
  ...
  // Fills a vector with a list of ghost cells (a halo just outside of the
  // simulation domain)
  ...
  void forwardFluidDensityAndVelocityToNextStep(
      const DataXYZ &fluidForcingAtHalfStep);
  auto computeStochasticTensor();
  void fillGhostCells(FluidPointers fluid);
  void
  updateFluidWithRungeKutta3(const DataXYZ &fluidForcingAtHalfStep,
  ...
  void forwardPositionsToNextStep(const cached_vector<real4> &currentPositions,
