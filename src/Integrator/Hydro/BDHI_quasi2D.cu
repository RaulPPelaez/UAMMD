...
BDHI2D<HydroKernel>::BDHI2D(shared_ptr<ParticleGroup> pg, Parameters par)
    : Integrator(pg, "BDHI::BDHI2D"), dt(par.dt), temperature(par.temperature),
      tolerance(par.tolerance), viscosity(par.viscosity), box(par.box),
      grid(par.box, int3()), hydroKernel(par.hydroKernel),
...
void BDHI2D<HydroKernel>::checkInputParametersValidity(Parameters par) {
  if (par.box.boxSize.x == real(0.0) && par.box.boxSize.y == real(0.0)) {
    sys->log<System::ERROR>(
        "[BDHI::BDHI2D] Box of size zero detected, cannot work without a box! "
        "(make sure a box parameter was passed)");
    throw std::runtime_error("Invalid box");
  }
  if (hydrodynamicRadius <= 0) {
    sys->log<System::ERROR>(
        "[BDHI::BDHI2D] Please specify a valid hydrodynamic radius");
...
void BDHI2D<HydroKernel>::initializeInterpolationKernel(Parameters par) {
  int support =
      (int(3.0 * hydrodynamicRadius * grid.cellDim.x / grid.box.boxSize.x) +
       1) *
          2 +
      1;
...
template <class HydroKernel> void BDHI2D<HydroKernel>::resizeContainers() {
  try {
...
template <class HydroKernel> void BDHI2D<HydroKernel>::initCuFFT() {
  ...
  CufftSafeCall(cufftMakePlanMany(
      cufft_plan_forward, 2, &cdtmp.x, &inembed.x, 2, 1, &oembed.x, 2, 1,
      CUFFT_Real2Complex<real>::value, 2, &cufftWorkSizef));
  CufftSafeCall(cufftMakePlanMany(
      cufft_plan_inverse, 2, &cdtmp.x, &oembed.x, 2, 1, &inembed.x, 2, 1,
      CUFFT_Complex2Real<real>::value, 2, &cufftWorkSizei));
  size_t cufftWorkSize = std::max(cufftWorkSizef, cufftWorkSizei) + 10;
  size_t free_mem, total_mem;
  CudaSafeCall(cudaMemGetInfo(&free_mem, &total_mem));
  sys->log<System::DEBUG>("[BDHI::BDHI2D] cuFFT grid size: %d %d", cdtmp.x,
                          cdtmp.y);
  sys->log<System::DEBUG>("[BDHI::BDHI2D] Necessary work space for cuFFT: %s, "
                          "available: %s, total: %s",
  ...
  try {
    ...
    sys->log<System::ERROR>("[BDHI::BDHI2D] Not enough memory in device to "
...
template <class HydroKernel> void BDHI2D<HydroKernel>::printStartingMessages() {
  sys->log<System::MESSAGE>("[BDHI::BDHI2D] Initialized");
  sys->log<System::WARNING>(
      "[BDHI::BDHI2D] The tolerance parameter is currently ignored. Tolerance "
      "is hardcoded to ~1e-4");
  ...
  sys->log<System::MESSAGE>("[BDHI::BDHI2D] hydrodynamic radius: %g",
                            hydrodynamicRadius);
  ...
  if (box.boxSize.x != box.boxSize.y) {
    sys->log<System::WARNING>(
        "[BDHI::BDHI2D] Self mobility will be different for non square boxes!");
  }
  sys->log<System::MESSAGE>("[BDHI::BDHI2D] Box Size: %g %g",
                            grid.box.boxSize.x, grid.box.boxSize.y);
  sys->log<System::MESSAGE>("[BDHI::BDHI2D] Grid dimensions: %d %d",
                            grid.cellDim.x, grid.cellDim.y);
  double rh = hydrodynamicRadius;
  sys->log<System::MESSAGE>("[BDHI::BDHI2D] Interpolation kernel support: %g "
                            "rh max distance, %d cells total",
                            ibmKernel->support * 0.5 * grid.cellSize.x / rh,
                            ibmKernel->support);
  sys->log<System::MESSAGE>("[BDHI::BDHI2D] Gaussian variance: %g",
                            hydroKernel->getGaussianVariance(rh));
  sys->log<System::MESSAGE>("[BDHI::BDHI2D] h: %g %g", grid.cellSize.x,
                            grid.cellSize.y);
  sys->log<System::MESSAGE>("[BDHI::BDHI2D] Cell volume: %e",
                            grid.cellSize.x * grid.cellSize.y);
  sys->log<System::MESSAGE>("[BDHI::BDHI2D] Requested kernel tolerance: %g",
                            tolerance);
}

template <class HydroKernel> void BDHI2D<HydroKernel>::forwardTime() {
...
template <class HydroKernel> void BDHI2D<HydroKernel>::resetContainers() {
  sys->log<System::DEBUG2>("[BDHI::BDHI2D] Setting vels to zero...");
  thrust::fill(thrust::cuda::par.on(st2), particleVels.begin(),
               particleVels.end(), real2());
  if (interactors.size() > 0 or
    ...
    thrust::fill(thrust::cuda::par.on(st), gridVels.begin(), gridVels.end(),
                 typename decltype(gridVels)::value_type());
  }
...
template <class HydroKernel> void BDHI2D<HydroKernel>::spreadParticles() {
  spreadThermalDrift();
...
template <class HydroKernel> void BDHI2D<HydroKernel>::spreadThermalDrift() {
  if (hydroKernel->hasThermalDrift() and temperature > 0) {
    ...
    auto kernelX =
        std::make_shared<KernelThermalDrift<0>>(ibmKernel->support, width);
    IBM<KernelThermalDrift<0>> ibmX(
        kernelX, grid, IBM_ns::LinearIndex3D(2 * (n.x / 2 + 1), n.y, n.z));
    ibmX.spread(pos.begin(), trX, d_gridVels, numberParticles, st);
    const auto trY = thrust::make_constant_iterator<real2>({0, -temperature});
    auto kernelY =
        std::make_shared<KernelThermalDrift<1>>(ibmKernel->support, width);
    IBM<KernelThermalDrift<1>> ibmY(
        kernelY, grid, IBM_ns::LinearIndex3D(2 * (n.x / 2 + 1), n.y, n.z));
...
namespace BDHI2D_ns {
struct toReal2 {
  template <class vtype> inline __device__ real2 operator()(vtype q) {
    return make_real2(q);
  }
};
} // namespace BDHI2D_ns

template <class HydroKernel> void BDHI2D<HydroKernel>::spreadParticleForces() {
  if (interactors.size() > 0) {
    ...
    const auto f_tr =
        thrust::make_transform_iterator(force.begin(), BDHI2D_ns::toReal2());
    const auto n = grid.cellDim;
    IBM<Kernel> ibm(ibmKernel, grid,
...
void BDHI2D<HydroKernel>::forwardTransformVelocities() {
  bool isGridDataNonZero = interactors.size() > 0 or
                           (hydroKernel->hasThermalDrift() and temperature > 0);
  if (isGridDataNonZero) {
    ...
    CufftSafeCall(cufftExecReal2Complex<real>(cufft_plan_forward,
                                              (cufftReal *)d_gridVels,
                                              (cufftComplex *)d_gridVelsF));
  }
}
...
// Performs the product G_k*factor_k, given f_k and g_k, see Algorithm 1.3 in
// [1]
...
template <class HydroKernel>
__global__ void
forceFourier2Vel(cufftComplex2 *gridForces, cufftComplex2 *gridVels, real vis,
                 Grid grid, HydroKernel hydroKernel, real hydrodynamicRadius) {
...
template <class HydroKernel>
__global__ void
fourierBrownianNoise(cufftComplex2 *__restrict__ gridVelsFourier, Grid grid,
                     real prefactor, real vis, ullint seed, ullint step,
                     HydroKernel hydroKernel, real hydrodynamicRadius) {
  const int id = blockIdx.x * blockDim.x + threadIdx.x;
  const int2 nk = make_int2(grid.cellDim);
  if (id >= nk.y * (nk.x / 2 + 1)) {
    return;
  ...
  if (ik.x == 0 and ik.y > (nk.y - ik.y))
    return;
  if (ik.x == nk.x - ik.x and ik.y > (nk.y - ik.y))
    return;
  ...
  const bool isNyquist =
      (isYnyquist and ik.x == 0) or (isXnyquist and isYnyquist);
  cufftComplex2 noise;
  {
...
void BDHI2D<HydroKernel>::applyGreenFunctionConvolutionFourier() {
  ...
  if (isGridDataNonZero) {
    ...
    BDHI2D_ns::forceFourier2Vel<<<nblocks, nthreads, 0, st>>>(
        d_gridVels, d_gridVels, viscosity, grid, *hydroKernel,
        hydrodynamicRadius);
  }
}
...
void BDHI2D<HydroKernel>::addStochastichTermFourier() {
  ...
  if (temperature > 0) {
    ...
    const real prefactor = sqrt(
        2.0 * temperature / (viscosity * dt * box.boxSize.x * box.boxSize.y));
    const int nthreads = 128;
    const int nblocks =
        ((grid.cellDim.x / 2 + 1) * grid.cellDim.y) / nthreads + 1;
    BDHI2D_ns::fourierBrownianNoise<<<nblocks, nthreads, 0, st>>>(
        d_gridVels, grid, prefactor, viscosity, seed, counter, *hydroKernel,
        hydrodynamicRadius);
  }
}
...
template <class HydroKernel> void BDHI2D<HydroKernel>::interpolateVelocities() {
  sys->log<System::DEBUG2>(
      "[BDHI::BDHI2D] Interpolate grid velocities to particles");
...
void BDHI2D<HydroKernel>::updateParticlePositions() {
  ...
  try {
    thrust::transform(thrust::cuda::par.on(st), particleVels.begin(),
                      particleVels.end(), pos.begin(), pos.begin(),
                      BDHI2D_ns::euler_functor(dt));
  } catch (thrust::system_error &e) {
    sys->log<System::ERROR>("[BDHI::BDHI2D] Thrust transform failed with: %s",
                            e.what());
    throw;
  }
  CudaCheckError();
...
template <class HydroKernel> BDHI2D<HydroKernel>::~BDHI2D() {
  try {
    CudaCheckError();
    CudaSafeCall(cudaDeviceSynchronize());
    ...
    sys->log<System::ERROR>(
        "[BDHI::BDHI2D] Exception raised in BDHI2D destructor, silencing...");
  }
}
