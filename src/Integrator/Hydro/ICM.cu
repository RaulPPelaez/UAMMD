/*Raul P. Pelaez 2018-2021. Inertiam Coupling Method for particles in an
incompressible fluctuating fluid.

See ICM.cuh for more info.

REFERENCES:

[1] Inertial coupling method for particles in an incompressible fluctuating
fluid. Florencio Balboa, Rafael Delgado-Buscalioni, Boyce E. Griffith and
Aleksandar Donev. 2014 https://doi.org/10.1016/j.cma.2013.10.029. [2] Staggered
Schemes for Fluctuating Hydrodynamics. Florencio Balboa, et.al. 2012. [3]
Brownian Dynamics without Green's Functions. Steve Delong, Florencio Balboa, et.
al. 2014
...
int3 nextFFTWiseSize3D(int3 size) {
  ...
  do {
    ...
    fori(0, n)
        forj(0, n) for (int k = 0; k < n5;
                        k++) for (int k7 = 0; k7 < n7;
                                  k7++) for (int k11 = 0; k11 < n11; k11++) {
      ...
      int number =
          pow(2, i) * pow(3, j) * pow(5, k) * pow(7, k7) * pow(11, k11);
      // The fastest FFTs always have at least a factor of 2
      if (i == 0)
        continue;
      // I have seen empirically that factors 11 and 7 only works well with at
      // least a factor 2 involved
      if ((k11 > 0 && (i == 0)))
        continue;
      tmp[id] = number;
    }
  ...
  forj(0, 3) {
    fori(0, tmp.size()) {
      if (tmp[i] < cdim[j])
        continue;
      for (int k = 0; k < sizeof(forbiddenSizes) / sizeof(int); k++)
...
__global__ void spreadParticleForces(real4 *pos, real4 *force, real3 *gridData,
  ...
  const int3 cellix =
      grid.getCell(make_real3(pi.x - real(0.5) * h.x, pi.y, pi.z));
  const int3 celliy =
      grid.getCell(make_real3(pi.x, pi.y - real(0.5) * h.y, pi.z));
  const int3 celliz =
      grid.getCell(make_real3(pi.x, pi.y, pi.z - real(0.5) * h.z));
  const real3 cellPosOffset =
      real(0.5) * (h - grid.box.boxSize); // Cell index to cell center position
  ...
  for (int i = 0; i < numberNeighbourCells; i++) {
    // Contribution to vx
    {
      ...
      real fx = kernel.phi(r.x) * kernel.phi(r.y) * kernel.phi(r.z) *
                spreadQuantity.x;
    ...
    // Contribution to vy
    {
      ...
      real fy = kernel.phi(r.x) * kernel.phi(r.y) * kernel.phi(r.z) *
                spreadQuantity.y;
    ...
    // Contribution to vz
    {
      ...
      real fz = kernel.phi(r.x) * kernel.phi(r.y) * kernel.phi(r.z) *
                spreadQuantity.z;
...
__global__ void addThermalDrift(real4 *pos, real3 *gridVels, Grid grid,
                                ...
                                real deltaRFD, int numberParticles,
                                Kernel kernel, uint seed, uint step) {
  int id = blockIdx.x * blockDim.x + threadIdx.x;
  if (id >= numberParticles)
    return;
  const real3 pi = make_real3(pos[id]);
  // Corresponding cell of each direction in the staggered grid
  const int3 cellix =
      grid.getCell(make_real3(pi.x - real(0.5) * grid.cellSize.x, pi.y, pi.z));
  const int3 celliy =
      grid.getCell(make_real3(pi.x, pi.y - real(0.5) * grid.cellSize.y, pi.z));
  const int3 celliz =
      grid.getCell(make_real3(pi.x, pi.y, pi.z - real(0.5) * grid.cellSize.z));
  const real3 cellPosOffset =
      real(0.5) *
      (grid.cellSize - grid.box.boxSize); // Cell index to cell center position
  constexpr int P = Kernel::support / 2;
  constexpr int supportCells = Kernel::support;
  constexpr int numberNeighbourCells =
      supportCells * supportCells * supportCells;
  ...
  for (int i = 0; i < numberNeighbourCells; i++) {
    ...
    // Contribution to vx
    {
      ...
      real SmSdWx;
      {
        /*Staggered distance from q + noise to center of cell j*/
        const real3 rijx =
            q_p_noise - make_real3(celljx) * grid.cellSize - cellPosOffset;
        // Spread Wx, delta with the distance of the point where vx is defined,
        // which is 0.5dx to the left of the center
        auto r = grid.box.apply_pbc(
            {rijx.x - real(0.5) * grid.cellSize.x, rijx.y, rijx.z});
        SmSdWx = kernel.phi(r.x) * kernel.phi(r.y) * kernel.phi(r.z) * W.x;
      }
      {
        /*Staggered distance from q - noise to center of cell j*/
        const real3 rijx =
            q_m_noise - make_real3(celljx) * grid.cellSize - cellPosOffset;
        // Spread Wx
        auto r = grid.box.apply_pbc(
            {rijx.x - real(0.5) * grid.cellSize.x, rijx.y, rijx.z});
        SmSdWx -= kernel.phi(r.x) * kernel.phi(r.y) * kernel.phi(r.z) * W.x;
    ...
    // Contribution to vy
    {
      ...
      {
        const real3 rijy =
            q_p_noise - make_real3(celljy) * grid.cellSize - cellPosOffset;
        auto r = grid.box.apply_pbc(
            {rijy.x, rijy.y - real(0.5) * grid.cellSize.y, rijy.z});
        SmSdWy = kernel.phi(r.x) * kernel.phi(r.y) * kernel.phi(r.z) * W.y;
      }
      {
        const real3 rijy =
            q_m_noise - make_real3(celljy) * grid.cellSize - cellPosOffset;
        auto r = grid.box.apply_pbc(
            {rijy.x, rijy.y - real(0.5) * grid.cellSize.y, rijy.z});
        SmSdWy -= kernel.phi(r.x) * kernel.phi(r.y) * kernel.phi(r.z) * W.y;
      }
    ...
    // Contribution to vz
    {
      ...
      {
        const real3 rijz =
            q_p_noise - make_real3(celljz) * grid.cellSize - cellPosOffset;
        auto r = grid.box.apply_pbc(
            {rijz.x, rijz.y, rijz.z - real(0.5) * grid.cellSize.z});
        SmSdWz = kernel.phi(r.x) * kernel.phi(r.y) * kernel.phi(r.z) * W.z;
      }
      {
        const real3 rijz =
            q_m_noise - make_real3(celljz) * grid.cellSize - cellPosOffset;
        auto r = grid.box.apply_pbc(
            {rijz.x, rijz.y, rijz.z - real(0.5) * grid.cellSize.z});
        SmSdWz -= kernel.phi(r.x) * kernel.phi(r.y) * kernel.phi(r.z) * W.z;
      }
...
template <class vec3>
inline __device__ vec3 cellToWaveNumber(const int3 &cell, const int3 &cellDim,
                                        const vec3 &L) {
  const vec3 pi2invL = real(2.0) * real(M_PI) / L;
  /*My wave number*/
  vec3 k = {cell.x * pi2invL.x, cell.y * pi2invL.y, cell.z * pi2invL.z};
  /*Be careful with the conjugates*/
  /*Remember that FFT stores wave numbers as K=0:N/2+1:-N/2:-1 */
  if (cell.x >= (cellDim.x + 1) / 2)
    k.x -= real(cellDim.x) * pi2invL.x;
  if (cell.y >= (cellDim.y + 1) / 2)
    k.y -= real(cellDim.y) * pi2invL.y;
  if (cell.z >= (cellDim.z + 1) / 2)
    k.z -= real(cellDim.z) * pi2invL.z;
  return k;
}
...
// Apply a phase shift in Fourier space to a certain v(k), given cos(phase*k)
// and sin(phase*k)
inline __device__ cufftComplex3 shiftVelocity(cufftComplex3 vk,
                                              const real3 &cosk,
                                              const real3 &sink) {
...
// Solves the velocity in eq. 36 in [1] given the Fourier representation of the
// RHS and stores it in gridVelsPrediction. For that it applies the operator
// \mathcal{L}^-1 to the fluid forcing \vec{g}. See ICM.cuh. If projectOnly is
// true, this kernel applies only the P operator, without the (\rho/dt I -
// \eta/2 L)^{-1}
...
__global__ void solveStokesFourier(const cufftComplex3 *fluidForcing, // input
                                   ...
                                   real viscosity, real density, real dt,
  ...
  if (cell.x > grid.cellDim.x / 2 + 1)
    return; // I use R2C and C2R ffts
  if (cell.y >= grid.cellDim.y)
    return;
  if (cell.z >= grid.cellDim.z)
    return;
  ...
  // Staggered grid requires to define an effective k -> keff_\alpha =
  // 2/d\alpha*sin(k_\alpha*d\alpha/2)
  //  that takes into account the cell face shifted velocities in the discrete
  //  operators.
  // See eq. 57 in [2].
  real3 keff;
  // sin and cos for a phase of d\alpha/2, a phase of -d\alpha/2 corresponds to
  // (cosk, -sink)
  real3 sink, cosk;
  {
      ...
      shiftVelocity(fluidForcing[icell], cosk, real(-1.0) * sink);
  {
    ...
    if (!projectOnly) {
      ...
      prefactor =
          real(1.0) / (real(1.0) - (dt / density) * real(0.5) * viscosity * L);
    }
...
// In predictor mode takes q^n and replaces it by q^{n+1/2}, storing q^n in
// posOld In corrector mode takes q^{n+1/2} and q^n and replaces pos by q^{n+1}
enum class Step { PREDICTOR, CORRECTOR };
template <Step mode = Step::PREDICTOR, class Kernel>
__global__ void midPointStep(real4 *pos, real4 *posOld, const real3 *gridVels,
                             real dt, Grid grid, Kernel kernel,
                             int numberParticles) {
  int id = blockIdx.x * blockDim.x + threadIdx.x;
  if (id >= numberParticles)
    return;
  ...
  const int3 cellix = grid.getCell(make_real3(
      posCurrent.x - real(0.5) * grid.cellSize.x, posCurrent.y, posCurrent.z));
  const int3 celliy = grid.getCell(make_real3(
      posCurrent.x, posCurrent.y - real(0.5) * grid.cellSize.y, posCurrent.z));
  const int3 celliz = grid.getCell(make_real3(
      posCurrent.x, posCurrent.y, posCurrent.z - real(0.5) * grid.cellSize.z));
  ...
  for (int i = 0; i < numberNeighbourCells; i++) {
    ...
    }
    {
      ...
      const int jcelly = grid.getCellIndex(celljy);
      const real3 rijy =
          posCurrent - make_real3(celljy) * grid.cellSize - cellPosOffset;
    ...
    }
    {
      ...
      const int jcellz = grid.getCellIndex(celljz);
      const real3 rijz =
          posCurrent - make_real3(celljz) * grid.cellSize - cellPosOffset;
...
__device__ real3 computeNoiseDivergence(
    ...
    const real3 *gridVels, // Real space velocities of each cell, defined at the
                           // cell faces
    Grid grid, real noisePrefactor,
    // 6 random numbers per cell, sorted by direction [first number for all
    // cells, ..., second number for all cells,...]
    const real *random) {
  if (noisePrefactor == real(0.0))
    return real3();
  const int ncells = grid.getNumberCells();
  ...
  {
    const int n_mx = icell; // n_[m/p]\alpha -> cell index of neighbour in a
                            // certain direction
    real wxx_mx = random[n_mx + ncells * Direction::XX];
    const int n_px =
        grid.getCellIndex(grid.pbc_cell({cell.x + 1, cell.y, cell.z}));
  ...
  }
  {
    ...
    const int n_py =
        grid.getCellIndex(grid.pbc_cell({cell.x, cell.y + 1, cell.z}));
  ...
  }
  {
    ...
    const int n_pz =
        grid.getCellIndex(grid.pbc_cell({cell.x, cell.y, cell.z + 1}));
...
__device__ real3 computeAdvection(int3 cell, int icell, const real3 *gridVels,
  ...
  real3 advection;
  advection.x =
      invh.x * ((vx_px + vx) * (vx_px + vx) - (vx + vx_mx) * (vx + vx_mx));
  advection.x += invh.y * ((vx_py + vx) * (vy_px + vy) -
                           (vx + vx_my) * (vy_px_my + vy_my));
  advection.x += invh.z * ((vx_pz + vx) * (vz_px + vz) -
                           (vx + vx_mz) * (vz_px_mz + vz_mz));
  advection.y = invh.x * ((vy_px + vy) * (vx_py + vx) -
                          (vy + vy_mx) * (vx_mx_py + vx_mx));
  advection.y +=
      invh.y * ((vy_py + vy) * (vy_py + vy) - (vy + vy_my) * (vy + vy_my));
  advection.y += invh.z * ((vy_pz + vy) * (vz_py + vz) -
                           (vy + vy_mz) * (vz_py_mz + vz_mz));
  advection.z = invh.x * ((vz_px + vz) * (vx_pz + vx) -
                          (vz + vz_mx) * (vx_mx_pz + vx_mx));
  advection.z += invh.y * ((vz_py + vz) * (vy_pz + vy) -
                           (vz + vz_my) * (vy_my_pz + vy_my));
  advection.z +=
      invh.z * ((vz_pz + vz) * (vz_pz + vz) - (vz + vz_mz) * (vz + vz_mz));
...
// Computes fluid forcing \vec{g} (except the SF and thermal drift terms) for
// the unperturbed velocity field (unperturbed means that m_e = 0). See eq. 36
// and 41 in [1]
__global__ void updateCellVelocityUnperturbed(real3 *gridVels,
                                              real3 *cellAdvection, Grid grid,
                                              real density, real viscosity,
                                              real noiseAmp, real dt,
                                              int ncells, const real *random) {
  // I expect a 3D grid of threads, one for each fourier node/grid cell
  // Get my cell
  int3 cell;
  cell.x = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  if (cell.z >= grid.cellDim.z)
    return;
  real3 DivNoise =
      computeNoiseDivergence(cell, icell, gridVels, grid, noiseAmp, random);
  real3 Lv = computeVelLaplacian(cell, icell, gridVels, grid);
  real3 advection = computeAdvection(cell, icell, gridVels, grid, density);
  gridVels[icell] +=
      (dt * viscosity * real(0.5) / density) * Lv + DivNoise -
      (dt / density) *
          (real(1.5) * advection - real(0.5) * cellAdvection[icell]) +
      dt / density * externalFluidForcing(cell, icell, gridVels, grid);
  cellAdvection[icell] = advection;
}
} // namespace ICM_ns

ICM::ICM(shared_ptr<ParticleGroup> pg, Parameters par)
    : Integrator(pg, "Hydro::ICM"), dt(par.dt), temperature(par.temperature),
      density(par.density), viscosity(par.viscosity), box(par.box),
  ...
  if (density < 0)
    sys->log<System::CRITICAL>("[Hydro::ICM] Please provide fluid density");
  if (viscosity < 0)
    sys->log<System::CRITICAL>("[Hydro::ICM] Please provide fluid viscosity");
  if (par.hydrodynamicRadius > 0 and par.cells.x > 0)
    sys->log<System::CRITICAL>("[Hydro::ICM] Please provide hydrodynamic "
                               "radius OR cell dimensions, not both.");
  initializeGrid(par);
...
void ICM::initializeGrid(Parameters par) {
  ...
  if (par.cells.x < 0) {
    if (par.hydrodynamicRadius < 0)
      sys->log<System::CRITICAL>("[BHDI::ICM] I need either the hydrodynamic "
...
void ICM::printMessages(Parameters par) {
  /*Print information*/
  if (par.hydrodynamicRadius > 0)
    sys->log<System::MESSAGE>("[Hydro::ICM] Target hydrodynamic radius: %f",
                              par.hydrodynamicRadius);
  sys->log<System::MESSAGE>(
      "[Hydro::ICM] Closest possible hydrodynamic radius: %f",
      getHydrodynamicRadius());
  sys->log<System::MESSAGE>("[Hydro::ICM] Box Size: %f %f %f", box.boxSize.x,
                            box.boxSize.y, box.boxSize.z);
  sys->log<System::MESSAGE>("[Hydro::ICM] Mesh dimensions: %d %d %d",
  ...
  sys->log<System::MESSAGE>("[Hydro::ICM] Self mobility: %f",
                            this->getSelfMobility());
  sys->log<System::MESSAGE>("[Hydro::ICM] dt: %f", dt);
  if (sumThermalDrift and temperature > real(0.0)) {
    sys->log<System::MESSAGE>("[Hydro::ICM] Random Finite Diference delta: %e",
                              deltaRFD);
    sys->log<System::MESSAGE>("[Hydro::ICM] Summing thermal drift");
  } else
    sys->log<System::MESSAGE>(
        "[Hydro::ICM] Not taking into account thermal drift");
  {
...
void ICM::resizeContainers() {
  ...
  try {
    ...
    sys->log<System::EXCEPTION>(
        "[Hydro::ICM] Thrust could not allocate necessary arrays at "
        "initialization with error: %s",
        e.what());
    throw;
  }
}
...
void ICM::initCuFFT() {
  ...
  /*I want to make three 3D FFTs, each one using one of the three interleaved
   * coordinates*/
  CufftSafeCall(cufftMakePlanMany(
      cufft_plan_forward, 3, &cdtmp.x, /*Three dimensional FFT*/
      &cdtmp.x,
      ...
      CUFFT_Real2Complex<real>::value, 3, &cufftWorkSizef));
  sys->log<System::DEBUG>("[Hydro::ICM] cuFFT grid size: %d %d %d", cdtmp.x,
                          cdtmp.y, cdtmp.z);
  /*Same as above, but with C2R for inverse FFT*/
  CufftSafeCall(cufftMakePlanMany(
      cufft_plan_inverse, 3, &cdtmp.x, /*Three dimensional FFT*/
      &cdtmp.x,
      /*Each FFT starts in 1+previous FFT index. FFTx in 0*/
      3, 1, // Each element separated by three others x0 y0 z0 x1 y1 z1...
      &cdtmp.x, 3, 1,
      /*Perform 3 inverse batched FFTs*/
      CUFFT_Complex2Real<real>::value, 3, &cufftWorkSizei));

  /*Allocate cuFFT work area*/
  size_t cufftWorkSize = std::max(cufftWorkSizef, cufftWorkSizei);
  sys->log<System::DEBUG>("[Hydro::ICM] Necessary work space for cuFFT: %s",
                          printUtils::prettySize(cufftWorkSize).c_str());
  size_t free_mem, total_mem;
  CudaSafeCall(cudaMemGetInfo(&free_mem, &total_mem));
  if (free_mem < cufftWorkSize) {
    sys->log<System::CRITICAL>("[Hydro::ICM] Not enough memory in device to "
                               "allocate cuFFT free %s, needed: %s!!!",
...
void ICM::initFluid() {
  ...
  real3 *d_cellVelCPU =
      (real3 *)thrust::raw_pointer_cast(cellVelocityCPU.data());
  fori(0, ncells) {
    double fluidVelAmp = sqrt(temperature / (density * cellVolume));
    d_cellVelCPU[i] = make_real3(sys->rng().gaussian(0.0, fluidVelAmp),
  ...
  try {
    ...
    sys->log<System::CRITICAL>("[Hydro::ICM] Thrust could not upload initial "
                               "fluid velocity with error: %s",
                               e.what());
  }
...
void ICM::spreadParticleForces() {
  ...
  auto indexIter = pg->getIndexIterator(access::location::gpu);
  {
    ...
    thrust::fill(thrust::cuda::par, force_gr, force_gr + numberParticles,
                 real4());
  }
  /*Compute new force*/
  for (auto forceComp : interactors)
    forceComp->sum({.force = true, .energy = false, .virial = false}, 0);
  auto pos = pd->getPos(access::location::gpu, access::mode::read);
  auto force = pd->getForce(access::location::gpu, access::mode::read);
  auto d_gridVels = thrust::raw_pointer_cast(gridVels.data());
  const real prefactor = dt / density;
  ICM_ns::spreadParticleForces<<<nblocks, nthreads, 0, st>>>(
      pos.raw(), force.raw(), (real3 *)d_gridVels, grid, numberParticles,
      prefactor, Kernel(grid.cellSize.x));
  CudaCheckError();
}
...
void ICM::thermalDrift() {
  ...
  auto pos = pd->getPos(access::location::gpu, access::mode::read);
  ICM_ns::addThermalDrift<<<Nblocks, Nthreads, 0, st>>>(
      pos.raw(), (real3 *)d_gridVels, grid, driftPrefactor, deltaRFD,
      numberParticles, Kernel(grid.cellSize.x), seed, step);
  CudaCheckError();
}
// Compute \vec{g} (except SF and thermal drift) and store it in
// gridVelsPrediction Also stores the current advection term
void ICM::unperturbedFluidForcing() {
  ...
  real3 *d_cellAdvection =
      (real3 *)thrust::raw_pointer_cast(cellAdvection.data());
  auto d_random = thrust::raw_pointer_cast(random.data());
  dim3 NthreadsCells = dim3(8, 8, 8);
  dim3 NblocksCells;
  NblocksCells.x = grid.cellDim.x / NthreadsCells.x +
                   ((grid.cellDim.x % NthreadsCells.x) ? 1 : 0);
  NblocksCells.y = grid.cellDim.y / NthreadsCells.y +
                   ((grid.cellDim.y % NthreadsCells.y) ? 1 : 0);
  NblocksCells.z = grid.cellDim.z / NthreadsCells.z +
                   ((grid.cellDim.z % NthreadsCells.z) ? 1 : 0);
  real dV = grid.cellSize.x * grid.cellSize.y * grid.cellSize.z;
  real noiseAmp = sqrt(2 * temperature * viscosity * dt / dV) / density;
  int ncells = grid.getNumberCells();
  ICM_ns::updateCellVelocityUnperturbed<<<NblocksCells, NthreadsCells, 0, st>>>(
      d_gridVels, d_cellAdvection, grid, density, viscosity, noiseAmp, dt,
      ncells, d_random);
  CudaCheckError();
}
// Computes \vec{\tilde{v}}=\mathcal{L}^{-1}\vec{g} in Fourier space to the
// current \vec{g} Stores \vec{\tilde{v}} in gridVels
void ICM::applyStokesSolutionOperator() {
  ...
  auto d_gridDataF =
      (cufftComplex *)thrust::raw_pointer_cast(gridVelsPredictionF.data());
  // Go to fourier space
  ...
  CufftSafeCall(
      cufftExecReal2Complex<real>(cufft_plan_forward, d_gridData, d_gridDataF));
  dim3 NthreadsCells = dim3(8, 8, 8);
  dim3 NblocksCells;
  NblocksCells.x = grid.cellDim.x / NthreadsCells.x +
                   ((grid.cellDim.x % NthreadsCells.x) ? 1 : 0);
  ...
  ICM_ns::solveStokesFourier<<<NblocksCells, NthreadsCells, 0, st>>>(
      (cufftComplex3 *)d_gridDataF, (cufftComplex3 *)d_gridDataF, viscosity,
      density, dt, grid, removeTotalMomentum);
  sys->log<System::DEBUG3>("[Hydro::ICM] Going back to real space");
  // Go back to real space
  CufftSafeCall(
      cufftExecComplex2Real<real>(cufft_plan_inverse, d_gridDataF, d_gridData));
  CudaCheckError();
}
...
void ICM::predictorStep() {
  ...
  ICM_ns::midPointStep<ICM_ns::Step::PREDICTOR>
      <<<Nblocks, Nthreads, 0, st>>>(pos.raw(), d_posOld, d_gridVels, dt, grid,
                                     Kernel(grid.cellSize.x), numberParticles);
  CudaCheckError();
}
...
void ICM::correctorStep() {
  ...
  real4 *d_posOld = thrust::raw_pointer_cast(posOld.data());
  auto pos = pd->getPos(access::location::gpu, access::mode::readwrite);
  ICM_ns::midPointStep<ICM_ns::Step::CORRECTOR>
      <<<Nblocks, Nthreads, 0, st>>>(pos.raw(), d_posOld, d_gridVels, dt, grid,
                                     Kernel(grid.cellSize.x), numberParticles);
  CudaCheckError();
}
