/*Raul P. Pelaez 2018-2021. Inertial Coupling Method for particles in an
...
improved translational invariance [2]. ICM solves the incompressible fluctuating
Navier-Stokes equation and couples the resulting fluid with immersed particles
particles with the Immerse Boundary Method. Currently the case with excess mass
0 is encoded.
...
par.density = 1.0;
par.viscosity = 1.0;
//The hydrodynamic radius is given by ~0.91*Lbox/cellDimension in ICM, so the
hydrodynamic radius given here cannot be enforced exactly, rather the most
...
par.dt = 0.01;
par.box = Box(64);
//par.sumThermalDrift = false; //Default is false, controls if the thermal drift
...
The function getHydrodynamicRadius() will return the used hydrodynamic radius.
You should expect a variation of +- 1% between this value and the one you
...
But others can be selected with arbitrary support to better describe or satisfy
certain conditions.

The thermal drift is computed using random finite diferences as explained in
[3].
...
// This kernel takes velocities in a staggered grid and interpolates them to the
// cell centers. Input: gridVelsStaggered, grid Output: gridVelsCollocated Must
// be launched in a 3D grid of threads with at least grid.cellDim threads.
__global__ void
interpolateVelocitiesToCellCentersD(const real3 *gridVelsStaggered,
                                    real3 *gridVelsCollocated, Grid grid) {
  ...
  const int icelljx =
      grid.getCellIndex(grid.pbc_cell(cell - make_int3(1, 0, 0)));
  const real vx = real(0.5) * (v0.x + gridVelsStaggered[icelljx].x);
  const int icelljy =
      grid.getCellIndex(grid.pbc_cell(cell - make_int3(0, 1, 0)));
  const real vy = real(0.5) * (v0.y + gridVelsStaggered[icelljy].y);
  const int icelljz =
      grid.getCellIndex(grid.pbc_cell(cell - make_int3(0, 0, 1)));
  const real vz = real(0.5) * (v0.z + gridVelsStaggered[icelljz].z);
  gridVelsCollocated[icell] = {vx, vy, vz};
}

// This function takes GPU velocities in a staggered grid and interpolates them
// to the cell centers. Input: gridVelsStaggered, grid Output:
// gridVelsCollocated
void interpolateVelocitiesToCellCenters(const real3 *gridVelsStaggered,
                                        real3 *gridVelsCollocated, Grid grid) {
  const auto n = grid.cellDim;
  dim3 blockSize = {8, 8, 8};
  dim3 Nblocks = {n.x / blockSize.x + 1, n.y / blockSize.y + 1,
                  n.z / blockSize.z + 1};
  interpolateVelocitiesToCellCentersD<<<Nblocks, blockSize>>>(
      gridVelsStaggered, gridVelsCollocated, grid);
}
...
public:
  struct Parameters {
    ...
    real dt;
    Box box;
    int3 cells = {-1, -1, -1}; // Default is compute the closest number of cells
                               // that is a FFT friendly number
    bool sumThermalDrift =
        false; // Thermal drift has a neglegible contribution in ICM
    bool removeTotalMomentum =
        true; // Set the total fluid momentum to zero in each step
  };
  ...
  real getSelfMobility() {
    ...
    return 1.0 / (6 * M_PI * viscosity * rh) *
           (1 - 2.837297 * rh / box.boxSize.x);
  }
  real getHydrodynamicRadius() {
    return 0.91 * box.boxSize.x / (real)grid.cellDim.x;
  }

  // Interpolates the grid velocities to the cell centers and returns a pointer
  // to them The velocity assigned to cell (i,j,k) is located at i+(j+k*n.y)*n.x
  // Where n, the number of fluid cells in each direction, is returned by
  // getNumberFluidCells
  const real3 *getFluidVelocities(access::location dev) {
    gridVels_collocated.resize(gridVels.size());
    icm_ns::interpolateVelocitiesToCellCenters(
        thrust::raw_pointer_cast(gridVels.data()),
        thrust::raw_pointer_cast(gridVels_collocated.data()), grid);
    real3 *ptr = nullptr;
    switch (dev) {
    case access::gpu:
    ...
    default:
      System::log<System::EXCEPTION>(
          "Invalid device in ICM::getFluidVelocities");
  ...
  int3 getNumberFluidCells() { return grid.cellDim; }
...
private:
  ...
  // Host and device versions of grid velocities for getFluidVelocities (with
  // velocities defined in the centers of the cells)
