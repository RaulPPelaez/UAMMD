...
template <class T> using cached_vector = uammd::uninitialized_cached_vector<T>;
...
struct FluidPointers {
  ...
  FluidPointers(const RealContainer &dens, const DataXYZ &vel,
                const DataXYZ &momentum)
      : density(const_cast<real *>(thrust::raw_pointer_cast(dens.data()))),
        velocityX(const_cast<real *>(vel.x())),
        velocityY(const_cast<real *>(vel.y())),
        velocityZ(const_cast<real *>(vel.z())),
        momentumX(const_cast<real *>(momentum.x())),
        momentumY(const_cast<real *>(momentum.y())),
        momentumZ(const_cast<real *>(momentum.z())) {}
  real *density;
...
// That is, the cell =(0,0,0) is not located at index i=0, rather at index
// 1+(1+(n.y+2))*(n.x+2)
...
__device__ real fetchScalar(ScalarIterator scalar, int3 cell, int3 n) {
  // cell = pbc_cell(cell, n); //With ghost cells the limits of the grid are
  // never accessed.
...
__device__ auto getVelocityPointer(FluidPointers fluid) {
  if (direction == subgrid::x)
    return fluid.velocityX;
  if (direction == subgrid::y)
    return fluid.velocityY;
  if (direction == subgrid::z)
    return fluid.velocityZ;
}

template <subgrid direction>
__device__ auto getMomentumPointer(FluidPointers fluid) {
  if (direction == subgrid::x)
    return fluid.momentumX;
  if (direction == subgrid::y)
    return fluid.momentumY;
  if (direction == subgrid::z)
    return fluid.momentumZ;
}
