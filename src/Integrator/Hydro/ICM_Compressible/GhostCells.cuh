...
__host__ __device__ int mod(int a, int b) { return ((a %= b) < 0) ? a + b : a; }

// Computes the cell coordinates that correspond to a given ghost cell with
// periodic boundary conditions. The returned coordinates are in full grid (the
// fluid grid including ghost cells, size n+2)
...
auto listGhostCells(int3 n) {
  ...
  const int numberGhostCells =
      (n.x + 2) * (n.y + 2) * (n.z + 2) - n.x * n.y * n.z;
  ghostCells.reserve(numberGhostCells);
  for (int id = 0; id < (n.x + 2) * (n.y + 2) * (n.z + 2); id++) {
    const auto cell = getCellFromThreadId(id, n + 2);
    if (cell.x == 0 or cell.x == n.x + 1 or cell.y == 0 or cell.y == n.y + 1 or
  ...
  System::log<System::DEBUG3>(
      "[ICM_Compressible] Number of ghost cells: %d, expected %d",
...
template <wall awall> __device__ bool isGhostCellAtWall(int3 cell, int3 n) {
  if (awall == wall::ztop and cell.z == n.z + 1)
    return true;
  if (awall == wall::zbottom and cell.z == 0)
    return true;
  return false;
}

// Applies periodic boundary conditions to the given ghost cell
__device__ void periodifyGhostCell(FluidPointers fluid, int3 ghostCell,
                                   int3 n) {
  const int ighost =
  ...
  const int iperiodic =
      periodicCell.x +
      (periodicCell.y + periodicCell.z * (n.y + 2)) * (n.x + 2);
...
__global__ void updateGhostCellsD(FluidPointers fluid, Walls walls,
                                  int3 *ghostCells, int3 n,
                                  int numberGhostCells) {
  uint id = blockIdx.x * blockDim.x + threadIdx.x;
  if (id >= numberGhostCells)
    return;
...
template <class Walls>
__global__ void updateGhostCellsWallsD(FluidPointers fluid, Walls walls,
                                       int3 *ghostCells, int3 n,
                                       int numberGhostCells) {
  uint id = blockIdx.x * blockDim.x + threadIdx.x;
  if (id >= numberGhostCells)
...
void callUpdateGhostCells(FluidPointers fluid, std::shared_ptr<Walls> walls,
  ...
  updateGhostCellsD<<<blocks, threads>>>(fluid, *walls, ghostCells_ptr, n,
                                         numberGhostCells);
  updateGhostCellsWallsD<<<blocks, threads>>>(fluid, *walls, ghostCells_ptr, n,
                                              numberGhostCells);
}
...
__device__ int getFluctTensIndexGhost(int3 cell_i, int3 n) {
  ...
  const int ntot = numberCellsWithGhosts.x * numberCellsWithGhosts.y *
                   numberCellsWithGhosts.z;
...
template <subgrid alpha, subgrid beta>
__device__ void mirrorFluctuations(real2 *fluidStochasticTensor, int3 ghostCell,
                                   int3 periodicCell, int3 n) {
...
__device__ void periodifyFluctuatingGhostCell(real2 *fluidStochasticTensor,
                                              int3 ghostCell, int3 n) {
  int3 periodicCell = computeMirrorGhostCell(ghostCell, n);
  mirrorFluctuations<subgrid::x, subgrid::x>(fluidStochasticTensor, ghostCell,
                                             periodicCell, n);
  mirrorFluctuations<subgrid::y, subgrid::y>(fluidStochasticTensor, ghostCell,
                                             periodicCell, n);
  mirrorFluctuations<subgrid::z, subgrid::z>(fluidStochasticTensor, ghostCell,
                                             periodicCell, n);
  mirrorFluctuations<subgrid::x, subgrid::y>(fluidStochasticTensor, ghostCell,
                                             periodicCell, n);
  mirrorFluctuations<subgrid::x, subgrid::z>(fluidStochasticTensor, ghostCell,
                                             periodicCell, n);
  mirrorFluctuations<subgrid::y, subgrid::z>(fluidStochasticTensor, ghostCell,
                                             periodicCell, n);
}

template <subgrid alpha, subgrid beta>
__device__ void wallFluctuations(real2 *fluidStochasticTensor, int3 ghostCell,
                                 int3 n) {
  int isource = getFluctTensIndexGhost<alpha, beta>(ghostCell, n);
  // See eq. 2.126 in Floren's thesis. We double the noise for Dirichlet
  // boundary conditions
  fluidStochasticTensor[isource] *= real(2.0);
}

__device__ void fillWallFluctuatingGhostCell(real2 *fluidStochasticTensor,
                                             int3 ghostCell, int3 n) {
...
template <class Walls>
__global__ void updateGhostCellsFluctuationsD(real2 *fluidStochasticTensor,
                                              Walls walls, int3 *ghostCells,
...
void callUpdateGhostCellsFluctuations(Container2 &fluidStochasticTensor,
  ...
  updateGhostCellsFluctuationsD<<<blocks, threads>>>(
      noise_ptr, *walls, ghostCells_ptr, n, numberGhostCells);
}

// Copies the density from the input (which includes ghost cells) to an output
// without ghost cells.
__global__ void deghostifyDensityD(const real *inputDensity,
                                   real *outputDensity, int3 n) {
...
auto deghostifyDensity(const Container &density, int3 n) {
  ...
  deghostifyDensityD<<<blocks, threads>>>(
      thrust::raw_pointer_cast(density.data()),
      thrust::raw_pointer_cast(outputDensity.data()), n);
