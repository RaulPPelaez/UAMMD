...
// Cell to index transformation for the IBM module, takes into account ghost
// cells
struct LinearIndexGhost3D {
  LinearIndexGhost3D(int3 n) : n(n) {}

  // Returns the index of a certain cell, c, including ghost cells
  inline __device__ int operator()(int3 c) const { return linearIndex3D(c, n); }
...
struct ShiftTransform {
  ...
  __device__ auto operator()(real4 p) { return make_real3(p) + shift; }
};
...
auto spreadParticleForces(const ParticleIterator &particleData,
                          const PositionIterator &positions,
...
auto interpolateFluidVelocities(const DataXYZ &gridData,
                                const PositionIterator &positions,
  ...
  if (numberParticles > 0) {
    const int3 n = grid.cellDim;
    const real3 h = grid.cellSize;
    IBM<Kernel, Grid, LinearIndexGhost3D> ibm(kernel, grid,
                                              LinearIndexGhost3D(n));
    auto posX = make_shift_iterator(positions, {-real(0.5) * h.x, 0, 0});
