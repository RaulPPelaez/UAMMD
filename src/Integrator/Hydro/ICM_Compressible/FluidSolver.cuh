...
Other substepping schemes might be used with slight modifications to this code
(see Florencio Balboa's Ph.D manuscript) References:
...
__device__ real computeDensityIncrement(int3 cell_i, FluidPointers fluid,
                                        real dt, Grid grid) {
  const real3 h = grid.getCellSize(cell_i);
  const auto n = grid.cellDim;
  const real momentumDivergence =
      computeMomentumDivergence(cell_i, n, fluid, h);
  return -momentumDivergence * dt;
}

// Computes the divergence of the stress tensor \tens{\sigma} (without
// fluctuations),
//  -\nabla\cdot\tens{\sigma} = -\nabla\pi + \eta\nabla^2\vec{v} + (\xi +
//  \eta/3)\nabla(\nabla\cdot\vec{v})
...
__device__ real3 computeDeterministicStressDivergence(
    int3 cell_i, FluidPointers fluid, FluidParameters par,
    EquationOfState densityToPressure, Grid grid) {
  const int3 n = grid.cellDim;
  const real3 h = grid.getCellSize(cell_i);
  const real3 pressureGradient =
      computePressureGradient(cell_i, n, h, densityToPressure, fluid.density);
  const real3 velocityLaplacian = computeVelocityLaplacian(cell_i, n, h, fluid);
  const real3 velocityDivergenceGradient =
      computeVelocityDivergenceGradient(cell_i, n, h, fluid);
  const real3 diffusion = par.shearViscosity * velocityLaplacian;
  const real3 convection =
      (par.bulkViscosity + par.shearViscosity / real(3.0)) *
      velocityDivergenceGradient;
...
template <class EquationOfState>
__device__ real3 computeDeterministicMomentumIncrement(
    int3 cell_i, FluidPointers fluid, FluidParameters par,
    EquationOfState densityToPressure, Grid grid) {
  real3 momentumIncrement = real3();
  momentumIncrement += computeKineticDerivative(cell_i, fluid, grid);
  momentumIncrement -= computeDeterministicStressDivergence(
      cell_i, fluid, par, densityToPressure, grid);
...
public:
  RungeKutta3(int subStep) { setTimePrefactors(subStep); }

  __device__ real incrementScalar(real scalarAtTimeA, real scalarAtTimeB,
                                  real increment) const {
    return timePrefactorA * scalarAtTimeA +
           timePrefactorB * (scalarAtTimeB + increment);
  }
};

// Solves U^c = A*U^a + B(U^b + \Delta U(U^b))
// Where U might be the density or the fluid velocity and (a,b,c) are three
// different time points inside a time step In order to go from the time step n
// to n+1 the solver must be called three times for the density and then the
// velocity:
//  1- a=0, b=n and c=n+1/3
...
__global__ void
rungeKuttaSubStepD(FluidTimePack fluid, DataXYZPtr fluidForcingAtHalfStep,
                   const real2 *fluidStochasticTensor, FluidParameters par,
                   EquationOfState densityToPressure, int step, Grid grid,
                   RungeKutta3 rk) {
  const int id = blockDim.x * blockIdx.x + threadIdx.x;
  if (id >= grid.getNumberCells())
    return;
  // Cell inside the domain assigned to this thread
  const auto cell_i = getCellFromThreadId(id, grid.cellDim);
  // The linear index in the fluid arrays assigned to this cell, taking into
  // account the ghost cells
  const int ghostId = linearIndex3D(cell_i, grid.cellDim);
  real densityAtTimeC;
  {
    const real densityIncrement =
        computeDensityIncrement(cell_i, fluid.timeB, par.dt, grid);
    densityAtTimeC =
        rk.incrementScalar(fluid.timeA.density[ghostId],
                           fluid.timeB.density[ghostId], densityIncrement);
  }
  real3 momentumIncrement;
  {
    const real3 externalForcing = fluidForcingAtHalfStep.xyz()[id];
    const real3 deterministicMomentumIncrement =
        computeDeterministicMomentumIncrement(cell_i, fluid.timeB, par,
                                              densityToPressure, grid);
    // Only compute fluctuations if temperature >0, which is encoded as the
    // pointer for the flucutations being non-null
    real3 fluctuatingMomentumIncrement = real3();
    if (fluidStochasticTensor) {
      fluctuatingMomentumIncrement =
          computeFluctuatingStressDivergence<subStep>(
              cell_i, fluidStochasticTensor, grid);
    }
    momentumIncrement = deterministicMomentumIncrement +
                        fluctuatingMomentumIncrement + par.dt * externalForcing;
  }
  ...
  momentumAtTimeC.x = rk.incrementScalar(momentumAtTimeA.x, momentumAtTimeB.x,
                                         momentumIncrement.x);
  momentumAtTimeC.y = rk.incrementScalar(momentumAtTimeA.y, momentumAtTimeB.y,
                                         momentumIncrement.y);
  momentumAtTimeC.z = rk.incrementScalar(momentumAtTimeA.z, momentumAtTimeB.z,
                                         momentumIncrement.z);
  ...
  // Store momentum in these arrays, just after this kernel the velocities are
  // computed with it We cannot do it now because all the densities at time C
