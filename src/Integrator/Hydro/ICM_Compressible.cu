...
auto ICM_Compressible_impl<Walls>::storeCurrentPositions() {
  System::log<System::DEBUG2>(
      "[ICM_Compressible] Store current particle positions");
...
template <class Walls>
auto ICM_Compressible_impl<Walls>::interpolateFluidVelocityToParticles(
    const DataXYZ &fluidVelocity) {
  System::log<System::DEBUG2>(
      "[ICM_Compressible] Interpolate fluid velocities");
  using namespace icm_compressible;
  int numberParticles = pg->getNumberParticles();
  auto pos = pd->getPos(access::gpu, access::read);
  auto kernel = std::make_shared<Kernel>(grid.cellSize.x);
  auto vel = staggered::interpolateFluidVelocities(
      fluidVelocity, pos.begin(), kernel, numberParticles, grid);
  return vel;
}
...
auto ICM_Compressible_impl<Walls>::spreadCurrentParticleForcesToFluid() {
  ...
  auto kernel = std::make_shared<Kernel>(grid.cellSize.x);
  int numberParticles = pg->getNumberParticles();
  auto fluidForcing = staggered::spreadParticleForces(
      forces.begin(), pos.begin(), kernel, numberParticles, grid);
  return fluidForcing;
}
...
auto ICM_Compressible_impl<Walls>::callRungeKuttaSubStep(
    const DataXYZ &fluidForcingAtHalfStep,
    const cached_vector<real2> &fluidStochasticTensor,
    FluidPointers fluidAtSubTime) {
  System::log<System::DEBUG2>("[ICM_Compressible] Runge Kutta sub step %d",
                              subStep);
  using namespace icm_compressible;
  FluidData fluidAtNewTime(getGhostGridSize());
  FluidTimePack fluid{currentFluid.getPointers(), fluidAtSubTime,
                      fluidAtNewTime.getPointers()};
  FluidParameters params{shearViscosity, bulkViscosity, dt};
  callRungeKuttaSubStepGPU<subStep>(
      grid, fluid, DataXYZPtr(fluidForcingAtHalfStep),
      thrust::raw_pointer_cast(fluidStochasticTensor.data()), params,
      *densityToPressure, steps);
  ...
  for (auto i : updatables)
    i->updateSimulationTime((steps + subStepTime) * dt);
...
void ICM_Compressible_impl<Walls>::fillGhostCells(FluidPointers fluid) {
  ...
  icm_compressible::callUpdateGhostCells(fluid, walls, ghostCells,
                                         grid.cellDim);
}

// Uses the RK3 solver in FluidSolver.cuh
template <class Walls>
void ICM_Compressible_impl<Walls>::updateFluidWithRungeKutta3(
    const DataXYZ &fluidForcingAtHalfStep,
    const cached_vector<real2> &fluidStochasticTensor) {
  System::log<System::DEBUG2>("[ICM_Compressible] Update fluid with RK3");
  auto fluidPrediction =
      callRungeKuttaSubStep<1>(fluidForcingAtHalfStep, fluidStochasticTensor,
                               currentFluid.getPointers());
  auto fluidAtHalfStep =
      callRungeKuttaSubStep<2>(fluidForcingAtHalfStep, fluidStochasticTensor,
                               fluidPrediction.getPointers());
  fluidPrediction.clear();
  fluidPrediction =
      callRungeKuttaSubStep<3>(fluidForcingAtHalfStep, fluidStochasticTensor,
...
auto ICM_Compressible_impl<Walls>::computeStochasticTensor() {
  ...
  if (temperature > 0) {
    ...
    auto fluidStochasticTensor_ptr =
        thrust::raw_pointer_cast(fluidStochasticTensor.data());
    FluidParameters params{shearViscosity, bulkViscosity, dt};
    callFillStochasticTensorGPU(grid, fluidStochasticTensor_ptr, seed,
                                uint(steps), params, temperature);
    icm_compressible::callUpdateGhostCellsFluctuations(
        fluidStochasticTensor, walls, ghostCells, grid.cellDim);
  }
  return fluidStochasticTensor;
}

template <class Walls>
void ICM_Compressible_impl<Walls>::forwardFluidDensityAndVelocityToNextStep(
    const DataXYZ &fluidForcingAtHalfStep) {
...
// The solver assumes the external forcing remains constant throughout the
// timestep and requires the forces at time n+1/2
template <class Walls>
void ICM_Compressible_impl<Walls>::addFluidExternalForcing(
    DataXYZ &fluidForcingAtHalfStep) {
  // thrust::transform(fluidForcingAtHalfStep.x(),
  // 			fluidForcingAtHalfStep.x() +
  // fluidForcingAtHalfStep.size(), fluidForcingAtHalfStep.x(),
...
auto ICM_Compressible_impl<Walls>::computeCurrentFluidForcing() {
  ...
  for (auto i : updatables)
    i->updateSimulationTime((steps + 0.5) * dt);
...
// Takes positions to n+1/2: \vec{q}^{n+1/2} = \vec{q}^n +
// dt/2\oper{J}^n\vec{v}^n
template <class Walls>
void ICM_Compressible_impl<Walls>::forwardPositionsToHalfStep() {
  if (pg->getNumberParticles() > 0) {
    System::log<System::DEBUG2>(
        "[ICM_Compressible] Forward particles to n+1/2");
    auto velocities =
        interpolateFluidVelocityToParticles(currentFluid.velocity);
    auto pos = pd->getPos(access::gpu, access::readwrite);
    thrust::transform(thrust::cuda::par, pos.begin(), pos.end(),
                      velocities.xyz(), pos.begin(),
                      icm_compressible::MidStepEulerFunctor(dt));
  }
}

// Takes positions to n+1: \vec{q}^{n+1} = \vec{q}^n +
// dt/2\oper{J}^{n+1/2}(\vec{v}^n + \vec{v}^{n+1})
template <class Walls>
void ICM_Compressible_impl<Walls>::forwardPositionsToNextStep(
    const cached_vector<real4> &positionsAtN,
    const DataXYZ &fluidVelocitiesAtN) {
  System::log<System::DEBUG2>("[ICM_Compressible] Forward particles to n+1");
  auto fluidVelocitiesAtMidStep = icm_compressible::sumVelocities(
      fluidVelocitiesAtN, currentFluid.velocity);
  auto velocities =
      interpolateFluidVelocityToParticles(fluidVelocitiesAtMidStep);
  auto pos = pd->getPos(access::gpu, access::readwrite);
  thrust::transform(thrust::cuda::par, positionsAtN.begin(), positionsAtN.end(),
                    velocities.xyz(), pos.begin(),
                    icm_compressible::MidStepEulerFunctor(dt));
}

template <class Walls> void ICM_Compressible_impl<Walls>::forwardTime() {
