...
public:
  ...
  // Add to each particle (via ParticleData::getEnergy) the energy due to the
  // Integrator (typically the kinetic energy)
  virtual real sumEnergy() { return 0.0; }

  // Add an Interactor to the Integrator.
  // This also adds it as an updatable, so there is no need to also call
  // addUpdatable for Interactors.
  ...
  // Get a list of the ParameterUpdatables that have been added to the
  // Integrator Note that this also includes the Interactors
  auto getUpdatables() {
