/*Raul P. Pelaez 2017-2021. Verlet NVT Integrator module.
  ...
  verlet MD algorithm that conserves the energy, volume and number of particles.
  ...
  VerletNVE::Parameters par;
     par.energy = 1.0; //Target energy per particle, will be ignored if
  initVelocities=false par.dt = 0.01; par.is2D = false; par.initVelocities=true;
  //Modify starting velocities to ensure the target energy, default is true
...
VerletNVE::VerletNVE(shared_ptr<ParticleGroup> pg, VerletNVE::Parameters par)
  ...
  if (initVelocities)
    sys->log<System::MESSAGE>("[VerletNVE] Target energy per particle: %g",
                              energy);
  else
    sys->log<System::MESSAGE>(
        "[VerletNVE] Not fixing an initial per particle energy.");
  ...
  if (pd->isVelAllocated() && initVelocities) {
    sys->log<System::WARNING>("[VerletNVE] Velocity will be overwritten to "
...
VerletNVE::~VerletNVE() { cudaStreamDestroy(stream); }
...
void VerletNVE::initializeVelocities() {
  ...
  int numberParticles = pg->getNumberParticles();
  {
    ...
    thrust::fill(thrust::cuda::par, energy_gr, energy_gr + numberParticles,
                 real(0.0));
  }
  ...
  }
  {
    ...
    auto energy_gr = pg->getPropertyIterator(energy);
    U += thrust::reduce(thrust::cuda::par, energy_gr,
                        energy_gr + numberParticles);
  }
  U = U / numberParticles;
  real K = energy - U;
  if (K < 0) {
    sys->log<System::ERROR>("[VerletNVE] Cannot fix requested energy per "
                            "particle. Requested E = U + K = %g, but U=%g",
                            energy, U);
  ...
  auto mass =
      pd->getMassIfAllocated(access::location::cpu, access::mode::read).raw();
  auto gindex = pg->getIndexIterator(access::cpu);
  std::transform(gindex, gindex + pg->getNumberParticles(), vel_gr, [&](int i) {
...
template <int step> void VerletNVE::callIntegrate() {
  ...
  VerletNVE_ns::integrateGPU<step><<<Nblocks, Nthreads, 0, stream>>>(
...
void VerletNVE::resetForces() {
  ...
  thrust::fill(thrust::cuda::par.on(stream), force_gr,
...
__global__ void sumKineticEnergy(const VelocityIterator vel,
                                 EnergyIterator energy, const MassIterator mass,
                                 int numberParticles) {
...
real VerletNVE::sumEnergy() {
  ...
  auto energy_gr = pg->getPropertyIterator(
      pd->getEnergy(access::location::gpu, access::mode::write));
  int Nthreads = 128;
  int Nblocks =
      numberParticles / Nthreads + ((numberParticles % Nthreads) ? 1 : 0);
  ...
  if (defaultMass > 0) {
    ...
    verletnve_ns::sumKineticEnergy<<<Nblocks, Nthreads>>>(
        vel_gr, energy_gr, mass_iterator, numberParticles);
  } else {
