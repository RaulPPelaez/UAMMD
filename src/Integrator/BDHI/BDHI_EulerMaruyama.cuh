/*Raul P. Pelaez 2016. Brownian Euler Maruyama with hydrodynamics Integrator
derived class implementation
  ...
  Solves the following stochastic differential equation:
   ...
   Being:
     ...
     B - B*B^T = M -> i.e Cholesky decomposition B=chol(M) or Square root
B=sqrt(M) div(M) - Divergence of the mobility matrix, only non-zero in 2D
...
space. [2]
  ...
  BDHI::FCM:
  -Fluctuating Froce Coupling Method, also spectral but computing only long
range [3]
...
Brownian dynamics simulations J. Chem. Phys. 137, 064106 (2012);
doi: 10.1063/1.4742347 [2] Rapid sampling of stochastic displacements in
Brownian dynamics simulations The Journal of Chemical Physics 146, 124116
(2017); doi: http://dx.doi.org/10.1063/1.4978242 [3] Fluctuating force-coupling
method for simulations of colloidal suspensions; J. Comp. Phys. 269 (2014); doi:
https://doi.org/10.1016/j.jcp.2014.03.013
...
/*-----------------------------INTEGRATOR
 * CLASS----------------------------------*/
template <class Method> class EulerMaruyama : public Integrator {
public:
  ...
  real getHydrodynamicRadius() { return bdhi->getHydrodynamicRadius(); }

  auto getScheme() { return bdhi; }
