/*Raul P. Pelaez 2018-2021. Fluctuating Immerse Boundary for Brownian Dynamics
...
par.temperature = 1.0;
par.viscosity = 1.0;
//The hydrodynamic radius is given by ~0.91*Lbox/cellDimension in FIB, so the
...
//The MIDPOINT scheme is faster as it requires only one Stokes solve and thus
half the FFTs as the improved one. Furthemore the simple midpoint scheme
computes particle forces only one time as opposed to two times in the improved
scheme. Both algorithms are described in [1].
...
The incompressible fluctuating Navier-Stokes equation in the creeping flow
regime can be written like this for the fluid velocity:

  \vec{v_fluid} = 1/\eta\mathcal{L}^-1\vec{g} -> fluid velocity

  Where \mathcal{L} is an operator coming from a proyection method that allows
to solve the velocity without computing or storing the pressure and can be
written as:
  \mathcal{L}^-1 = -L^{-1}P
...
inline __device__ __host__ cufftComplex3 operator+(cufftComplex3 a,
                                                   const cufftComplex3 &b) {
  return {a.x + b.x, a.y + b.y, a.z + b.z};
}
inline __device__ __host__ void operator+=(cufftComplex3 &a,
                                           const cufftComplex3 &b) {
  a.x += b.x;
  a.y += b.y;
  a.z += b.z;
}
inline __device__ __host__ cufftComplex3 operator*(const real &a,
                                                   const cufftComplex3 &b) {
  return {a * b.x, a * b.y, a * b.z};
}
inline __device__ __host__ cufftComplex3 operator*(const cufftComplex3 &b,
                                                   const real &a) {
  return a * b;
}
...
public:
  ...
  real getCellSize() { return grid.cellSize.x; }
