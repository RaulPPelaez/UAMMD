/*Raul P. Pelaez 2018. Fluctuating Immerse Boundary for Brownian Dynamics with
Hydrodynamic Interactions.

This file implements the algorithm described in [1] for PBC using FFT to solve
the stokes operator. Fluid properties are stored in a staggered grid for
improved translational invariance [2]. This module solves the same regime as the
rest of the BDHI modules, but without imposing a mobility kernel. FIB solves the
fluctuating Navier-Stokes equation directly.

See FIB.cuh for more info.

References:
[1] Brownian Dynamics without Green's Functions. Steve Delong, Florencio Balboa,
et. al. 2014 [2] Staggered Schemes for Fluctuating Hydrodynamics. Florencio
Balboa, et.al. 2012.
 */
#include "FIB.cuh"
...
FIB::FIB(shared_ptr<ParticleGroup> pg, Parameters par)
    : Integrator(pg, "BDHI::FIB"), dt(par.dt), temperature(par.temperature),
      viscosity(par.viscosity), box(par.box), scheme(par.scheme) {
  CudaCheckError();
  sys->log<System::MESSAGE>("[BDHI::FIB] Initialized");
  seed = sys->rng().next();
  int numberParticles = pg->getNumberParticles();
  if (par.hydrodynamicRadius > 0 and par.cells.x > 0)
    sys->log<System::CRITICAL>("[BDHI::FIB] Please provide hydrodynamic radius "
                               "OR cell dimensions, not both.");
  int3 cellDim = par.cells;
  // If cells were not provided compute the closest one for the requested Rh
  if (par.cells.x < 0) {
    if (par.hydrodynamicRadius < 0)
      sys->log<System::CRITICAL>("[BHDI::FIB] I need either the hydrodynamic "
  ...
  if (par.cells.x > 0)
    cellDim = par.cells;
  if (par.hydrodynamicRadius > 0)
    sys->log<System::MESSAGE>("[BDHI::FIB] Target hydrodynamic radius: %f",
  ...
  sys->log<System::MESSAGE>(
      "[BDHI::FIB] Closest possible hydrodynamic radius: %f", rh);
  sys->log<System::MESSAGE>("[BDHI::FIB] Box Size: %f %f %f", box.boxSize.x,
                            box.boxSize.y, box.boxSize.z);
  sys->log<System::MESSAGE>("[BDHI::FIB] Mesh dimensions: %d %d %d", cellDim.x,
                            cellDim.y, cellDim.z);
  sys->log<System::MESSAGE>("[BDHI::FIB] Temperature: %f", temperature);
  sys->log<System::MESSAGE>("[BDHI::FIB] Self mobility: %f",
                            this->getSelfMobility());
  if (box.boxSize.x != box.boxSize.y || box.boxSize.y != box.boxSize.z ||
      box.boxSize.x != box.boxSize.z) {
    sys->log<System::WARNING>(
        "[BDHI::FCM] Self mobility will be different for non cubic boxes!");
  }
  sys->log<System::MESSAGE>("[BDHI::FIB] Random Finite Diference delta: %e",
                            deltaRFD);
  sys->log<System::MESSAGE>("[BDHI::FIB] dt: %f", dt);
  if (scheme == Scheme::MIDPOINT)
    sys->log<System::MESSAGE>(
        "[BDHI::FIB] Temporal integrator: Simple midpoint");
  else if (scheme == Scheme::IMPROVED_MIDPOINT)
    sys->log<System::MESSAGE>(
        "[BDHI::FIB] Temporal integrator: Improved midpoint");
  ...
  try {
    ...
    sys->log<System::CRITICAL>(
        "[BDHI::FIB] Thrust could not allocate necessary arrays at "
  ...
  sys->log<System::DEBUG>("[BDHI::FIB] cuFFT grid size: %d %d %d", cdtmp.x,
                          cdtmp.y, cdtmp.z);
  ...
  sys->log<System::DEBUG>("[BDHI::FIB] Necessary work space for cuFFT: %s",
                          printUtils::prettySize(cufftWorkSize).c_str());
  size_t free_mem, total_mem;
  CudaSafeCall(cudaMemGetInfo(&free_mem, &total_mem));
  if (free_mem < cufftWorkSize) {
    sys->log<System::CRITICAL>("[BDHI::FIB] Not enough memory in device to "
  ...
  try {
    ...
    sys->log<System::CRITICAL>(
        "[BDHI::FIB] Thrust could not allocate cufft work area of size: %s "
        "(free memory: %s), with error: %s",
        printUtils::prettySize(cufftWorkSize).c_str(),
        printUtils::prettySize(free_mem).c_str(), e.what());
  }
  try {
    ...
    /*The layout of this array is
      fx000, fy000, fz000, fx001, fy001, fz001..., fxnnn, fynnn, fznnn.
      n=ncells-1 When used in real space each f is a real number, whereas in
      wave space each f will be a complex number.
    */
...
__global__ void addRandomAdvection(
    real3 *gridVels, // Real space velocities of each cell, defined at the cell
                     // faces
  ...
  for (int i = 0; i < numberNeighbourCells; i++) {
    ...
    // Contribution to vx
    {
      ...
      real SmSdWx;
      {
        ...
        const auto r = grid.box.apply_pbc(
...
template <class Grid>
__device__ int3 computeSupportShift(real3 pos, int3 celli, Grid grid,
                                    int support) {
  int3 P = make_int3(support / 2);
  // Kernels with even support might need an offset of one cell depending on the
  // position of the particle inside the cell
  ...
  if (shift) {
    ...
    P -= make_int3((pi_pbc + grid.box.boxSize * real(0.5)) * invCellSize -
                   make_real3(celli) + real(0.5));
  }
  return P;
}
...
__global__ void spreadParticleForces(real4 *pos, real4 *force, real3 *gridVels,
                                     ...
                                     int numberParticles, Kernel kernel) {
  ...
  for (int i = 0; i < numberNeighbourCells; i++) {
    // Contribution to vx
    {
      ...
      int3 celljx = make_int3(cellix.x + i % supportCells,
                              ...
                              cellix.z + i / (supportCells * supportCells)) -
                    Px;
      celljx = grid.pbc_cell(celljx);
      const int jcellx = grid.getCellIndex(celljx);
      /*Staggered distance from q - noise to center of cell j*/
      const real3 r = grid.distanceToCellCenter(
          pi - make_real3(real(0.5) * grid.cellSize.x, 0, 0), celljx);
      // Spread Wx
    ...
    // Contribution to vy
    {
      int3 celljy = make_int3(celliy.x + i % supportCells,
                              ...
                              celliy.z + i / (supportCells * supportCells)) -
                    Py;
      celljy = grid.pbc_cell(celljy);
      const int jcelly = grid.getCellIndex(celljy);
      const real3 r = grid.distanceToCellCenter(
          pi - make_real3(0, real(0.5) * grid.cellSize.y, 0), celljy);
    ...
    // Contribution to vz
    {
      int3 celljz = make_int3(celliz.x + i % supportCells,
                              ...
                              celliz.z + i / (supportCells * supportCells)) -
                    Pz;
      celljz = grid.pbc_cell(celljz);
      const int jcellz = grid.getCellIndex(celljz);
      const real3 r = grid.distanceToCellCenter(
          pi - make_real3(0, 0, real(0.5) * grid.cellSize.z), celljz);
...
template <Step mode = Step::PREDICTOR,
          class Kernel = IBM_kernels::Peskin::threePoint>
__global__ void
midPointStep(real4 *pos, // q^n in predictor and euler, q^{n+1/2} in corrector
             real4 *posOld, // empty in predictor and euler, q^n in corrector
             const real3 *gridVels, Grid grid, Kernel kernel, real dt,
             int numberParticles) {
  int id = blockIdx.x * blockDim.x + threadIdx.x;
  if (id >= numberParticles)
    return;
  ...
  const real3 cellPosOffset = real(0.5) * (grid.cellSize - grid.box.boxSize);
  constexpr int supportCells = Kernel::support;
  const real3 pix = make_real3(posCurrent.x - real(0.5) * grid.cellSize.x,
                               posCurrent.y, posCurrent.z);
  const int3 cellix = grid.getCell(pix);
  const int3 Px = computeSupportShift(pix, cellix, grid, supportCells);
  const real3 piy = make_real3(
      posCurrent.x, posCurrent.y - real(0.5) * grid.cellSize.y, posCurrent.z);
  const int3 celliy = grid.getCell(piy);
  const int3 Py = computeSupportShift(piy, celliy, grid, supportCells);
  const real3 piz = make_real3(posCurrent.x, posCurrent.y,
                               posCurrent.z - real(0.5) * grid.cellSize.z);
  const int3 celliz = grid.getCell(piz);
  const int3 Pz = computeSupportShift(piz, celliz, grid, supportCells);
  real prefactor = dt;
  ...
  for (int i = 0; i < numberNeighbourCells; i++) {
    ...
    }
    {
      ...
      const real3 r = grid.distanceToCellCenter(
          posCurrent - make_real3(0, real(0.5) * grid.cellSize.y, 0), celljy);
    ...
    }
    {
      ...
      const real3 r = grid.distanceToCellCenter(
          posCurrent - make_real3(0, 0, real(0.5) * grid.cellSize.z), celljz);
...
void FIB::spreadParticleForces() {
  ...
  FIB_ns::spreadParticleForces<<<nblocks, nthreads>>>(
...
void FIB::randomAdvection(real noisePrefactor) {
  ...
  FIB_ns::addRandomAdvection<<<NblocksCells, NthreadsCells>>>(
      d_gridVels, grid, noisePrefactor,
...
void FIB::thermalDrift() {
  ...
  auto pos = pd->getPos(access::location::gpu, access::mode::read);
  FIB_ns::addThermalDrift<<<Nblocks, Nthreads>>>(
      pos.raw(), d_gridVels, grid, driftPrefactor, deltaRFD, numberParticles,
...
void FIB::applyStokesSolutionOperator() {
  cufftReal *d_gridVels =
      (cufftReal *)thrust::raw_pointer_cast(gridVels.data());
  ...
  FIB_ns::solveStokesFourier<<<NblocksCells, NthreadsCells>>>(
      (cufftComplex3 *)d_gridVelsFourier, (cufftComplex3 *)d_gridVelsFourier,
      viscosity, grid);
...
void FIB::predictorStep() {
  ...
  FIB_ns::midPointStep<FIB_ns::Step::PREDICTOR><<<Nblocks, Nthreads>>>(
      pos.raw(), d_posOld, d_gridVels, grid, *kernel, dt, numberParticles);
}
...
void FIB::correctorStep() {
  ...
  real4 *d_posOld = thrust::raw_pointer_cast(posOld.data());
  FIB_ns::midPointStep<FIB_ns::Step::CORRECTOR><<<Nblocks, Nthreads>>>(
      pos.raw(), d_posOld, d_gridVels, grid, *kernel, dt, numberParticles);
}
// Updates positions to the next step according to the current fluid velocities
// q^{n+1} = q^n + dt J^n v
void FIB::eulerStep() {
  ...
  auto pos = pd->getPos(access::location::gpu, access::mode::readwrite);
  real4 *d_posOld = thrust::raw_pointer_cast(posOld.data());
  FIB_ns::midPointStep<FIB_ns::Step::EULER><<<Nblocks, Nthreads>>>(
      pos.raw(), d_posOld, d_gridVels, grid, *kernel, dt, numberParticles);
}

// forwards the simulation to the next time step with the simple midpoint scheme
// in [1]
void FIB::forwardMidpoint() {
  ...
  // kT/\delta [ S(q^{n+1/2} + \delta/2\hat{\bf{W}}^{n+1/2}) - S(q^{n+1/2} -
  // \delta/2\hat{\bf{W}}^{n+1/2})]\hat{\bf{W}}^{n+1/2} Spread thermal drift
  // with RFD
  thermalDrift();
...
// forwards the simulation to the next time step with the improved midpoint
// scheme in [1]
void FIB::forwardImprovedMidpoint() {
  ...
  // sqrt(vis·kT/(dt·dV))·\hat{D}(\bf{W}^{n,1}+\bf{W}^{n,2})
  {
    ...
    if (temperature != real(0.0))
      CurandSafeCall(
          curandgeneratenormal(curng, thrust::raw_pointer_cast(random.data()),
                               random.size(), 0.0, 1.0));
    randomAdvection(noisePrefactor);
  }
  sys->log<System::DEBUG2>("[BDHI::FIB] Thermal drift");
