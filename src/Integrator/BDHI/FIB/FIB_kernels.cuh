...
public:
  ...
  real fixHydrodynamicRadius(real hydrodynamicRadius, real h) const {
    real upsampling = computeUpsampling(2 * bm.alpha);
    return h / upsampling;
  }

  __host__ __device__ real phi(real r) const { return bm.phi(r / a) / a; }
...
public:
  ...
  __host__ __device__ real phi(real r) const { return kern.phi(r); }
...
public:
  ...
  __host__ __device__ real phi(real r) const { return kern.phi_tabulated(r); }
};
} // namespace GaussianFlexible
