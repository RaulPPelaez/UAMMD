...
struct euler_functor {
  ...
  template <class vec>
  __host__ __device__ real4 operator()(vec vel, real4 pos) const {
    vel.w = 0;
    return pos + vel * dt;
  }
};
...
public:
  ...
  DPStokes(shared_ptr<ParticleData> pd, shared_ptr<System> sys, Parameters par)
      : DPStokes(pd, std::make_shared<ParticleGroup>(pd, sys, "All"), sys,
                 par) {}

  DPStokes(shared_ptr<ParticleData> pd, shared_ptr<ParticleGroup> pg,
           shared_ptr<System> sys, Parameters par)
      : Integrator(pd, pg, sys, "BDHI::DPStokes"), dt(par.dt) {
...
private:
  ...
  void computeForces() {
    sys->log<System::DEBUG2>("[DPStokes] Compute particle forces");
    {
      ...
      thrust::fill(thrust::cuda::par.on(st), forces.begin(), forces.end(),
                   real4());
    }
  ...
  cached_vector<real4> computeParticleDisplacements() {
    ...
    auto velocities =
        dpstokes->Mdot(pos.begin(), force.begin(), numberParticles, st);
    return velocities;
  }
  ...
  void updatePositions(cached_vector<real4> &particleVelsAndPressure) {
    ...
    thrust::transform(thrust::cuda::par.on(st), particleVelsAndPressure.begin(),
                      particleVelsAndPressure.end(), pos.begin(), pos.begin(),
