...
template <class Kernel, class KernelTorque> class FCM_impl {
public:
  ...
  FCM_impl(Parameters par)
      : par(par), viscosity(par.viscosity),
        hydrodynamicRadius(par.hydrodynamicRadius), box(par.box),
        grid(par.box, par.cells), kernel(par.kernel),
    ...
    if (box.boxSize.x == real(0.0) && box.boxSize.y == real(0.0) &&
        box.boxSize.z == real(0.0)) {
      System::log<System::CRITICAL>(
          "[BDHI::FCM] Box of size zero detected, cannot work without a box! "
    ...
    if (par.cells.x <= 0) {
      System::log<System::EXCEPTION>(
          "FCM_impl requires a valid grid dimension");
      throw std::runtime_error("Invalid arguments");
    }
    if (not par.kernel or not par.kernelTorque) {
      System::log<System::EXCEPTION>(
          "FCM_impl requires instances of the spreading kernels");
      throw std::runtime_error("Invalid arguments");
    }
    printMessages(par);
    initCuFFT();
  ...
  real getHydrodynamicRadius() { return hydrodynamicRadius; }
...
private:
  ...
  void printMessages(Parameters par) {
    ...
    System::log<System::MESSAGE>("[BDHI::FCM] Box Size: %g %g %g",
                                 grid.box.boxSize.x, grid.box.boxSize.y,
                                 grid.box.boxSize.z);
    System::log<System::MESSAGE>("[BDHI::FCM] Grid dimensions: %d %d %d",
                                 grid.cellDim.x, grid.cellDim.y,
                                 grid.cellDim.z);
    // System::log<System::MESSAGE>("[BDHI::FCM] Interpolation kernel support:
    // %g rh max distance, %d cells total",
    // 			     kernel->support*0.5*grid.cellSize.x/rh,
    // kernel->support);
    System::log<System::MESSAGE>("[BDHI::FCM] h: %g %g %g", grid.cellSize.x,
                                 grid.cellSize.y, grid.cellSize.z);
    // System::log<System::MESSAGE>("[BDHI::FCM] Requested kernel tolerance:
    // %g", par.tolerance);
  }
  ...
  void initCuFFT() {
    ...
    System::log<System::DEBUG>("[BDHI::FCM] cuFFT grid size: %d %d %d", cdtmp.x,
                               cdtmp.y, cdtmp.z);
    ...
    System::log<System::DEBUG>("[BDHI::FCM] Necessary work space for cuFFT: "
                               "%s, available: %s, total: %s",
    ...
    if (free_mem < cufftWorkSize) {
      System::log<System::EXCEPTION>(
          "[BDHI::FCM] Not enough memory in device to allocate cuFFT free %s, "
          "needed: %s!!",
          printUtils::prettySize(free_mem).c_str(),
          printUtils::prettySize(cufftWorkSize).c_str());
...
struct ToReal3 {
  template <class vtype> inline __device__ real3 operator()(vtype q) {
    return make_real3(q);
  }
};
...
spreadForces(IterPos &pos, IterForce &force, int numberParticles,
  ...
  thrust::fill(thrust::cuda::par.on(st), gridVels.begin(), gridVels.end(),
               real3());
...
auto getCoordinateVector(Container &v, int coord) {
  ...
  auto permit = thrust::make_transform_iterator(
      thrust::make_counting_iterator(0), Perm());
...
struct Zip23 {
  template <class Zip3> __host__ __device__ complex3 operator()(Zip3 a) {
...
__global__ void addTorqueCurl(complex3 *gridTorquesFourier,
...
void addSpreadTorquesFourier(IterPos &pos, IterTorque &torque,
  ...
  thrust::fill(thrust::cuda::par.on(st), gridTorques.begin(), gridTorques.end(),
               real3());
  IBM<Kernel> ibm(kernel, grid, IBM_ns::LinearIndex3D(nx, n.y, n.z));
  ibm.spread(pos, torque_r3, d_gridTorques3, numberParticles, st);
  auto gridTorquesFourier =
      forwardTransform(gridTorques, grid.cellDim, plan, st);
  int BLOCKSIZE = 128;
  int numberCells = n.z * n.y * (n.x / 2 + 1);
  uint Nthreads = BLOCKSIZE < numberCells ? BLOCKSIZE : numberCells;
  uint Nblocks =
      numberCells / Nthreads + ((numberCells % Nthreads != 0) ? 1 : 0);
  auto d_gridVelsFourier =
      (complex3 *)thrust::raw_pointer_cast(gridVelsFourier.data());
  auto d_gridTorquesFourier3 =
      (complex3 *)thrust::raw_pointer_cast(gridTorquesFourier.data());
  addTorqueCurl<<<Nblocks, Nthreads, 0, st>>>(d_gridTorquesFourier3,
...
forceFourier2Vel(const complex3 *gridForces, /*Input array*/
  ...
  if (id >= (ncells.z * ncells.y * (ncells.x / 2 + 1)))
    return;
  ...
  gridVels[id] = projectFourier(k2, dk, factor) *
...
void convolveFourier(cached_vector<complex3> &gridVelsFourier, real viscosity,
  ...
  forceFourier2Vel<<<Nblocks, Nthreads, 0, st>>>(
      d_gridVelsFourier, d_gridVelsFourier, viscosity, grid);
  CudaCheckError();
}
...
__global__ void fourierBrownianNoise(complex3 *gridVelsFourier, Grid grid,
  ...
  if (id >= (nk.z * nk.y * (nk.x / 2 + 1)))
    return;
  const int3 cell = make_int3(id % (nk.x / 2 + 1), (id / (nk.x / 2 + 1)) % nk.y,
                              id / ((nk.x / 2 + 1) * nk.y));
  /*cuFFT R2C and C2R only store half of the innermost dimension, the one that
    varies the fastest The input of R2C is real and the output of C2R is real.
    The only way for this to be true is if v_k={i,j,k} = v*_k{N-i, N-j, N-k}
    So the conjugates are redundant and the is no need to compute them nor store
    them except on two exceptions. In this scheme, the only cases in which v_k
    and v_{N-k} are stored are: 1- When the innermost dimension coordinate is 0.
  ...
  */
  ...
  /*The special cases k_i=0 do not have conjugates, a.i N-k = N which is not
   * stored*/
  if (nyquist)
    return; // Nyquist points do not have conjugates
...
cached_vector<real3> inverseTransform(cached_vector<complex3> &gridFourier,
  ...
  thrust::fill(thrust::cuda::par.on(st), gridReal.begin(), gridReal.end(),
               real3());
  auto d_gridFourier = (complex *)thrust::raw_pointer_cast(gridFourier.data());
  auto d_gridReal = (real *)thrust::raw_pointer_cast(gridReal.data());
  cufftSetStream(plan, st);
...
interpolateVelocity(IterPos &pos, cached_vector<real3> &gridVels, Grid grid,
  ...
  thrust::fill(thrust::cuda::par.on(st), linearVelocities.begin(),
               linearVelocities.end(), real3());
...
// Compute the curl of the velocity, V, in Fourier space. This is equal to the
// angular velocity
//  0.5\nabla \times V = 0.5 (i*k_x i*k_y i*k_z)\times (V_x V_y V_z) =
//  = 0.5( i*k_y*V_z - i*k_z(V_y), i*k_z(V_x) - i*k_x*V_z, i*k_x*V_y -
//  i*k_y*V_x)
...
computeGridAngularVelocityFourier(cached_vector<complex3> &gridVelsFourier,
  ...
  auto d_gridAngVelsFourier =
      thrust::raw_pointer_cast(gridAngVelsFourier.data());
  computeVelocityCurlFourier<<<numberBlocks, blockSize, 0, st>>>(
      d_gridVelsFourier, d_gridAngVelsFourier, grid);
  CudaCheckError();
...
template <class IterPos, class Kernel>
cached_vector<real3>
interpolateAngularVelocity(IterPos &pos, cached_vector<real3> &gridAngVels,
  ...
  thrust::fill(thrust::cuda::par.on(st), angularVelocities.begin(),
               angularVelocities.end(), real3());
...
std::pair<cached_vector<real3>, cached_vector<real3>>
FCM_impl<Kernel, KernelTorque>::computeHydrodynamicDisplacements(
    real4 *pos, real4 *force, real4 *torque, int numberParticles,
  ...
  if (force) {
    ...
    gridVelsFourier =
        forwardTransform(gridVels, grid.cellDim, cufft_plan_forward, st);
  } else {
    const auto n = grid.cellDim;
    gridVelsFourier.resize((n.x / 2 + 1) * n.y * n.z);
    thrust::fill(thrust::cuda::par.on(st), gridVelsFourier.begin(),
                 gridVelsFourier.end(), complex3());
  }
  if (torque) {
  ...
  addBrownianNoise(gridVelsFourier, temperature, viscosity, prefactor, seed,
                   grid, st);
  cached_vector<real3> angularVelocities;
  if (torque) {
    auto gridAngVelFourier =
        computeGridAngularVelocityFourier(gridVelsFourier, grid, st);
    auto gridAngVel = inverseTransform(gridAngVelFourier, grid.cellDim,
                                       cufft_plan_inverse, st);
    angularVelocities = interpolateAngularVelocity(
        pos, gridAngVel, grid, kernelTorque, numberParticles, st);
  }
  auto gridVels =
      inverseTransform(gridVelsFourier, grid.cellDim, cufft_plan_inverse, st);
  auto linearVelocities =
      interpolateVelocity(pos, gridVels, grid, kernel, numberParticles, st);
  CudaCheckError();
