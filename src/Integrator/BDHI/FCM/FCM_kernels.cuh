/*Raul P. Pelaez 2020, Kernels (Window  functions) for FCM.
  These are adapted from kernels in the IBM module, they are adapted to provide
the resulting hydrodynamic radius and the support for a given tolerance.

TODO:
 100- Create the function:
      template<class Kernel>
      auto createKernelTorque(...);
      Given the translational kernel and a series of parameters, this function
should return a version of the kernel that can be used for rotation.
 */
...
public:
  int support;
  real rmax;
  Gaussian(real h, real tolerance) : kern(h * computeUpsampling(tolerance)) {
    const real dr = 0.5 * h;
    real r = dr;
  ...
  real fixHydrodynamicRadius(real hydrodynamicRadius, real h) { return a; }
...
public:
  int support;
  real rmax;
  GaussianTorque(real width, real h, real tolerance) : kern(width) {
    const real dr = 0.5 * h;
    real r = dr;
    while (kern.phi(r) > tolerance) {
      r += dr;
...
class BarnettMagland {
  ...
  static real computeUpsampling(real i_w) {
    // Empirical fit from the taylor expansion of the BM kernel, the first term
    // goes with x^2 Which allows to make a simil with a gaussian such that
    // \sigma = w/sqrt(beta)
...
public:
  ...
  BarnettMagland(real width, real h, real tolerance)
    ...
    System::log<System::EXCEPTION>(
        "BarnettMagland: This constructor cannot be called");
    throw std::runtime_error("Invalid constructor");
  }

  BarnettMagland(real h, real tolerance) : bm(initBM(tolerance)) {
    support = ceil(2 * bm.alpha);
    this->a = h;
  }

  BarnettMagland(real w, real alpha, real beta, real h) : bm(alpha, beta) {
    support = ceil(2 * bm.alpha);
    this->a = h;
  }
...
public:
  ...
  __host__ __device__ real phi(real r, real3 pos) const { return kern.phi(r); }
};
...
public:
  ...
  static constexpr real adviseGridSize(real hydrodynamicRadius,
                                       real tolerance) {
  ...
  __device__ real phi(real r, real3 pos) const { return kern.phi(r); }
};
} // namespace Peskin
...
public:
  ...
  __device__ real phi(real r, real3 pos) const { return kern.phi_tabulated(r); }
};
} // namespace GaussianFlexible
