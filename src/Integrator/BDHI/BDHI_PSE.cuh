/*Raul P. Pelaez 2017-2022. Positively Split Edwald Rotne-Prager-Yamakawa
...
tensor.
  ...
  and partitions the RPY tensor in two, positively defined contributions [1], so
that: M = Mr + Mw Mr - A real space short range contribution. Mw - A wave space
...
Notes:
Storing F and G functions in r^2 scale (a.i. table(r^2) = F(sqrt(r^2))) creates
artifacts due to the linear interpolation of a cuadratic scale, so it is best to
just store table(sqrt(r^2)) = F(r). The cost of the sqrt seems neglegible and
yields a really better accuracy.
...
public:
  ...
  void computeMFFarField(real3 *MF, cudaStream_t st) {
    ...
    farField->computeHydrodynamicDisplacements(pos.begin(), force.begin(), MF,
                                               numberParticles, temperature,
                                               1.0 / sqrt(dt), st);
  }
  ...
  void computeHydrodynamicDisplacements(real4 *force, real3 *MF,
    ...
    nearField->computeStochasticDisplacements(MF, temperature, noise_prefactor,
                                              st);
    auto pos = pg->getParticleData()->getPos(access::gpu, access::read);
    // Compute both deterministic and stochastic part in the far field
    farField->computeHydrodynamicDisplacements(pos.begin(), force, MF,
                                               numberParticles, temperature,
                                               noise_prefactor, st);
  }

  void setShearStrain(real newStrain) {
    System::log<System::DEBUG>("[BDHI::PSE] Shear strain changed to %g",
                               newStrain);
  ...
  real getSelfMobility() { return this->M0; }
