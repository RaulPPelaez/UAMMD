/*Raul P. Pelaez 2016-2021. Brownian Euler Maruyama with hydrodynamics
  ...
  1- Krylov subspace methods for computing hydrodynamic interactions in Brownian
  dynamics simulations J. Chem. Phys. 137, 064106 (2012); doi: 10.1063/1.4742347
  2- Rapid sampling of stochastic displacements in Brownian dynamics simulations
  The Journal of Chemical Physics 146, 124116 (2017); doi:
  http://dx.doi.org/10.1063/1.4978242
...
EulerMaruyama<Method>::EulerMaruyama(shared_ptr<ParticleGroup> pg,
  ...
  if (par.K.size() == 3) {
    ...
    real3 Kz = par.K[2];
    System::log<System::MESSAGE>(
        "[BDHI::EulerMaruyama] Shear Matrix: [ %f %f %f; %f %f %f; %f %f %f ]",
        Kx.x, Kx.y, Kx.z, Ky.x, Ky.y, Ky.z, Kz.x, Kz.y, Kz.z);
  }
  CudaSafeCall(cudaStreamCreate(&stream));
  MF.resize(numberParticles, real3());
  BdW.resize(numberParticles + 1, real3());
}
template <class Method> EulerMaruyama<Method>::~EulerMaruyama() {
...
integrateGPUD(real4 *__restrict__ pos, IndexIterator indexIterator,
              const real3 *__restrict__ MF, const real3 *__restrict__ BdW,
              const real3 *__restrict__ K, int N, real sqrt2Tdt, real T,
              real dt, bool is2D) {
...
template <class Method> void EulerMaruyama<Method>::resetForces() {
  int numberParticles = pg->getNumberParticles();
...
template <class Method> void EulerMaruyama<Method>::forwardTime() {
  ...
  real3 *d_BdW =
      (par.temperature > 0) ? thrust::raw_pointer_cast(BdW.data()) : nullptr;
  ...
  EulerMaruyama_ns::integrateGPUD<<<nblocks, nthreads, 0, stream>>>(
      pos.raw(), indexIter, d_MF, d_BdW, d_K,
      // d_divM,
      numberParticles, sqrt2Tdt, par.temperature, par.dt, par.is2D);
}

template <class Method> real EulerMaruyama<Method>::sumEnergy() {
