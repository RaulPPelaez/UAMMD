...
struct Gaussian{
	...
	for(int i = nz-1; i>=0; i--){
      ...
      __global__ void addCurlCheb(DataXYZPtr<complex> input,
				  DataXYZPtr<complex> curl,
				  real3 L, int3 n){
	...
	auto zToIndex =  make_third_index_iterator(thrust::make_counting_iterator(0),
						   ik.x, ik.y, Index3D(n.x/2+1, n.y, 2*n.z-2));
	...
	real alpha_d = -1;
      ...
      SpreadInterp(Grid grid, Parameters par):
	grid(grid){
	...
	return gridForce;
      ...
      void addSpreadTorquesFourier(PosIterator pos, TorqueIterator torques, int numberParticles,
	...
	CudaCheckError();
      ...
      const int3 n = grid.cellDim;
      IBM<Kernel, Grid> ibm(kernel, grid, IBM_ns::LinearIndex3D(2*(n.x/2+1), n.y, n.z));
      ibm.gather(pos, particleVels.x(), gridData.x(),
		 *qw, IBM_ns::DefaultWeightCompute(),
		 numberParticles, st);
      ibm.gather(pos, particleVels.y(), gridData.y(),
		 *qw, IBM_ns::DefaultWeightCompute(),
		 numberParticles, st);
      ibm.gather(pos, particleVels.z(), gridData.z(),
		 *qw, IBM_ns::DefaultWeightCompute(),
		 numberParticles, st);
      CudaCheckError();
	...
	detail::addCurlCheb<<<numberBlocks, blockSize, 0, st>>>(DataXYZPtr<complex>(gridVelsChebCopy),
								DataXYZPtr<complex>(gridAngVelsCheb),
								grid.box.boxSize, n);
	CudaCheckError();
	...
	ibm.gather(pos, particleAngVels.x(), gridAngVel.x(),
		   *qw, IBM_ns::DefaultWeightCompute(),
		   numberParticles, st);
	ibm.gather(pos, particleAngVels.y(), gridAngVel.y(),
		   *qw, IBM_ns::DefaultWeightCompute(),
		   numberParticles, st);
	ibm.gather(pos, particleAngVels.z(), gridAngVel.z(),
		   *qw, IBM_ns::DefaultWeightCompute(),
		   numberParticles, st);
	CudaCheckError();
	...
	this->kernelTorque = std::make_shared<KernelTorque>(par.w_d, par.beta_d, par.alpha_d,
							    grid.cellSize.x, H, grid.cellDim.z);
