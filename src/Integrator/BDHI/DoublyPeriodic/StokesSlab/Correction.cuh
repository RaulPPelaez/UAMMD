...
__global__ void scaleFFTToForwardChebyshevTransform(complex *signal, int nz,
                                                    int nbatch) {
  const int id = blockIdx.x * blockDim.x + threadIdx.x;
  if (id >= nbatch * nz)
    return;
...
// For bottom wall x or y velocity have the BCs, We only need k=0 but I left the
// BCs generic for any k u(k=0, z=0) = 0 and u'(k=0, z=H) = 0 For slit channel
// we have u(k=0, z=0,H) = 0 In both cases z velocity is 0: w(k=0, z) = 0
...
public:
  ...
  real getFirstIntegralFactor() const { return (k != 0) ? H : 1.0; }

  real getSecondIntegralFactor() const { return k != 0 ? (H * H * k) : (0.0); }
};
...
// Solves the zero mode of the BVP y''= fn with Boundary conditions given by C
// and D C and D are the Boundary condition matrices an is temporal storage for
// the cheb coefficients of y''
__global__ void correctZeroMode(FluidPointers<complex> correction,
                                DataXYZPtr<complex> gridForce, complex *an,
                                real4 D, real *C, int nkx, int nky, int nz,
                                real H, real mu) {
  ...
  { // X VELOCITY
    ...
    for (int i = 0; i < nz; i++) {
      corr[i] =
          si.computeSecondIntegralCoefficient(i, an, c0d0.first, c0d0.second) *
          H * H;
    }
  }
  { // Y VELOCITY
    ...
    BVP::SecondIntegralMatrix si(nz);
    auto vycorr =
        make_third_index_iterator(correction.velocityY, 0, 0, indexer);
    auto vzcorr =
        make_third_index_iterator(correction.velocityZ, 0, 0, indexer);
    auto pcorr = make_third_index_iterator(correction.pressure, 0, 0, indexer);
    for (int i = 0; i < nz; i++) {
      vycorr[i] =
          si.computeSecondIntegralCoefficient(i, an, c0d0.first, c0d0.second) *
          H * H;
      vzcorr[i] *= 0; // Z velocity zero mode correction is zero in both bottom
                      // and slit modes
...
// Set the zero mode of the inside solution to zero. It is used to store the
// grid forces for the zero mode before the correction
__global__ void cleanSolutionZeroMode(complex4 *insideSolution, int nkx,
                                      int nky, int nz) {
  int id = blockIdx.x * blockDim.x + threadIdx.x;
  if (id != 0)
    return;
  auto fn = make_third_index_iterator(insideSolution, 0, 0,
...
void sumCorrectionToInsideSolution(FluidData<complex> &correction,
                                   FluidData<complex> &insideSolution, int size,
                                   cudaStream_t st) {
  System::log<System::DEBUG>("Sum correction to solution");
  sumRange(insideSolution.pressure.begin(), correction.pressure.begin(), size,
           st);
...
class Correction {
  ...
  FluidData<complex> computeAnalyticalCorrectionFourierSpace(
      const FluidData<complex> &insideSolution, cudaStream_t st);
...
public:
  ...
  Correction(Parameters par)
      : Correction(par.H, par.Lxy, par.cells, par.viscosity, par.mode) {}

  ~Correction() { cufftDestroy(cufft_plan_forward); }

  void correctSolution(FluidData<complex> &insideSolution,
                       DataXYZ<complex> &gridForces, cudaStream_t st) {
    System::log<System::DEBUG>("Computing correction");
    auto analyticalCorrection =
        computeAnalyticalCorrectionFourierSpace(insideSolution, st);
    ...
    sumCorrectionToInsideSolution(analyticalCorrection, insideSolution, size,
                                  st);
  }
...
private:
  void chebyshevTransform(complex *d_data, cudaStream_t st) {
    ...
    correction_ns::periodicExtension<<<nblocks, blockSize, 0, st>>>(
        d_data, n.z, (n.x / 2 + 1) * n.y);
    ...
    correction_ns::
        scaleFFTToForwardChebyshevTransform<<<nblocks, blockSize, 0, st>>>(
            d_data, n.z, (n.x / 2 + 1) * n.y);
    CudaCheckError();
  }

  void takeAnalyticalCorrectionToChebyshevSpace(
      FluidData<complex> &analyticalCorrection, cudaStream_t st) {
  ...
  void correctZeroMode(FluidData<complex> &correction,
                       DataXYZ<complex> &gridForces, cudaStream_t st) {
    System::log<System::DEBUG>("Correcting zero mode");
    // Only the zero mode BVP is needed for the correction, in this case the
    // second integral matrix is the identity Additionally only the velocities
    // in the plane have to be corrected The equation to solve is y'' = f
    // Furthermore the BCs in this case are that either y(k=0, z=0,H) = 0 (slit
    // channel) or  y'(k=0, z=H) =0 (bottom wall) In both cases alpha and beta
    // are zero, so the system:
    //  (C*invA*B - D) (c0; d0) = C*invA*f - (alpha;beta) is simplified to:
    //  -D*(c0;d0) = C*f
    // We just need to compute the BCs, solve the 2x2 system above to get c0 and
    // d0 and then use them to integrate y'' twice.
    auto n = cells;
    ...
    correction_ns::correctZeroMode<<<1, 1, 0, st>>>(
        correction.getPointers(), DataXYZPtr<complex>(gridForces), an_ptr,
        zeroModeDMatrix, C_ptr, n.x, n.y, n.z, 0.5 * H, viscosity);
...
__global__ void fillRightHandSide(complex *rhs,
  ...
  auto vx =
      make_third_index_iterator(insideSolution.velocityX, ik.x, ik.y, indexer);
  auto vy =
      make_third_index_iterator(insideSolution.velocityY, ik.x, ik.y, indexer);
  auto vz =
      make_third_index_iterator(insideSolution.velocityZ, ik.x, ik.y, indexer);
...
__global__ void
computeAnalyticalCorrectionFourierSlit(FluidPointers<complex> correction,
  ...
  auto indexer = Index3D(n.x / 2 + 1, n.y, n.z);
  auto pcorr =
      make_third_index_iterator(correction.pressure, ik.x, ik.y, indexer);
  auto vxcorr =
      make_third_index_iterator(correction.velocityX, ik.x, ik.y, indexer);
  auto vycorr =
      make_third_index_iterator(correction.velocityY, ik.x, ik.y, indexer);
  auto vzcorr =
      make_third_index_iterator(correction.velocityZ, ik.x, ik.y, indexer);
  ...
  for (int i = 0; i < n.z; i++) {
    const real z = H - (real(-0.5) * H * cospi(i / real(n.z - 1)) +
                        real(0.5) * H); // from H to 0
    const real ekzmH = exp(k * (z - H));
    const real ekz = exp(-k * z);
    vxcorr[i] = {C[0].y * kvec.x * halfmu / k * z * ekz -
                     C[1].y * kvec.x * halfmu / k * z * ekzmH + C[2].x * ekz +
                     C[3].x * ekzmH,
                 -C[0].x * kvec.x * halfmu / k * z * ekz +
                     C[1].x * kvec.x * halfmu / k * z * ekzmH + C[2].y * ekz +
                     C[3].y * ekzmH};
    vycorr[i] = {C[0].y * kvec.y * halfmu / k * z * ekz -
                     C[1].y * kvec.y * halfmu / k * z * ekzmH + C[4].x * ekz +
                     C[5].x * ekzmH,
                 -C[0].x * kvec.y * halfmu / k * z * ekz +
                     C[1].x * kvec.y * halfmu / k * z * ekzmH + C[4].y * ekz +
                     C[5].y * ekzmH};
    vzcorr[i] = C[0] * halfmu * z * ekz + C[1] * halfmu * z * ekzmH +
                C[6] * ekz + C[7] * ekzmH;
    pcorr[i] = C[0] * z * ekz + C[1] * ekzmH;
  }
}

__global__ void computeAnalyticalCorrectionFourierBottomWall(
    FluidPointers<complex> correction,
    const FluidPointers<complex> insideSolution, real H, real2 Lxy, int3 n) {
  const int id = blockIdx.x * blockDim.x + threadIdx.x;
  int2 ik = make_int2(id % (n.x / 2 + 1), id / (n.x / 2 + 1));
  if (id >= n.y * (n.x / 2 + 1)) {
    return;
  ...
  for (int i = 0; i < n.z; i++) {
    ...
    const real ekz = exp(-k * z);
    vxcorr[i] = {
        -kvec.x / k * (-k * w0.y + kvec.x * u0.x + kvec.y * v0.x) * z * ekz +
            u0.x * ekz,
        -kvec.x / k * (k * w0.x + kvec.x * u0.y + kvec.y * v0.y) * z * ekz +
            u0.y * ekz};
    vycorr[i] = {
        -kvec.y / k * (-k * w0.y + kvec.x * u0.x + kvec.y * v0.x) * z * ekz +
            v0.x * ekz,
        -kvec.y / k * (k * w0.x + kvec.x * u0.y + kvec.y * v0.y) * z * ekz +
            v0.y * ekz};
    vzcorr[i] = {
        (k * w0.x + kvec.x * u0.y + kvec.y * v0.y) * z * ekz + w0.x * ekz,
...
FluidData<complex> Correction::computeAnalyticalCorrectionFourierSpace(
    const FluidData<complex> &insideSolution, cudaStream_t st) {
  int3 n = cells;
  int nk = (n.x / 2 + 1) * n.y;
  ...
  if (mode == WallMode::slit) {
    ...
    correction_ns::fillRightHandSide<<<nblocks, blockSize, 0, st>>>(
        d_rhs, insideSolution.getPointers(), H, Lxy, n);
    auto d_invA = thrust::raw_pointer_cast(invA.data());
    correction_ns::
        computeAnalyticalCorrectionFourierSlit<<<nblocks, blockSize, 0, st>>>(
            analyticalCorrection.getPointers(), H, Lxy, viscosity, d_invA,
            d_rhs, n);
  } else if (mode == WallMode::bottom) {
    correction_ns::computeAnalyticalCorrectionFourierBottomWall<<<
        nblocks, blockSize, 0, st>>>(analyticalCorrection.getPointers(),
                                     insideSolution.getPointers(), H, Lxy, n);
  } else {
    analyticalCorrection.velocity.fillWithZero();
    thrust::fill(analyticalCorrection.pressure.begin(),
                 analyticalCorrection.pressure.end(), complex());
  }
  return analyticalCorrection;
}

std::vector<complex> evaluateSlitCorrectionMatrix(real2 kvec, real viscosity,
                                                  real H) {
  const complex o = {1, 0};
  const complex i = {0, 1};
  const complex z = {0, 0};
  ...
  const std::vector<complex> A{halfmu * o,
                               ekH * halfmu * o,
                               z,
                               z,
                               z,
...
void Correction::initializeSlitCorrectionMatrix() {
  ...
  std::for_each(it + 1, it + nk, // The zero mode is excluded
                [&](int i) {
                  ...
                  std::copy(h_invA_i.begin(), h_invA_i.end(),
                            h_invA.begin() + 8 * 8 * i);
                });
  this->invA = h_invA;
}
...
void Correction::initializeZeroModeBVPSolver() {
  if (mode != WallMode::none) {
    ...
    zeroModeCMatrix =
        gpu_container<real>(CandD.begin(), CandD.begin() + 2 * n.z);
    zeroModeDMatrix = {CandD[2 * n.z], CandD[2 * n.z + 1], CandD[2 * n.z + 2],
                       CandD[2 * n.z + 3]};
    CudaCheckError();
  }
}
