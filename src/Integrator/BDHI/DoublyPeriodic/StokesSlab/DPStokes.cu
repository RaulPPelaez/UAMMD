...
FluidData<complex>
DPStokes::solveBVPVelocity(DataXYZ<complex> &gridForcesFourier,
                           cudaStream_t st) {
  ...
  const auto d_precomputedPressure =
      thrust::raw_pointer_cast(zeroModePressureChebyshevIntegrals.data());
  const auto d_precomputedVelocity =
      thrust::raw_pointer_cast(zeroModeVelocityChebyshevIntegrals.data());
  const int blockSize = 64;
  const int numberBlocks = numberSystems / blockSize + 1;
  solveBVPVelocityD<<<numberBlocks, blockSize, 0, st>>>(
      bvpSolver->getGPUSolver(), n.x, n.y, n.z, make_real2(Lx, Ly), H * 0.5,
      DataXYZPtr<complex>(gridForcesFourier), fluid.getPointers(), tmp,
      thrust::raw_pointer_cast(tmp_storage.data()), d_precomputedPressure,
      d_precomputedVelocity, viscosity, mode);
  CudaCheckError();
