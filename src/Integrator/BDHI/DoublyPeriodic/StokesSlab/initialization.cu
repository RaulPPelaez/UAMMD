/*Raul P. Pelaez 2019. Spectral/Chebyshev Doubly Periodic Stokes solver.
...
DPStokes::DPStokes(DPStokes::Parameters par)
  ...
  if (par.mode != WallMode::none) { // Correction is only needed for walls
    this->correction = std::make_shared<Correction>(
        H, make_real2(Lx, Ly), grid.cellDim, viscosity, par.mode);
  }
...
public:
  ...
  real getSecondIntegralFactor() const { return k != 0 ? (H * H * k) : (1.0); }
};
...
void DPStokes::initializeBoundaryValueProblemSolver() {
  ...
  auto klist = DPStokesSlab_ns::make_wave_vector_modulus_iterator(
      nk, make_real2(Lx, Ly));
  real halfH = H * 0.5;
  auto topdispatch =
      detail::BoundaryConditionsDispatch<detail::TopBoundaryConditions,
                                         decltype(klist)>(klist, halfH);
  auto topBC = thrust::make_transform_iterator(
      thrust::make_counting_iterator<int>(0), topdispatch);
  auto botdispatch =
      detail::BoundaryConditionsDispatch<detail::BottomBoundaryConditions,
                                         decltype(klist)>(klist, halfH);
  auto botBC = thrust::make_transform_iterator(
      thrust::make_counting_iterator<int>(0), botdispatch);
  int numberSystems = (nk.x / 2 + 1) * nk.y;
  int nz = grid.cellDim.z;
  this->bvpSolver = std::make_shared<BVP::BatchedBVPHandlerReal>(
      klist, topBC, botBC, numberSystems, halfH, nz);
  CudaCheckError();
}
...
std::vector<real> computeZeroModeIntegrals(real H, int nz, VelOrPressure mode) {
  ...
  forj(0, Nint) { zwt[j] = 0.5 * H * chebyshev::clencurt(j, Nint - 1); }
  fori(0, nz) {
    forj(0, Nint) {
...
std::vector<real> computeZeroModePressureChebyshevIntegral(real H, int nz) {
  return std::move(
      computeZeroModeIntegrals(H, nz, detail::VelOrPressure::pressure));
}

std::vector<real> computeZeroModeVelocityChebyshevIntegral(real H, int nz) {
  return std::move(
      computeZeroModeIntegrals(H, nz, detail::VelOrPressure::velocity));
}
...
void DPStokes::precomputeIntegrals() {
  ...
  zeroModeVelocityChebyshevIntegrals =
      detail::computeZeroModeVelocityChebyshevIntegral(H, nz);
  zeroModePressureChebyshevIntegrals =
      detail::computeZeroModePressureChebyshevIntegral(H, nz);
}
