...
BVPKernelTemporalStorage
setUpBVPKernelTemporalStorage(int numberConcurrentThreads, int nz) {
  BVP::StorageRegistration mem(numberConcurrentThreads);
  BVPKernelTemporalStorage tmp;
  tmp.rightHandSideChebyshevCoefficients =
      mem.registerStorageRequirement<complex>(nz);
  tmp.secondDerivativeChebyshevCoefficients =
      mem.registerStorageRequirement<complex>(nz);
  tmp.pressureChebyshevCoefficients =
      mem.registerStorageRequirement<complex>(nz);
  tmp.velocityChebyshevCoefficients =
      mem.registerStorageRequirement<complex>(nz);
  tmp.allocationSize = mem.getRequestedStorageBytes();
  return tmp;
}

// Computes the right hand side for the different PDEs (velocity xyz and
// pressure)
...
public:
  ...
  __device__ void pressure(ForceIterator &fxn, ForceIterator &fyn,
                           ForceIterator &fzn, RightHandSideIterator &rhs) {
  ...
  __device__ void perpendicularVelocity(const ForceIterator &fzn,
    ...
    for (int i = nz - 1; i >= 0; i--) {
      ...
      if (i <= nz - 2)
        Dpni = Dpnp2 + real(2.0) * (i + 1) * pressure[i + 1] / H;
      if (i == 0)
        Dpni *= real(0.5);
...
private:
  template <int dir, class ForceIterator, class RightHandSideIterator,
            class PressureIterator>
...
public:
  ...
  __device__
  VelocityBoundaryConditionsRightHandSide(const PressureIterator &pressure,
                                          real kmod, int nz)
      : kmod(kmod) {
    pH = pmH = complex();
    fori(0, nz) {
...
__global__ void solveBVPVelocityD(
    BVPSolver bvp, int nkx, int nky, int nz, real2 Lxy,
    real H, // Domain is [-H, H]
    const DataXYZPtr<complex> gridForce, FluidPointers<complex> fluid,
    BVPKernelTemporalStorage tmp, char *tmp_storage_raw_memory,
    const real *precomputedVelocityChebyshevIntegrals,
    const real *precomputedPressureChebyshevIntegrals, real mu, WallMode mode) {
  ...
  auto pressure =
      make_third_index_iterator(fluid.pressure, ik.x, ik.y, indexer);
  // Zero mode is added as a correction when there are walls
  if (id == 0) {
    // If there are no walls the zero mode is just zero
    // In other modes we need the forces stored in the zero mode, so do not
    // overwrite.
  ...
  RightHandSideCompute rhsCompute(nz, H, waveVector, mu, isUnpairedX,
                                  isUnpairedY);
  BVP::StorageRetriever stor(numberSystems, id, tmp_storage_raw_memory);
  auto rightHandSide =
      stor.retrieveStorage(tmp.rightHandSideChebyshevCoefficients);
  //      auto pressure =
  //      stor.retrieveStorage(tmp.pressureChebyshevCoefficients);
  auto an = stor.retrieveStorage(tmp.secondDerivativeChebyshevCoefficients);
  //      auto velocity =
  //      stor.retrieveStorage(tmp.velocityChebyshevCoefficients);
  // PRESSURE SOLVE
  ...
  VelocityBoundaryConditionsRightHandSide rhs_bc(pressure, k, nz);
  {
    ...
    {
      const complex alpha =
          rhs_bc.computeTopParallel((not isUnpairedX) * waveVector.x, mu);
      const complex beta =
          rhs_bc.computeBottomParallel((not isUnpairedX) * waveVector.x, mu);
    ...
    {
      const complex alpha =
          rhs_bc.computeTopParallel((not isUnpairedY) * waveVector.y, mu);
      const complex beta =
          rhs_bc.computeBottomParallel((not isUnpairedY) * waveVector.y, mu);
