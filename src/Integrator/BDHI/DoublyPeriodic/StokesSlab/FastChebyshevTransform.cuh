/*Raul P. Pelaez 2020-2021. Fast Chebyshev Transform for the Doubly Periodic
 * Stokes solver.
 */
...
template <class T>
__global__ void
scaleFFTToForwardChebyshevTransform(const DataXYZPtr<T> signalin,
...
template <class T>
__global__ void scaleFFTToInverseChebyshevTransform(DataXYZPtr<T> signal,
                                                    int3 n) {
  const int id = blockIdx.x * blockDim.x + threadIdx.x;
  if (id >= (n.x / 2 + 1) * n.y * n.z)
    return;
  const int iz = id / ((n.x / 2 + 1) * n.y);
  // signal[id] *= ((iz==0 or iz ==
  // n.z-1)?real(1.0):real(0.5))/real(n.x*n.y*(2*n.z-2)); signal[id] *=
  // real(2.0)*n.z-real(2.0);
  auto pm = (iz == 0 or iz == (n.z - 1)) ? real(1.0) : real(2.0);
...
public:
  FastChebyshevTransform(int3 gs) : gridSize(gs) {
    System::log<System::DEBUG>("[FastChebyshevTransform] Initialized");
    initCuFFT();
  }
  // In order to use cufft correctly I have to go back and forth between a AoS
  // and SoA layout, there has to be a better way.
  auto forwardTransform(DataXYZ<real> &gridData, cudaStream_t st) {
    System::log<System::DEBUG2>(
        "[DPStokesSlab] Transforming to wave/Chebyshev space");
    ...
    fct_ns::periodicExtension<<<nblocks, blockSize, 0, st>>>(
        DataXYZPtr<real>(gridData), make_int3(2 * (n.x / 2 + 1), n.y, n.z));
    CudaCheckError();
    DataXYZ<complex> gridDataFou((2 * n.z - 2) * n.y * (n.x / 2 + 1));
    CufftSafeCall(cufftExecReal2Complex<real>(cufft_plan_forward, gridData.x(),
                                              gridDataFou.x()));
    CufftSafeCall(cufftExecReal2Complex<real>(cufft_plan_forward, gridData.y(),
                                              gridDataFou.y()));
    CufftSafeCall(cufftExecReal2Complex<real>(cufft_plan_forward, gridData.z(),
                                              gridDataFou.z()));
    ...
    CudaCheckError();
    System::log<System::DEBUG2>(
        "[DPStokesSlab] Taking forces to wave/Chebyshev space");
    return gridDataFou;
  }
  ...
  auto inverseTransform(DataXYZ<complex> &gridDataFourier, cudaStream_t st) {
    ...
    fct_ns::scaleFFTToInverseChebyshevTransform<<<nblocks, blockSize, 0, st>>>(
        d_gridDataFou, make_int3(n.x, n.y, n.z));
    fct_ns::periodicExtension<<<nblocks, blockSize, 0, st>>>(
        d_gridDataFou, make_int3((n.x / 2 + 1), n.y, n.z));
    DataXYZ<real> gridData(2 * (n.x / 2 + 1) * n.y * (2 * n.z - 2));
    CufftSafeCall(cufftExecComplex2Real<real>(
        cufft_plan_inverse, gridDataFourier.x(), gridData.x()));
    CufftSafeCall(cufftExecComplex2Real<real>(
        cufft_plan_inverse, gridDataFourier.y(), gridData.y()));
    CufftSafeCall(cufftExecComplex2Real<real>(
        cufft_plan_inverse, gridDataFourier.z(), gridData.z()));
    CudaCheckError();
    return gridData;
  }
...
private:
  void initCuFFT() {
    ...
    CufftSafeCall(
        cufftMakePlanMany(cufft_plan_forward, 3, &cdtmp.x, &inembed.x, 1,
                          inembed.x * inembed.y * inembed.z, &oembed.x, 1,
                          oembed.x * oembed.y * oembed.z,
                          CUFFT_Real2Complex<real>::value, 1, &cufftWorkSizef));
    ...
    CufftSafeCall(
        cufftMakePlanMany(cufft_plan_inverse, 3, &cdtmp.x, &oembed.x, 1,
                          ...
                          CUFFT_Complex2Real<real>::value, 1, &cufftWorkSizei));
    System::log<System::DEBUG>("[DPStokesSlab] cuFFT grid size: %d %d %d",
