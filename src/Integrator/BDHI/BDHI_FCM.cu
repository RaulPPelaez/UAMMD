...
auto FCMIntegrator::computeHydrodynamicDisplacements() {
  ...
  auto torque =
      pd->getTorqueIfAllocated(access::location::gpu, access::mode::read);
  auto dir =
      pd->getDirIfAllocated(access::location::gpu, access::mode::readwrite);
  int numberParticles = pg->getNumberParticles();
  return fcm->computeHydrodynamicDisplacements(pos.raw(), force.raw(),
                                               torque.raw(), numberParticles,
...
__global__ void integrateEulerMaruyamaD(real4 *pos, real4 *dir,
                                        IndexIterator indexIterator,
                                        const real3 *linearV,
                                        const real3 *angularV, int N, real dt) {
...
void FCMIntegrator::forwardTime() {
  ...
  real3 *d_angularV =
      dir.raw() ? thrust::raw_pointer_cast(angularVelocities.data()) : nullptr;
  ...
  FCM_ns::integrateEulerMaruyamaD<<<nblocks, nthreads, 0, st>>>(
      pos.raw(), dir.raw(), indexIter, d_linearV, d_angularV, numberParticles,
      dt);
  CudaCheckError();
}
} // namespace BDHI
