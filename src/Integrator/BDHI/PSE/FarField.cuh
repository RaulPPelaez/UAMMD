...
public:
  ...
  Kernel(int P, real width) {
    this->support = 2 * P + 1;
    this->prefactor =
        cbrt(1.0 / (width * width * width * pow(2.0 * M_PI, 1.5)));
...
__device__ real greensFunction(real3 waveVector, real shearStrain,
  ...
  /*The Hashimoto splitting function,
    ...
    eta is the splitting of the gaussian kernel used in the grid interpolation,
    see sec. 2 in [2]*/
...
__global__ void forceFourier2Vel(
    ...
    real shearStrain, real hydrodynamicRadius, real viscosity, real split,
    real eta, Grid grid) {
  ...
  const real B = greensFunction(waveVector, shearStrain, hydrodynamicRadius,
                                viscosity, split, eta, ncells);
...
__global__ void fourierBrownianNoise(cufftComplex3 *gridVelsFourier, Grid grid,
  ...
  */
  ...
  { // Compute for v_k wave number
    ...
    const real B = greensFunction(k, shearStrain, hydrodynamicRadius, viscosity,
                                  split, eta, nk);
    gridVelsFourier[id] +=
        sqrt(B) * projectFourier(shearWaveVector(k, shearStrain), noise);
  }
  ...
  if (cell.x == nk.x - cell.x or cell.x == 0) {
    ...
    const real B = greensFunction(k, shearStrain, hydrodynamicRadius, viscosity,
                                  split, eta, nk);
    gridVelsFourier[id_conj] +=
        sqrt(B) * projectFourier(shearWaveVector(k, shearStrain), factor);
  }
}

struct ToReal3 {
  template <class T> __device__ real3 operator()(T vec) {
...
public:
  ...
  FarField(std::shared_ptr<System> sys, Parameters par)
      : sys(sys), box(par.box), viscosity(par.viscosity),
        hydrodynamicRadius(par.hydrodynamicRadius), psi(par.psi) {
    ...
    initializeCuFFT();
    CudaCheckError();
    System::log<System::MESSAGE>("[BDHI_PSE] Gaussian kernel support: %d",
                                 this->kernel->support);
...
private:
  ...
  void convolveFourier(cached_vector<cufftComplex3> &gridVelsFourier,
                       cudaStream_t st) {
    ...
    detail::forceFourier2Vel<<<Nblocks, Nthreads, 0, st>>>(
        d_gridVelsFourier, d_gridVelsFourier, shearStrain, hydrodynamicRadius,
        viscosity, psi, eta, grid);
    CudaCheckError();
  }
  ...
  auto deterministicPart(real4 *pos, real4 *forces, int numberParticles,
    ...
    if (forces) {
      ...
      thrust::fill(thrust::cuda::par.on(st), gridVelsFourier.begin(),
                   gridVelsFourier.end(), cufftComplex3());
      return gridVelsFourier;
    }
  }
  ...
  auto inverseTransformVelocity(cached_vector<cufftComplex3> &gridVelsFourier,
                                cudaStream_t st) {
...
void FarField::initializeCuFFT() {
  ...
  System::log<System::DEBUG>("[BDHI::PSE] cuFFT grid size: %d %d %d", cdtmp.x,
                             cdtmp.y, cdtmp.z);
  ...
  System::log<System::DEBUG>("[BDHI::PSE] Necessary work space for cuFFT: %s",
  ...
  if (free_mem < cufftWorkSize) {
    System::log<System::EXCEPTION>(
        "[BDHI::PSE] Not enough memory in device to allocate cuFFT free %s, "
        "needed: %s!!, try lowering the splitting parameter!",
...
void FarField::initializeKernel(real tolerance) {
   ...
   * accuracy*/
  while (erfc(m / sqrt(2)) > 0.1 * tolerance)
    m += 0.01;
  // This is P in [2]
  int support;
  // Support must be odd
  while ((support = int(pow(m / C, 2) / M_PI + 0.5) + 1) % 2 == 0)
    m += tolerance;
  // P is in each direction, nearest neighbours -> P=1
  int P = support / 2;
  // If P is too large for the grid set it to the grid size-1 (or next even
  // number)
  const int3 n = grid.cellDim;
  ...
  if (support > minCellDim) {
    support = minCellDim;
    if (support % 2 == 0)
      support--; // minCellDim will be 3 at least
    P = support / 2;
...
void FarField::initializeGrid(real tolerance) {
  ...
  System::log<System::MESSAGE>("[BDHI::PSE] Far range wave number cut off: %f",
                               kcut);
