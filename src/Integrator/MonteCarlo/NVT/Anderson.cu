...
namespace uammd {
  namespace MC_NVT{
    ...
    Anderson<Pot>::Anderson(shared_ptr<ParticleData> pd,
      ...
      sys->log<System::MESSAGE>("[MC_NVT::Anderson] Temperature: %e", par.temperature);
	...
	this-> is2D = true;
      ...
      sys->log<System::MESSAGE>("[MC_NVT::Anderson] Box size: %e %e %e",
				grid.box.boxSize.x, grid.box.boxSize.y,  grid.box.boxSize.z);
	...
	sys->log<System::ERROR>("[MC_NVT::Anderson] I cannot work with such a large cut off (%e) in this box (%e)!",
				rcut, box.boxSize.x);
    ...
    template<class Pot>
    void Anderson<Pot>::updateAcceptanceRatio(){
      uint naccepted = thrust::reduce(acceptedChanges.begin(), acceptedChanges.end(), 0);
      uint ntries = thrust::reduce(triedChanges.begin(), triedChanges.end(), 0);
      this->currentAcceptanceRatio = real(naccepted)/ntries;
    }

    template<class Pot>
    void Anderson<Pot>::updateJumpSize(){
    ...
    template<class Pot>
    void Anderson<Pot>::updateOrigin(){
      this->currentOrigin = make_real3(sys->rng().uniform3(-1.0, 1.0)*maxOriginDisplacement);
      if(is2D){
	currentOrigin.z = 0;
      }
	...
	__device__ real4 operator()(real4 pos){
	  return pos + make_real4(origin, 0);
	}
      };
    }

    template<class Pot>
    void Anderson<Pot>::updateListWithCurrentOrigin(){
    ...
    template<class Pot>
    void Anderson<Pot>::storeCurrentSortedPositions(){
      int numberParticles = pg->getNumberParticles();
      sortPos.resize(numberParticles);
      auto clData = cl->getCellList();
      auto origin_tr = Anderson_ns::ShiftPositionTransform(real(-1.0)*currentOrigin);
      thrust::transform(thrust::cuda::par, clData.sortPos, clData.sortPos + numberParticles, sortPos.begin(),
			origin_tr);
      CudaCheckError();
    }

    template<class Pot>
    void Anderson<Pot>::performStep(){
	...
	return newE.energy - oldE.energy;
      ...
      template<class CellList, class PotentialTransverser>
      __global__ void MCStepKernel(PotentialTransverser pot,
				   CellList cl,
				   real4* pos,
				   real3 origin,
				   int3 subgridOffset,
	...
	for(int attempt = 0; attempt < triesPerCell; attempt++){
	  ...
	  const bool isNewPositionInOldCell = checkIfPositionIsInCell(make_real3(newPos) + origin, celli, grid);
	  if(not isNewPositionInOldCell){
	    continue;
	  }
	  const real dH = computeEnergyDifference(group_i, newPos, oldPos, pos, cl, pot, celli);
	...
	thrust::fill(thrust::cuda::par, energyGroup, energyGroup + numberParticles, real());
      }
      int Nthreads=128;
