/*Raul P. Pelaez 2017. Verlet NVE Integrator module.
  ...
  par.energy = 1.0; //Target energy per particle, can be ommited if
  initVelocities=false par.dt = 0.01; par.is2D = false; par.initVelocities=true;
  //Modify starting velocities to ensure the target energy
...
public:
  struct Parameters {
    ...
    real dt = 0;
    bool is2D = false;
    bool initVelocities = true; // Modify initial particle velocities to enforce
                                // the provided energy
    // If >0 all particles will have this mass, otherwise the ParticleData mass
    // will be used, if masses have not been set
