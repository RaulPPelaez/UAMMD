...
namespace VerletNVT {
namespace Basic_ns {
// Fill the initial velocities of the particles in my group with a gaussian
// distribution according with my temperature.
__global__ void
initialVelocities(real3 *vel, const real *mass, real defaultMass,
                  ParticleGroup::IndexIterator
                      indexIterator, // global index of particles in my group
  ...
  double3 noisei = make_double3(rng.gd(0, vamp / mass_i),
                                is2D ? 0.0 : rng.gd(0, vamp / mass_i).x);
...
Basic::Basic(shared_ptr<ParticleGroup> pg, Basic::Parameters par,
             std::string name)
    : Integrator(pg, name), dt(par.dt), temperature(par.temperature),
      friction(par.friction), is2D(par.is2D), steps(0) {
  sys->rng().next32();
  sys->rng().next32();
  seed = sys->rng().next32();
...
void Basic::initVelocities() {
  ...
  if (pd->isVelAllocated()) {
    sys->log<System::WARNING>(
        "[%s] Velocity will be overwritten to ensure temperature conservation!",
        name.c_str());
  }
  ...
  Basic_ns::initialVelocities<<<Nblocks, Nthreads>>>(
      vel_handle.raw(), mass, defaultMass, groupIterator, velAmplitude, is2D,
      numberParticles, sys->rng().next32());
}

Basic::~Basic() { cudaStreamDestroy(stream); }
...
template <int step>
__global__ void integrateGPU(real4 *pos, real3 *vel, real4 *force,
                             const real *mass, real defaultMass,
  ...
  const real3 noisei =
      make_real3(rng.gf(0, noiseAmplitude), rng.gf(0, noiseAmplitude).x);
  vel[i] +=
      (make_real3(force[i]) * invMass - friction * vel[i]) * (dt * real(0.5)) +
      noisei;
  if (is2D)
    vel[i].z = real(0.0);
...
void Basic::resetForces() {
  ...
  thrust::fill(thrust::cuda::par.on(stream), force_group,
               force_group + numberParticles, real4());
}

template <int step> void Basic::callIntegrate() {
  ...
  int Nblocks =
      numberParticles / Nthreads + ((numberParticles % Nthreads) ? 1 : 0);
  Basic_ns::integrateGPU<step><<<Nblocks, Nthreads, 0, stream>>>(
...
real Basic::sumKineticEnergy() {
  ...
  if (defaultMass > 0) {
    ...
    verletnvt_ns::sumKineticEnergy<<<Nblocks, Nthreads>>>(
