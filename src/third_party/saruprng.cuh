...
class Saru {
 public:
  ...
  inline CUDAENABLED Saru(unsigned int seed);
  inline CUDAENABLED Saru(unsigned int seed1, unsigned int seed2);
  inline CUDAENABLED Saru(unsigned int seed1, unsigned int seed2, unsigned int seed3);

  
  /* Efficient compile-time computed advancements */
  template <unsigned int steps> inline CUDAENABLED void advance() 
  ...
  template <unsigned int steps> inline CUDAENABLED void rewind() 
    { rewindWeyl<steps>(); advanceLCG<-steps>(); }

  /* Slower (but still reasonable) run-time computed advancement */
  inline CUDAENABLED void advance(unsigned int steps);

  void setstate(unsigned int istate, unsigned int iwstate)
    { state=istate; wstate=iwstate; }

  template <unsigned int seed> inline CUDAENABLED Saru fork() const;

  template <unsigned int steps> inline CUDAENABLED unsigned int u32();
  template <unsigned int steps> inline CUDAENABLED float f();
  template <unsigned int steps> inline CUDAENABLED double d();
  template <unsigned int steps> inline CUDAENABLED float f(float low, float high);
  template <unsigned int steps> inline CUDAENABLED double d(double low, double high);
  inline CUDAENABLED unsigned int u32();
  inline CUDAENABLED float f();
  inline CUDAENABLED double d();
  inline CUDAENABLED float f(float low, float high);
  inline CUDAENABLED double d(double low, double high);
  
  //Gaussian distribution using Box Muller
  inline CUDAENABLED float2 gf(float mean, float std){

    constexpr float pi2 = 2.0*M_PI;

    float u0;
    //You really dont want to eval log(0)
    do{u0 = f();}while(u0<=FLT_MIN);
    
    const float u1 = f();

    const float r = sqrtf(-2.0f*logf(u0));
    const float theta = pi2*u1;
    return make_float2(r*sinf(theta), r*cosf(theta))*std + mean;
  }

  inline CUDAENABLED double2 gd(double mean, double std){

    constexpr double pi2 = 2.0*M_PI;

    double u0;
    //You really dont want to eval log(0)
    do{u0 = f();}while(u0<=DBL_MIN);
    
    const double u1 = f();

    const double r = sqrtf(-2.0*logf(u0));
    const double theta = pi2*u1;
    return make_double2(r*sinf(theta), r*cosf(theta))*std + mean;
  }
 ...
 private:
  ...
  template <unsigned int offset, unsigned int delta, 
    ...
    inline CUDAENABLED unsigned int advanceAnyWeyl(unsigned int);
...
/* Compile-time template function to efficently advance a state x with
  ...
  template <unsigned int steps> inline CUDAENABLED void advanceLCG()
    { state=CTpow<LCGA, steps>::value*state+LCGC*CTpowseries<LCGA, steps>::value; }

  template <unsigned int steps> inline CUDAENABLED void advanceWeyl()
    { wstate=advanceAnyWeyl<oWeylOffset, oWeylDelta, oWeylPeriod, steps>(wstate); }


  template <unsigned int steps> inline CUDAENABLED void rewindWeyl()
...
template <> inline CUDAENABLED void Saru::advanceWeyl<1>() /* especially efficient single step  */
{
...
inline CUDAENABLED Saru::Saru(unsigned int seed) 
{
...
inline CUDAENABLED Saru::Saru(unsigned int seed1, unsigned int seed2)
{
  seed2+=seed1<<16;
  seed1+=seed2<<11;
...
inline CUDAENABLED Saru::Saru(unsigned int seed1, unsigned int seed2, unsigned int seed3) 
{
...
template <unsigned int offset, unsigned int delta, 
	  unsigned int modulus, unsigned int steps> 
  inline CUDAENABLED unsigned int Saru::advanceAnyWeyl(unsigned int x) 
{
...
inline CUDAENABLED void Saru::advance(unsigned int steps)
{
...
inline CUDAENABLED Saru Saru::fork() const
{
...
inline CUDAENABLED unsigned int Saru::u32()
{
  advanceLCG<steps>();
  advanceWeyl<steps>();
...
template <unsigned int steps>
inline CUDAENABLED float Saru::f()
{
...
template <unsigned int steps>
inline CUDAENABLED float Saru::f(float low, float high)
{
...
template <unsigned int steps>
inline CUDAENABLED double Saru::d()
{
...
template <unsigned int steps>
inline CUDAENABLED double Saru::d(double low, double high)
{
  const double TWO_N32 = 0.232830643653869628906250e-9; /* 2^-32 */
  signed int v=(signed int)u32<steps>(); // deliberate cast to signed int for conversion speed
