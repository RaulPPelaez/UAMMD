...
namespace uammd{
  namespace BDHI{
    ...
    void FCM::initializeGrids(Parameters par){
      int3 cellDim;
      real h;
      if(par.cells.x<=0){
        ...
        h = Kernel::adviseGridSize(par.hydrodynamicRadius, par.tolerance, par.width);
    ...
    void FCM::initializeKernel(Parameters par){
      ...
      real factor = Kernel::computeUpsampling(par.tolerance);
      real hrot = hydrodynamicRadius/(pow(6*sqrt(M_PI),real(1.0)/real(3.0)));
      hrot/=factor;
      kernelRot = std::make_shared<Kernel>(hrot, par.tolerance);
    }
    ...
    */
    ...
    void FCM::Mdot(real3 *Mv, vtype *v, cudaStream_t st){
      ...
      //interpolateAngularVelocity(Mw,st);
	...
	*/
    ...
    template<typename vtype>
    void FCM::spreadTorques(vtype *quantity, cudaStream_t st){
      /*Spread force on particles to grid positions -> S·F*/
      sys->log<System::DEBUG2>("[BDHI::FCM] Particles to grid");
      int numberParticles = pg->getNumberParticles();
      auto torque = pd->getTorque(access::location::gpu, access::mode::read);
      auto torque_r3 = thrust::make_transform_iterator(torque.begin(), FCM_ns::ToReal3());
      auto pos = pd->getPos(access::location::gpu, access::mode::read);
      auto d_gridVels = (real3*)thrust::raw_pointer_cast(gridVels.data());
      IBM<Kernel> ibm(sys, kernelRot, grid);
      ibm.spreadAngular(pos.begin(), torque_r3, d_gridVels, numberParticles, st);
      CudaCheckError();
    }
    ...
    void FCM::interpolateAngularVelocity(real3* MT, cudaStream_t st){
      sys->log<System::DEBUG2>("[BDHI::FCM] Grid to particles");
      /*Interpolate the real space velocities back to the particle positions ->
	Output: Mv = Mw·F + sqrt(2*T/dt)·√Mw·dWw = σ·St·FFTi·(B·FFT·S·F + 1/√σ·√B·dWw )*/
      int numberParticles = pg->getNumberParticles();
      ...
      auto d_gridVels = (real3*)thrust::raw_pointer_cast(gridVels.data());
      IBM<Kernel> ibm(sys, kernel, grid);
      ibm.gatherAngular(pos.begin(), MT, d_gridVels, numberParticles, st);
      CudaCheckError();
    }
